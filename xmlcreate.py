...
dlist_xml = '<DList Name=g{0}DList{1} Offset="0x{2}"/>'
collision_xml = '<Collision Name=g{0}Collision{1} Offset="0x{2}"/>'
animation_xml = '<Animation Name=g{0}Anim{1} Offset="0x{2}"/>'
skeleton_xml = '<Collision Name=g{0}Skel{1} Type="{3}" LimbType="{4}" Offset="0x{2}"/>'
...
def unknown_to_xml(offset, type):
    ...
    return unknown_xml.format(name,animation_count,offset,type)
...
parser.add_argument('name', help='name to use for generated xml')
