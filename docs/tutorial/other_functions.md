...
void func_80A87CEC(EnJj *this, PlayState *play) {
    ...
    Camera_SetStateFlagEnd(play->cameraPtrs[play->activeCamId]);
...
matches, but generates a complier warning for `Camera_SetStateFlagEnd`, which it can't find. To fix this, add it to `functions.h`, in as near as possible the correct position in numerical order. Some detective work with VSCode's Search shows that this function lives in `z_camera.c`, and its prototype is `s16 Camera_SetStateFlagEnd(Camera* camera)`, so add this line to `functions.h` at the bottom of the camera functions part.
