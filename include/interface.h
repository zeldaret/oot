...
typedef enum RestrictionHGauge {
    RESTR_HGAUGE_0,
    RESTR_HGAUGE_2 = 2
} RestrictionHGauge;

typedef enum RestrictionBButton {
    RESTR_B_BTN_0,
    RESTR_B_BTN_1,
    RESTR_B_BTN_2
} RestrictionBButton;

typedef enum RestrictionAButton {
    RESTR_A_BTN_0
} RestrictionAButton;

typedef enum RestrictionBottles {
    RESTR_BOTTLES_ON,   // Bottles are allowed
    RESTR_BOTTLES_OFF,  // Bottles are disabled
    RESTR_BOTTLES_OFF_2 // Same as RESTR_BOTTLES_OFF
} RestrictionBottles;

typedef enum RestrictionTradeItems {
    RESTR_TRADE_ITEMS_ON,   // Trade items are allowed
    RESTR_TRADE_ITEMS_OFF,  // Trade items are disabled
    RESTR_TRADE_ITEMS_OFF_2 // Same as RESTR_TRADE_ITEMS_OFF
} RestrictionTradeItems;

typedef enum RestrictionHookshot {
    RESTR_HOOKSHOT_ON,   // Hookshot is allowed
    RESTR_HOOKSHOT_OFF,  // Hookshot is disabled
    RESTR_HOOKSHOT_OFF_2 // Same as RESTR_HOOKSHOT_OFF
} RestrictionHookshot;

typedef enum RestrictionOcarina {
    RESTR_OCARINA_ON,   // Ocarina is allowed
    RESTR_OCARINA_OFF,  // Ocarina is disabled
    RESTR_OCARINA_OFF_2 // Same as RESTR_OCARINA_OFF
} RestrictionOcarina;

typedef enum RestrictionWarpSongs {
    RESTR_WARP_SONGS_ON,   // Warp songs are allowed
    RESTR_WARP_SONGS_ON_1, // Same as RESTR_WARP_SONGS_ON
    RESTR_WARP_SONGS_ON_2, // Same as RESTR_WARP_SONGS_ON
    RESTR_WARP_SONGS_OFF   // Warp songs are disabled
} RestrictionWarpSongs;
...
typedef enum RestrictionFarores {
    RESTR_FARORES_ON,   // Farore's Wind is allowed
    RESTR_FARORES_OFF,  // Farore's Wind is disabled
    RESTR_FARORES_OFF_2 // Same as RESTR_FARORES_OFF
} RestrictionFarores;

typedef enum RestrictionDinsNayrus {
    RESTR_DINS_NAYRUS_ON,   // Din's Fire and Nayru's Love are allowed
    RESTR_DINS_NAYRUS_OFF,  // Din's Fire and Nayru's Love are disabled
    RESTR_DINS_NAYRUS_OFF_2 // Same as RESTR_DINS_NAYRUS_OFF
} RestrictionDinsNayrus;

typedef enum RestrictionAll {
    RESTR_ALL_ON,   // Items not covered by the other restriction flags are allowed
    RESTR_ALL_OFF,  // Items other than ocarina, bottles and trade items are disabled (except lens of truth in the treasure shop)
    RESTR_ALL_OFF_2 // Same as RESTR_ALL_OFF
} RestrictionAll;
