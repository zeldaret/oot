...
void __osMallocAddBlock(Arena* arena, void* start, s32 size) {
    ...
    if (start != NULL) {
        ...
        if (size2 > (s32)sizeof(ArenaNode)) {
            MemSet2(firstNode, BLOCK_UNINIT_MAGIC, size2);
            firstNode->next = NULL;
            firstNode->prev = NULL;
...
void ArenaImpl_RemoveAllBlocks(Arena* arena) {
    ...
    while (iter != NULL) {
        ...
        MemSet2(iter, BLOCK_UNINIT_MAGIC, iter->size + sizeof(ArenaNode)); // MemSet2
        iter = next;
    }
    ...
    while (iter != NULL) {
        if (iter->isFree && iter->size >= size) {
            ...
            if (arena->flag & FILL_ALLOCBLOCK) {
                MemSet2(alloc, BLOCK_ALLOC_MAGIC, size);
            }
...
void* __osMallocRDebug(Arena* arena, u32 size, const char* file, s32 line) {
    ...
    while (iter != NULL) {
        if (iter->isFree && iter->size >= size) {
            ...
            if (arena->flag & FILL_ALLOCBLOCK) {
                MemSet2(allocR, BLOCK_ALLOC_MAGIC, size);
            }
...
void __osFree_NoLock(Arena* arena, void* ptr) {
    ...
    if (arena->flag & FILL_FREEBLOCK) {
        MemSet2((u32)node + sizeof(ArenaNode), BLOCK_FREE_MAGIC, node->size);
    }
    ...
    if ((u32)next == (u32)node + sizeof(ArenaNode) + node->size && next->isFree) {
        ...
        if (arena->flag & FILL_FREEBLOCK) {
            MemSet2(next, BLOCK_FREE_MAGIC, sizeof(ArenaNode));
        }
        node->next = newNext;
        next = newNext;
    }
    ...
    if (prev != NULL && prev->isFree && (u32)node == (u32)prev + sizeof(ArenaNode) + prev->size) {
        ...
        if (arena->flag & FILL_FREEBLOCK) {
            MemSet2(node, BLOCK_FREE_MAGIC, sizeof(ArenaNode));
        }
    }
}
    ...
    if (ptr == NULL) {
        ...
        if (newSize == node->size) {
            ...
            if ((u32)next == ((u32)node + node->size + sizeof(ArenaNode)) && next->isFree && next->size >= sizeDiff) {
                ...
                node->next = newNext;
                node->size = newSize;
                MemMove(newNext, next, sizeof(ArenaNode));
            } else {
