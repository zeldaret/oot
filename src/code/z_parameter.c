...
u8 Item_Give(PlayState* play, u8 item) {
    ...
    if ((item >= ITEM_MEDALLION_FOREST) && (item <= ITEM_MEDALLION_LIGHT)) {
        ...
        return ITEM_NONE;
    } else if ((item >= ITEM_SPSTONE_KOKIRI_EMERALD) && (item <= ITEM_SPSTONE_ZORA_SAPPHIRE)) {
        gSaveContext.inventory.questItems |= gBitFlags[item - ITEM_SPSTONE_KOKIRI_EMERALD + QUEST_KOKIRI_EMERALD];
...
u8 Item_CheckObtainability(u8 item) {
    ...
    if ((item >= ITEM_MEDALLION_FOREST) && (item <= ITEM_MEDALLION_LIGHT)) {
        return ITEM_NONE;
    } else if ((item >= ITEM_SPSTONE_KOKIRI_EMERALD) && (item <= ITEM_SKULL_TOKEN)) {
        return ITEM_NONE;
    } else if ((item >= ITEM_SWORD_KOKIRI) && (item <= ITEM_SWORD_BIGGORON)) {
        if (item == ITEM_SWORD_BIGGORON) {
            return ITEM_NONE;
