...
s32 Object_Spawn(ObjectContext* objectCtx, s16 objectId) {
    ...
    DmaMgr_SyncDmaRequestDebug(objectCtx->status[objectCtx->num].segment, gObjectTable[objectId].vromStart, size,
                               "../z_scene.c", 145);
    ...
    for (i = 0; i < objectCtx->num; i++) {
        if (status->id < 0) {
            if (status->dmaRequest.vromAddr == 0) {
                ...
                DmaMgr_AsyncDmaRequest(&status->dmaRequest, status->segment, objectFile->vromStart, size, 0,
...
void func_800981B8(ObjectContext* objectCtx) {
    ...
    for (i = 0; i < objectCtx->num; i++) {
        ...
        osSyncPrintf("num=%d adrs=%x end=%x\n", objectCtx->num, (s32)objectCtx->status[i].segment + size,
                     objectCtx->spaceEnd);
        DmaMgr_SyncDmaRequestDebug(objectCtx->status[i].segment, gObjectTable[id].vromStart, size, "../z_scene.c", 342);
    }
}
