...
void Audio_InitNoteSub(Note* note, NoteSubEu* sub, NoteSubAttributes* attrs) {
    ...
    sub->logHarmonicCurAndPrev = note->noteSubEu.logHarmonicCurAndPrev;
 ...
 * @return s32 logHarmonic, the log of the harmonic for the synthetic wave generated by gWaveSamples
 */
s32 Audio_BuildSyntheticWave(Note* note, SequenceLayer* layer, s32 waveId) {
    ...
    note->playbackState.logHarmonic = logHarmonic;

    // Save the pointer to the synthethic wave
    // waveId index starts at 128, there are 64 samples read from
    note->noteSubEu.sound.waveSampleAddr = &gWaveSamples[waveId - 128][logHarmonic * 64];
...
void Audio_InitSyntheticWave(Note* note, SequenceLayer* layer) {
    ...
    prevLogHarmonic = note->playbackState.logHarmonic;
    curLogHarmonic = Audio_BuildSyntheticWave(note, layer, waveId);

    if (curLogHarmonic != prevLogHarmonic) {
        note->noteSubEu.logHarmonicCurAndPrev = (curLogHarmonic << 2) + prevLogHarmonic;
    }
}
