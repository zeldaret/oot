...
Vec3f* Quake_AddVec(Vec3f* dst, Vec3f* arg1, VecSph* arg2) {
    ...
    SphCoord_SphToVec3fRot90(&vec2, arg2);
...
void Quake_UpdateShakeInfo(QuakeRequest* req, ShakeInfo* shake, f32 y, f32 x) {
    ...
    if (req->unk_1C) {
        ...
        vec.y = 0;
        vec.z = 0;
        SphCoord_Vec3fToSphDiffRot90(&struc1, unk5C, unk50);
        struc2.r = req->y * y;
        ...
        if (req->callbackIdx != 0) {
            if (globalCtx->cameraPtrs[req->camPtrIdx] == 0) {
                ...
                if (sQuakeCallbacks[req->callbackIdx](req, &shake) == 0) {
                    ...
                    max = SphCoord_Vec3fDist(&shake.vec1, &vec) * absSpeedDiv;
                    max2 = SphCoord_Vec3fDist(&shake.vec2, &vec) * absSpeedDiv;
                    if (max < max2) {
                        max = max2;
                    }
