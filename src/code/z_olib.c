...
VecSph* OLib_Vec3fToVecSph(VecSph* dest, Vec3f* vec) {
    ...
    if ((dist == 0.0f) && (vec->y == 0.0f)) {
        ...
        sph.pitch = DEG_TO_BINANG(RAD_TO_DEG(Math_FAtan2F(dist, vec->y)));
    }
    ...
    if ((vec->x == 0.0f) && (vec->z == 0.0f)) {
        ...
        sph.yaw = DEG_TO_BINANG(RAD_TO_DEG(Math_FAtan2F(vec->x, vec->z)));
    }
...
Vec3s* OLib_Vec3fDiffBinAng(Vec3s* dest, Vec3f* a, Vec3f* b) {
    ...
    anglesBinAng.x = DEG_TO_BINANG(RAD_TO_DEG(anglesRad.x));
    anglesBinAng.y = DEG_TO_BINANG(RAD_TO_DEG(anglesRad.y));
    anglesBinAng.z = 0.0f;
