...
#define HORSETYPE_1 1
#define HORSETYPE_2 2
#define HORSETYPE_5 5
#define HORSETYPE_6 6
#define HORSETYPE_7 7
#define HORSETYPE_8 8
#define HORSETYPE_9 9
...
void func_8006D0EC(GlobalContext* globalCtx, Player* player) {
    ...
    HorseSpawn horseSpawns[] = {
        { SCENE_SPOT00, -460, 100, 6640, 0, HORSETYPE_2 }, { SCENE_SPOT06, -1929, -1025, 768, 0, HORSETYPE_2 },
        { SCENE_SPOT09, 2566, -259, 767, 0, HORSETYPE_2 }, { SCENE_SPOT12, -328, 10, 953, 0, HORSETYPE_2 },
        { SCENE_SPOT20, 928, 0, -2280, 0, HORSETYPE_2 },
    };
    ...
    if ((AREG(6) != 0) && (Flags_GetEventChkInf(0x18) || (DREG(1) != 0))) {
        player->rideActor = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, player->actor.world.pos.x,
                                        ...
                                        player->actor.shape.rot.y, player->actor.shape.rot.z, HORSETYPE_9);
        ...
        horseActor =
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, 3586.0f, 1413.0f, -402.0f, 0, 0x4000, 0, HORSETYPE_1);
        horseActor->room = -1;
    } else if ((gSaveContext.entranceIndex == 1230) && (gSaveContext.eventChkInf[1] & 0x100)) {
        Actor* horseActor =
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, -25.0f, 0.0f, -1600.0f, 0, -0x4000, 0, HORSETYPE_1);
        ...
        if (Horse_CheckSceneValid(gSaveContext.horseData.scene)) {
            Actor* horseActor = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE,
                                            ...
                                            gSaveContext.horseData.pos.z, 0, gSaveContext.horseData.angle, 0, HORSETYPE_1);
        ...
        Actor* horseActor =
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, 0.0f, 0.0f, -500.0f, 0, 0, 0, HORSETYPE_1);
        ...
        if ((DREG(1) == 0) && (globalCtx->sceneNum == SCENE_SOUKO) && !IS_DAY) {
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, 0.0f, 0.0f, -60.0f, 0, 0x7360, 0, HORSETYPE_1);
        }
    }
}
...
void func_8006D684(GlobalContext* globalCtx, Player* player) {
    ...
    if ((gSaveContext.entranceIndex == 0x028A || gSaveContext.entranceIndex == 0x028E ||
        ...
        player->rideActor = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, spawnPos.x, spawnPos.y,
                                        spawnPos.z, 0, player->actor.world.rot.y, 0, HORSETYPE_7);
        ...
        gSaveContext.horseData.scene = globalCtx->sceneNum;
               ...
               (Flags_GetEventChkInf(0x18) == 0) && (DREG(1) == 0)) {
        player->rideActor =
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, 894.0f, 0.0f, -2084.0f, 0, -0x7FFF, 0, HORSETYPE_5);
        ...
        static struct_8011F9B8 D_8011F9B8[] = {
            { 93, 0xFFF0, 0x0E10, 0x0585, 0x0168, 0x8001, HORSETYPE_8 }, { 99, 0xFFF0, 0xFF06, 0x0001, 0xF9D4, 0x4000, HORSETYPE_6 },
            { 99, 0xFFF1, 0x0000, 0x0000, 0x0000, 0x0000, HORSETYPE_5 }, { 99, 0xFFF5, 0x0000, 0x0000, 0x0000, 0x0000, HORSETYPE_7 },
            { 81, 0xFFF3, 0xF46F, 0x0139, 0x1E14, 0x0000, HORSETYPE_7 }, { 81, 0xFFF4, 0xF894, 0x0139, 0x1B67, 0x0000, HORSETYPE_7 },
            { 81, 0xFFF5, 0xF035, 0x0139, 0x1B15, 0x0000, HORSETYPE_7 }, { 81, 0xFFF6, 0xF035, 0x0139, 0x1B15, 0x0000, HORSETYPE_7 },
        };
        ...
        for (i = 0; i < ARRAY_COUNT(D_8011F9B8); i++) {
            if ((globalCtx->sceneNum == D_8011F9B8[i].scene) &&
                ...
                if (D_8011F9B8[i].type == HORSETYPE_7) {
                    ...
                    Actor_MountHorse(globalCtx, player, player->rideActor);
                    func_8002DE74(globalCtx, player);
                } else if ((D_8011F9B8[i].type == HORSETYPE_5) || (D_8011F9B8[i].type == HORSETYPE_6) || (D_8011F9B8[i].type == HORSETYPE_8)) {
                    Vec3f sp54;
                    s32 temp = 0;

                    if (((gSaveContext.eventInf[0] & 0x10) >> 4) && D_8011F9B8[i].type == HORSETYPE_6) {
                        temp = 0x8000;
                    }
