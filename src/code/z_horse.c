...
void func_8006D074(GlobalContext* globalCtx) {
    ...
    gSaveContext.memory.information.horseData.angle = -27353;
}

void func_8006D0AC(GlobalContext* globalCtx) {
    if (gSaveContext.memory.information.horseData.scene == SCENE_SPOT06) {
        gSaveContext.memory.information.horseData.scene = SCENE_SPOT06;
        gSaveContext.memory.information.horseData.pos.x = -2065;
        gSaveContext.memory.information.horseData.pos.y = -863;
        gSaveContext.memory.information.horseData.pos.z = 1839;
        gSaveContext.memory.information.horseData.angle = 0;
    }
}
...
void func_8006D0EC(GlobalContext* globalCtx, Player* player) {
    ...
    if ((AREG(6) != 0) && (Flags_GetEventChkInf(0x18) || (DREG(1) != 0))) {
        ...
        gSaveContext.memory.information.horseData.scene = globalCtx->sceneNum;
        ...
        horseActor->room = -1;
    } else if ((gSaveContext.memory.entranceIndex == 1230) &&
               (gSaveContext.memory.information.eventChkInf[1] & 0x100)) {
        Actor* horseActor;
        horseActor =
    ...
    } else if ((globalCtx->sceneNum == gSaveContext.memory.information.horseData.scene) &&
               (Flags_GetEventChkInf(0x18) != 0 || DREG(1) != 0)) {
        // Translates to: "SET BY EXISTENCE OF HORSE %d %d %d"
        osSyncPrintf("馬存在によるセット %d %d %d\n", gSaveContext.memory.information.horseData.scene,
                     Flags_GetEventChkInf(0x18), DREG(1));

        if (func_8006CFC0(gSaveContext.memory.information.horseData.scene)) {
            Actor* horseActor;
            horseActor = Actor_Spawn(
                &globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, gSaveContext.memory.information.horseData.pos.x,
                gSaveContext.memory.information.horseData.pos.y, gSaveContext.memory.information.horseData.pos.z, 0,
                gSaveContext.memory.information.horseData.angle, 0, 1);
            if (horseActor == NULL) {
        ...
        if ((DREG(1) == 0) && (globalCtx->sceneNum == SCENE_SOUKO) && (gSaveContext.memory.nightFlag != 0)) {
...
void func_8006D684(GlobalContext* globalCtx, Player* player) {
    ...
    if ((gSaveContext.memory.entranceIndex == 0x028A || gSaveContext.memory.entranceIndex == 0x028E ||
        ...
        if (gSaveContext.memory.entranceIndex == 0x028A) {
            spawnPos = spawnPositions[0];
        } else if (gSaveContext.memory.entranceIndex == 0x028E) {
            spawnPos = spawnPositions[1];
        } else if (gSaveContext.memory.entranceIndex == 0x0292) {
        ...
        for (i = 0; i < ARRAY_COUNT(D_8011F9B8); i++) {
            if ((globalCtx->sceneNum == D_8011F9B8[i].scene) &&
                (((void)0, gSaveContext.memory.cutsceneIndex) == D_8011F9B8[i].cutsceneIndex)) {
                if (D_8011F9B8[i].type == 7) {
                    if ((globalCtx->sceneNum == 99) && (((void)0, gSaveContext.memory.cutsceneIndex) == 0xFFF1)) {
    ...
    if (LINK_IS_ADULT) {
        ...
        if (func_8006CFC0(globalCtx->sceneNum)) {
            if ((gSaveContext.sceneSetupIndex > 3) ||
                ((gSaveContext.memory.entranceIndex == 0x028A || gSaveContext.memory.entranceIndex == 0x028E ||
                  gSaveContext.memory.entranceIndex == 0x0292 || gSaveContext.memory.entranceIndex == 0x0476) &&
                 (gSaveContext.respawnFlag == 0)) ||
