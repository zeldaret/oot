...
void AudioSeq_InitSequenceChannel(SequenceChannel* channel) {
    ...
    channel->adsr.decayRateIndex = 0xF0;
...
s32 AudioSeq_SeqLayerProcessScriptStep2(SequenceLayer* layer) {
    ...
    for (;;) {
        ...
        switch (cmd) {
            ...
            case 0xCF:
                layer->adsr.decayRateIndex = AudioSeq_ScriptReadU8(state);
                break;
...
s32 AudioSeq_SeqLayerProcessScriptStep4(SequenceLayer* layer, s32 cmd) {
    ...
    switch (instOrWave) {
        case 0:
            ...
            layer->adsr.decayRateIndex = (u8)drum->decayRateIndex;
...
u8 AudioSeq_GetInstrument(SequenceChannel* channel, u8 instId, Instrument** instOut, AdsrSettings* adsr) {
    ...
    adsr->decayRateIndex = inst->decayRateIndex;
    *instOut = inst;
    instId += 2;
    return instId;
}
        ...
        if (command >= 0xB0) {
            ...
            if (command >= 0xF2) {
                ...
                switch (command) {
                    ...
                    case 0xD9:
                        command = (u8)parameters[0];
                        channel->adsr.decayRateIndex = command;
                        break;
                    case 0xD8:
                        command = (u8)parameters[0];
