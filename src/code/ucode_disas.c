    ...
    while (!exit)
    {
        ...
        switch (cmd)
        {
            ...
            case G_TEXRECT: // small reordering
            {
                ...
                DISAS_LOG("gsSPTextureRectangle(%d,%d,%d,%d,%d,%d,%d,%d,%d),",
                                rect.xh, rect.yh, rect.xl, rect.yl,
                                rect.tile,
                                gfx[1].words.w1 >> 16, gfx[1].words.w1 & 0xFFFF, 
                                gfx[2].words.w1 >> 16, gfx[2].words.w1 & 0xFFFF
                                );
            ...
            case G_SETTILE: // reordering
            {
                ...
                DISAS_LOG("gsDPSetTile(%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d),", 
                                settile.fmt, settile.siz , settile.line, settile.tmem,
                                settile.tile, settile.palette, (settile.ct << 2) + (settile.mt),
                                settile.maskt, settile.shiftt, (settile.cs << 2) + (settile.ms),
                                settile.masks, settile.shifts
                                );
            ...
            case G_LOADTILE: // minor reordering (branch instruction)
            {
                ...
                DISAS_LOG("gsDPLoadTile(%d,%d,%d,%d,%d),", loadtile.tile, loadtile.sh, loadtile.th, loadtile.sl, loadtile.tl);
                break;
            }
            ...
            case G_LOADBLOCK: // gfx instead of curGfx(?)
            {
                ...
                DISAS_LOG("gsDPLoadBlock(%d,%d,%d,%d,%d),", loadtile.tile, loadtile.sh, loadtile.th, loadtile.sl, loadtile.tl);
            ...
            case G_SETTILESIZE: // gfx instead of curGfx(?) + minor reordering (branch instruction)
            {
                // CE8
                Gloadtile loadtile = curGfx.loadtile;
                DISAS_LOG("gsDPSetTileSize(%d,%d,%d,%d,%d),", loadtile.tile, loadtile.sh, loadtile.th, loadtile.sl, loadtile.tl);
                break;
            }
            ...
            case G_SETCOMBINE: // gfx instead of curGfx(?) + minor reordering (branch instruction)
            {
                ...
                DISAS_LOG("gsDPSetCombineLERP(%s,%s,%s,%s, %s,%s,%s,%s, %s,%s,%s,%s, %s,%s,%s,%s),",
                                UCodeDisas_ParseCombineColor(setcombine.a, 1),
                                UCodeDisas_ParseCombineColor(setcombine.b, 2),
                                UCodeDisas_ParseCombineColor(setcombine.c, 3),
                                UCodeDisas_ParseCombineColor(setcombine.d, 4),
                                
                                UCodeDisas_ParseCombineAlpha(setcombine.z, 1),
                                UCodeDisas_ParseCombineAlpha(setcombine.y, 2),
                                UCodeDisas_ParseCombineAlpha(setcombine.x, 3),
                                UCodeDisas_ParseCombineAlpha(setcombine.w, 4),
                                
                                UCodeDisas_ParseCombineColor(setcombine.e, 1),
                                UCodeDisas_ParseCombineColor(setcombine.f, 2),
                                UCodeDisas_ParseCombineColor(setcombine.g, 3),
                                UCodeDisas_ParseCombineColor(setcombine.h, 4),

                                UCodeDisas_ParseCombineAlpha(setcombine.v, 1),
                                UCodeDisas_ParseCombineAlpha(setcombine.u, 2),
                                UCodeDisas_ParseCombineAlpha(setcombine.t, 3),
                                UCodeDisas_ParseCombineAlpha(setcombine.s, 4)
                                );
            ...
            case G_SETOTHERMODE_H: // small reordering
            {
                ...
                {
                    F3DZEX_SETRENDERMACRO("SetAlphaDither", G_MDSFT_ALPHADITHER, 2, G_AD_PATTERN, G_AD_NOTPATTERN, G_AD_NOISE, G_AD_DISABLE),
                    ...
                    if (sft == sUCodeDisasModeHMacros[i1].shift) {
                        for (i2 = 0; i2 < 4; i2++) {
                            ...
                            if (curGfx.setothermode.data == sUCodeDisasModeHMacros[i1].values[i2].value) {
                                // 1038
                                DISAS_LOG("gsDP%s(%s),", sUCodeDisasModeHMacros[i1].name, sUCodeDisasModeHMacros[i1].values[i2].name);
                                goto block_1;
                            }
                        }
                    }
                        ...
                        if (sft == sUCodeDisasModeLMacros[i1].shift) {
                            for (i2 = 0; i2 < 4; i2++) {
                                if (curGfx.setothermode.data == sUCodeDisasModeLMacros[i1].values[i2].value) {
                                    // 1198
                                    DISAS_LOG("gsDP%s(%s),", sUCodeDisasModeLMacros[i1].name, sUCodeDisasModeLMacros[i1].values[i2].name);
                                    goto block_2;
                                }
                            }
                        }
...
block_2:
            ...
            case G_SETSCISSOR: // gfx instead of curGfx(?) + reordering
            {
                ...
                modeStr = (setscissor.pad == G_SC_NON_INTERLACE)
                    ...
                    : (setscissor.pad == G_SC_ODD_INTERLACE)
                        ? "G_SC_ODD_INTERLACE"
                        : (setscissor.pad == G_SC_EVEN_INTERLACE)
                            ? "G_SC_EVEN_INTERLACE"
                            : "???";
                if ((setscissor.x0frac | setscissor.y0frac | setscissor.x1frac | setscissor.y1frac)) {
                    ...
                    DISAS_LOG("gsDPSetScissorFrac(%s, %d, %d, %d, %d),", modeStr,
                                    (setscissor.x0 << 2) + setscissor.x0frac,
                                    (setscissor.y0 << 2) + setscissor.y0frac, 
                                    (setscissor.x1 << 2) + setscissor.x1frac,
                                    (setscissor.y1 << 2) + setscissor.y1frac);
                }
                else {
                    // 13E8
                    DISAS_LOG("gsDPSetScissor(%s, %d, %d, %d, %d),", modeStr,
                                    setscissor.x0,
                                    setscissor.y0, 
                                    setscissor.x1,
                                    setscissor.y1);
            ...
            case G_SETCIMG: // reordering
            {
                ...
                DISAS_LOG("gsDPSetColorImage(G_IM_FMT_%s, G_IM_SIZ_%s, %d, 0x%08x(0x%08x) ),",
                                (fmt == G_IM_FMT_RGBA) ? "RGBA" : (fmt == G_IM_FMT_YUV) ? "YUV" : (fmt == G_IM_FMT_CI) ? "CI" : (fmt == G_IM_FMT_IA) ? "IA" : "I",
                                (siz == G_IM_SIZ_4b) ? "4b" : (siz == G_IM_SIZ_8b) ? "8b" : (siz == G_IM_SIZ_16b) ? "16b" : "32b",
            ...
            {
                // 17A0
                DISAS_LOG("gsDPSetBlendColor(%d, %d, %d, %d),", curGfx.setcolor.r, curGfx.setcolor.g, curGfx.setcolor.b, curGfx.setcolor.a);
            ...
            {
                // 1800
                DISAS_LOG("gsDPSetFogColor(%d, %d, %d, %d),", curGfx.setcolor.r, curGfx.setcolor.g, curGfx.setcolor.b, curGfx.setcolor.a);
            ...
            {
                // 18FC
                DISAS_LOG("gsDPSetPrimColor(%d, %d, %d, %d, %d, %d),", curGfx.setcolor.prim_min_level, curGfx.setcolor.prim_level, curGfx.setcolor.r, curGfx.setcolor.g, curGfx.setcolor.b, curGfx.setcolor.a);
                break;
            }
            case G_RDPFULLSYNC:
            {
            ...
            case G_NOOP:
            {
                ...
                switch (curGfx.noop.type)
                {
                    ...
                    {
                        // 1BA0
                        DISAS_LOG("gsDPNoOpTag3(%02x, %08x, %04x),", curGfx.noop.type, curGfx.noop.value.u32, curGfx.noop.len);
                        break;
                    }
                }
                break;
                ...
                switch (this->ucodeType)
                {
                        ...
                        switch (curGfx.dma.cmd) // which one to choose here?
                        {
                            case G_MTX: // gfx instead of curGfx(?) + lbu -> lb + reordering
                            {
                                ...
                                for(i1 = 0; i1 < ARRAY_COUNT(sUCodeDisasMtxFlags); i1++) {
                                    DISAS_LOG("|%s", (sUCodeDisasMtxFlags[i1].value & params) ? sUCodeDisasMtxFlags[i1].setName : sUCodeDisasMtxFlags[i1].unsetName);
                                }
                                ...
                                if (this->enableLog >= 2) {
                                    ...
                                    DISAS_LOG(
                                        ...
                                        "| %04x.%04x %04x.%04x %04x.%04x %.04x.%04x || %12.6f %12.6f %12.6f %12.6f |\n"
                                        "\\ %04x.%04x %04x.%04x %04x.%04x %.04x.%04x /\\ %12.6f %12.6f %12.6f %12.6f /\n",
                                        mtxp->intPart[0][0], mtxp->fracPart[0][0], mtxp->intPart[0][1], mtxp->fracPart[0][1],
                                        mtxp->intPart[0][2], mtxp->fracPart[0][2], mtxp->intPart[0][3], mtxp->fracPart[0][3],
                                        mtx.mf[0][0], mtx.mf[0][1], mtx.mf[0][2], mtx.mf[0][3],

                                        mtxp->intPart[1][0], mtxp->fracPart[1][0], mtxp->intPart[1][1], mtxp->fracPart[1][1],
                                        mtxp->intPart[1][2], mtxp->fracPart[1][2], mtxp->intPart[1][3], mtxp->fracPart[1][3],
                                        mtx.mf[1][0], mtx.mf[1][1], mtx.mf[1][2], mtx.mf[1][3],

                                        mtxp->intPart[2][0], mtxp->fracPart[2][0], mtxp->intPart[2][1], mtxp->fracPart[2][1],
                                        mtxp->intPart[2][2], mtxp->fracPart[2][2], mtxp->intPart[2][3], mtxp->fracPart[2][3],
                                        mtx.mf[2][0], mtx.mf[2][1], mtx.mf[2][2], mtx.mf[2][3],

                                        mtxp->intPart[3][0], mtxp->fracPart[3][0], mtxp->intPart[3][1], mtxp->fracPart[3][1],
                                        mtxp->intPart[3][2], mtxp->fracPart[3][2], mtxp->intPart[3][3], mtxp->fracPart[3][3],
                                        mtx.mf[3][0], mtx.mf[3][1], mtx.mf[3][2], mtx.mf[3][3]
                                    );
                                }
                                break;
                            }
                            ...
                            case G_TRI1: // a Gtri1 struct should probably be used here + code missing
                            {
                                ...
                                DISAS_LOG("gsSP1Triangle(%d, %d, %d),", ((words2.w0 >> 16) & 0xFF)/2, ((words2.w0 >> 8) & 0xFF)/2, ((words2.w0 >> 0) & 0xFF)/2);
                            ...
                            case G_TRI2: // gfx instead of curGfx(?) + a Gtri2 struct should probably be used here + code missing
                            {
                                Gwords words2 = curGfx.words;
                                DISAS_LOG("gsSP2Triangles(%d, %d, %d, 0, %d, %d, %d, 0),",
                                                ((words2.w0 >> 16) & 0xFF)/2, ((words2.w0 >> 8) & 0xFF)/2, ((words2.w0 >> 0) & 0xFF)/2,
                                                ((words2.w1 >> 16) & 0xFF)/2, ((words2.w1 >> 8) & 0xFF)/2, ((words2.w1 >> 0) & 0xFF)/2
                                                );
                            ...
                            case G_QUAD: // gfx instead of curGfx(?)  + a Gquad struct should probably be used here + code missing
                            {
                                Gwords words2 = curGfx.words;
                                DISAS_LOG("gsSP1Quadrangle(%d, %d, %d, %d, 0),",
                                    ((words2.w0 >> 16) & 0xFF)/2, ((words2.w0 >> 8) & 0xFF)/2,
                                    ((words2.w0 >> 0) & 0xFF)/2, ((words2.w1 >> 0) & 0xFF)/2
                                    );
                                this->pipeSyncRequired = true;
                                this->quadCnt++;
                                break;
                            ...
                            {
                                DISAS_LOG("gsSPCullDisplayList(%d, %d),", (curGfx.words.w0 & 0xFFFF)/2, (curGfx.words.w1 & 0xFFFF)/2);
                                break;
                            }
                            case G_BRANCH_Z:
                            {
                                GfxMod* dst = UCodeDisas_TranslateAddr(this, rdpHalf);
                                DISAS_LOG("gsSPBranchLessZraw(0x%08x(0x%08x), %d, 0x%08x),", rdpHalf, dst, (curGfx.words.w0 & 0xFFF)/2, curGfx.words.w1);
                                gfx = dst - 1;
                                break;
                            }
                            case G_TEXTURE: // gfx instead of curGfx(?) + reordering + arithmetic issue?
                            {
                                ...
                                if (texture.lodscale == 0) {
                                    DISAS_LOG("gsSPTexture(%d, %d, %d, %d, %s),", texture.s, texture.t, texture.tile >> 3, texture.tile & 7, texture.on ? "G_ON" : "G_OFF");
                                }
                                else {
                                    DISAS_LOG("gsSPTextureL(%d, %d, %d, %d, %d, %s),", texture.s, texture.t, texture.tile >> 3, texture.lodscale, texture.tile & 7, texture.on ? "G_ON" : "G_OFF");
                                }
                                break;
                            }
                            ...
                            case G_MOVEWORD: // gfx instead of curGfx(?) + missing code + missing copy
                            {
                                ...
                                switch (dma.par)
                                {
                                    ...
                                    case G_MW_CLIP:
                                    {
                                        DISAS_LOG("gsSPClipRatio(FRUSTRATIO_%d), ", (dma.addr != 0) ? dma.addr : -dma.addr );
                                        gfx += 4 - 1;
                                        break;
                                    }
                                    case G_MW_NUMLIGHT:
                            ...
                            case G_MOVEMEM: // gfx instead of curGfx(?) + code is wrong
                            {
                                ...
                                switch (index)
                                {
                                    case G_MV_VIEWPORT:
                                    {
                                        ...
                                        DISAS_LOG("\t# vscale=[%d %d %d %d], ", vp->vscale[0], vp->vscale[1], vp->vscale[2], vp->vscale[3]);
                                        DISAS_LOG("vtrans=[%d %d %d %d] ", vp->vtrans[0], vp->vtrans[1], vp->vtrans[2], vp->vtrans[3]);
                                        break;
                                    }
                                    case G_MV_MATRIX:
                                    {
                    ...
                    case UCODE_S2DEX:
                    {
                        ...
                        switch (curGfx.dma.cmd) // which one to choose here?
                        {
                            ...
                            case G_RDPHALF_0:
                            {
                                DISAS_LOG("RDPHALF_0(0x%02x, 0x%08x, 0x%04x),", curGfx.dma.par, curGfx.dma.addr, curGfx.dma.len);
                                sid = curGfx.dma.par;
                                rdpHalf = curGfx.dma.addr;
                            ...
                            case G_SELECT_DL: // gfx instead of curGfx(?)
                            {
                                ...
                                if (dma.par == 0) {
                                    DISAS_LOG("gsSPSelectDL(0x%08x, %d, 0x%08x, 0x%08x),", dlAddr, sid, rdpHalf, dma.addr);
                                }
                                else {
                                    DISAS_LOG("gsSPSelectBranchDL(0x%08x, %d, 0x%08x, 0x%08x),", dlAddr, sid, rdpHalf, dma.addr);
                                }
