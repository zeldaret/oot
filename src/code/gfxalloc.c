...
/**
 * Creates a new temporary graphics display list pointer, using the memory reserved by gfxBufRef
 *
 * @param gfxBufRef is the display list yielding memory. It cannot be written to until Gfx_Close is called.
 ...
 *
 * @param gfxBufRef is the display list yielding memory.
 * @param gfxChildEnd is the graphics display list pointer that was created with Gfx_Open
 * @return gfxBufRef's new position.
 *
 * @note gfxBufRef must be updated after the call with the return value of this function to complete the operation.
 */
Gfx* Gfx_Close(Gfx* gfxBufRef, Gfx* gfxChildEnd) {
    gSPBranchList(gfxBufRef, gfxChildEnd);
    return gfxChildEnd;
}
