...
void func_800D20CC(MtxF* mf, Vec3s* vec, s32 flag) {
    ...
    temp *= temp;
    temp += SQ(mf->zz);
    vec->x = MathF_Atan2(-mf->zy, sqrtf(temp)) * (32768 / M_PI);

    if ((vec->x == 0x4000) || (vec->x == -0x4000)) {
        vec->z = 0;

        vec->y = MathF_Atan2(-mf->xz, mf->xx) * (32768 / M_PI);
    } else {
        vec->y = MathF_Atan2(mf->zx, mf->zz) * (32768 / M_PI);

        if (!flag) {
            vec->z = MathF_Atan2(mf->xy, mf->yy) * (32768 / M_PI);
        } else {
            temp = mf->xx;
            temp4 = mf->xz;
            temp3 = mf->yz;
            ...
            vec->z = MathF_Atan2(temp, temp2) * (32768 / M_PI);
        }
    }
}
#else
...
void func_800D2264(MtxF* mf, Vec3s* vec, s32 flag) {
    ...
    temp *= temp;
    temp += SQ(mf->xy);
    vec->y = MathF_Atan2(-mf->xz, sqrtf(temp)) * (32768 / M_PI);

    if ((vec->y == 0x4000) || (vec->y == -0x4000)) {
        vec->x = 0;
        vec->z = MathF_Atan2(-mf->yx, mf->yy) * (32768 / M_PI);
        return;
    }

    vec->z = MathF_Atan2(mf->xy, mf->xx) * (32768 / M_PI);

    if (!flag) {
        vec->x = MathF_Atan2(mf->yz, mf->zz) * (32768 / M_PI);
    } else {
        temp = mf->yx;
        temp *= temp;
        temp += SQ(mf->yy);
        ...
        vec->x = MathF_Atan2(temp, temp2) * (32768 / M_PI);
    }
}
#else
