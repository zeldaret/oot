...
u8 Message_ShouldAdvance(GlobalContext* globalCtx) {
    ...
    if (CHECK_BTN_ALL(input->press.button, BTN_A) || CHECK_BTN_ALL(input->press.button, BTN_B) ||
        ...
        Audio_PlaySfxGeneral(NA_SE_SY_MESSAGE_PASS, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
...
void Message_CloseTextbox(GlobalContext* globalCtx) {
    ...
    if (msgCtx->msgLength != 0) {
        ...
        Audio_PlaySfxGeneral(0, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal, &gSfxFreqOrVolDefaultVal,
                             &gSfxReverbAddNone);
    }
}
...
u16 Message_DrawItemIcon(GlobalContext* globalCtx, u16 itemId, Gfx** p, u16 i) {
    ...
    // clang-format off
    if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING) { Audio_PlaySfxGeneral(0, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal, &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone); }
    // clang-format on
        ...
        switch (character) {
            ...
            case MESSAGE_BOX_BREAK:
                if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING) {
                    if (!sTextboxSkipped) {
                        Audio_PlaySfxGeneral(0, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
                                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
            ...
            case MESSAGE_END:
                if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING) {
                    ...
                    if (msgCtx->textboxEndType == TEXTBOX_ENDTYPE_DEFAULT) {
                        Audio_PlaySfxGeneral(NA_SE_SY_MESSAGE_END, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
                                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
            ...
            case MSGMODE_OCARINA_PLAYING:
                ...
                if (msgCtx->ocarinaStaff->state < OCARINA_SONG_MEMORY_GAME) {
                    if (msgCtx->ocarinaStaff->state == OCARINA_SONG_SCARECROW ||
                        ...
                        Audio_OcaSetInstrument(0);
                        Audio_PlaySfxGeneral(NA_SE_SY_OCARINA_ERROR, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
                                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
                        msgCtx->msgMode = MSGMODE_OCARINA_STARTING;
                    }
                ...
                if (msgCtx->stateTimer == 0) {
                    if (msgCtx->ocarinaStaff->state == 0) {
                        if (msgCtx->msgMode == MSGMODE_MEMORY_GAME_LEFT_SKULLKID_PLAYING) {
                            Audio_PlaySfxGeneral(NA_SE_SY_METRONOME, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
                                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
                        } else {
