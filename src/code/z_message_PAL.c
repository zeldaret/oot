...
void Message_Update(PlayState* play) {
    ...
    switch (msgCtx->msgMode) {
        case MSGMODE_TEXT_START:
            ...
            var = false;
            if (R_SCENE_TYPE == SCENE_TYPE_PIVOT) {
                if (D_8014B2F4 >= 4) {
                    var = true;
                }
            } else if (R_SCENE_TYPE != SCENE_TYPE_DEFAULT || play->sceneNum == SCENE_HAIRAL_NIWA) {
                var = true;
            ...
            if (var) {
                ...
                if (!msgCtx->textBoxPos) { // variable position
                    if (R_SCENE_TYPE != SCENE_TYPE_DEFAULT || play->sceneNum == SCENE_HAIRAL_NIWA) {
                        if (averageY < XREG(92)) {
