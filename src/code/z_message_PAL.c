...
void Message_Update(PlayState* play) {
    ...
    switch (msgCtx->msgMode) {
        case MSGMODE_TEXT_START:
            ...
            } else if (R_SCENE_CAM_TYPE != SCENE_CAM_TYPE_DEFAULT || play->sceneId == SCENE_CASTLE_COURTYARD_GUARDS_DAY) {
                var = true;
            ...
            if (var) {
                ...
                if (!msgCtx->textBoxPos) { // variable position
                    if (R_SCENE_CAM_TYPE != SCENE_CAM_TYPE_DEFAULT || play->sceneId == SCENE_CASTLE_COURTYARD_GUARDS_DAY) {
                        if (averageY < XREG(92)) {
