...
void Message_OpenText(PlayState* play, u16 textId) {
    ...
    if (R_SCENE_CAM_TYPE == SCENE_CAM_TYPE_FIXED_SHOP_VIEWPOINT) {
        Interface_ChangeHudVisibilityMode(HUD_SHOW_A_HEARTS_MAGIC_FORCE);
    }
...
void Message_StartOcarina(PlayState* play, u16 ocarinaActionId) {
    ...
    if (noStop == false) {
        ...
        Interface_ChangeHudVisibilityMode(HUD_SHOW_B_ALT);
            ...
            if (play->csCtx.state == 0) {
                ...
                if (msgCtx->textId != 0x2061 && msgCtx->textId != 0x2025 && msgCtx->textId != 0x208C &&
                    ...
                    if (play->activeCamId == CAM_ID_MAIN) {
                        if (gSaveContext.prevHudVisibilityMode == HUD_NO_CHANGE ||
                            gSaveContext.prevHudVisibilityMode == HUD_HIDE_ALL ||
                            gSaveContext.prevHudVisibilityMode == HUD_HIDE_ALL_ALT) {
                            gSaveContext.prevHudVisibilityMode = HUD_SHOW_ALL;
                        }
                        gSaveContext.hudVisibilityMode = HUD_NO_CHANGE;
