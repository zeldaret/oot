...
void Message_OpenText(GlobalContext* globalCtx, u16 textId) {
    ...
    if (textId == 0xC2 || textId == 0xFA) {
        ...
        textId += (gSaveContext.inventory.questItems & 0xF0000000 & 0xF0000000) >> QUEST_HEART_PIECE;
...
void Message_Update(GlobalContext* globalCtx) {
    ...
    switch (msgCtx->msgMode) {
        ...
        case MSGMODE_TEXT_CLOSING:
            ...
            if ((s32)(gSaveContext.inventory.questItems & 0xF0000000) == (1 << QUEST_HEART_CONTAINER)) {
                gSaveContext.inventory.questItems ^= (1 << QUEST_HEART_CONTAINER);
