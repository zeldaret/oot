...
u16 Message_DrawItemIcon(PlayState* play, u16 itemId, Gfx** p, u16 i) {
    ...
    gSPTextureRectangle(gfx++, (msgCtx->textPosX + R_TEXTBOX_ICON_XPOS) << 2, R_TEXTBOX_ICON_YPOS << 2,
                        (msgCtx->textPosX + R_TEXTBOX_ICON_XPOS + R_TEXTBOX_ICON_DIMENSIONS) << 2,
                        (R_TEXTBOX_ICON_YPOS + R_TEXTBOX_ICON_DIMENSIONS) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10,
                        1 << 10);
    gDPPipeSync(gfx++);
...
void Message_LoadItemIcon(PlayState* play, u16 itemId, s16 y) {
    ...
    if (itemId < ITEM_MEDALLION_FOREST) {
        ...
        R_TEXTBOX_ICON_DIMENSIONS = ITEM_ICON_WIDTH; // assumes the image is square
        DmaMgr_SendRequest1((u32)msgCtx->textboxSegment + MESSAGE_STATIC_TEX_SIZE, GET_ITEM_ICON_VROM(itemId),
        ...
        R_TEXTBOX_ICON_DIMENSIONS = QUEST_ICON_WIDTH; // assumes the image is square
        DmaMgr_SendRequest1((u32)msgCtx->textboxSegment + MESSAGE_STATIC_TEX_SIZE, GET_QUEST_ICON_VROM(itemId),
                            QUEST_ICON_SIZE, "../z_message_PAL.c", 1482);
        // "Item 24"
        osSyncPrintf("アイテム24＝%d (%d) {%d}\n", itemId, itemId - ITEM_SPSTONE_KOKIRI_EMERALD, 84);
    }
    msgCtx->msgBufPos++;
    msgCtx->choiceNum = 1;
}
