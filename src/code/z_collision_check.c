...
void CollisionCheck_AC_JntSphVsTris(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* l,
    ...
    for (lItem = left->list; lItem < left->list + left->count; lItem++) {
        ...
        for (rItem = right->list; rItem < right->list + right->count; rItem++) {
            ...
            if (Math3D_TriTouchingSphereIntersect(&lItem->dim.worldSphere, &rItem->dim, &sp6C) == 1) {
        ...
        if (left->count > 0 && left->list != NULL) {
            for (rItem = right->list; rItem < right->list + right->count; rItem++) {
                ...
                for (lItem = left->list; lItem < left->list + left->count; lItem++) {
                    ...
                    if (Math3D_TriTouchingSphereIntersect(&rItem->dim.worldSphere, &lItem->dim, &sp7C) == 1) {
...
void CollisionCheck_AC_JntSphVsQuad(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx, Collider* l,
    ...
    if (left->count > 0 && left->list != NULL) {
        ...
        for (lItem = left->list; lItem < left->list + left->count; lItem++) {
            ...
            if (Math3D_TriTouchingSphereIntersect(&lItem->dim.worldSphere, &D_8015E230, &sp7C) == 1 ||
                Math3D_TriTouchingSphereIntersect(&lItem->dim.worldSphere, &D_8015E268, &sp7C) == 1) {
    ...
    if (right->count > 0 && right->list != NULL) {
        if (func_8005DF2C(&left->body) != 1) {
            ...
            for (rItem = right->list; rItem < right->list + right->count; rItem++) {
                ...
                if (Math3D_TriTouchingSphereIntersect(&rItem->dim.worldSphere, &D_8015E2A0, &sp88) != 1 &&
                    Math3D_TriTouchingSphereIntersect(&rItem->dim.worldSphere, &D_8015E2D8, &sp88) != 1) {
                    continue;
                }
...
s32 CollisionCheck_generalLineOcCheck_JntSph(GlobalContext* globalCtx, CollisionCheckContext* colChkCtx,
    ...
    for (phi_s0 = 0; phi_s0 < jntSph->count; phi_s0++) {
        ...
        D_8015E610.a = *arg3;
        D_8015E610.b = *arg4;
        if (Math3D_LineTouchingSphere(&item->dim.worldSphere, &D_8015E610) == 1) {
            return 1;
        }
    }
    return 0;
