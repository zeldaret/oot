...
void Actor_SetObjectDependency(PlayState* play, Actor* actor) {
    gSegments[6] = VIRTUAL_TO_PHYSICAL(play->objectCtx.entries[actor->objectEntry].segment);
}
...
void Actor_Init(Actor* actor, PlayState* play) {
    ...
    if (Object_IsEntryLoaded(&play->objectCtx, actor->objectEntry)) {
    ...
    for (i = 0; i < ARRAY_COUNT(actorCtx->actorLists); i++, sp80++) {
        ...
        while (actor != NULL) {
            ...
            if (actor->init != NULL) {
                ...
                actor = actor->next;
            } else if (!Object_IsEntryLoaded(&play->objectCtx, actor->objectEntry)) {
                Actor_Kill(actor);
                actor = actor->next;
...
void Actor_Draw(PlayState* play, Actor* actor) {
    ...
    gSPSegment(POLY_OPA_DISP++, 0x06, play->objectCtx.entries[actor->objectEntry].segment);
    gSPSegment(POLY_XLU_DISP++, 0x06, play->objectCtx.entries[actor->objectEntry].segment);
...
Actor* Actor_Spawn(ActorContext* actorCtx, PlayState* play, s16 actorId, f32 posX, f32 posY, f32 posZ, s16 rotX,
    ...
    if (actorInit->id == ACTOR_EN_PART) {
        actor->objectEntry = rotZ;
        rotZ = 0;
    } else {
        actor->objectEntry = objectEntry;
    }
...
void BodyBreak_Alloc(BodyBreak* bodyBreak, s32 count, PlayState* play) {
    ...
    if (bodyBreak->matrices != NULL) {
        ...
        if (bodyBreak->dLists != NULL) {
            ...
            if (bodyBreak->objectEntries != NULL) {
                ...
                Lib_MemSet((u8*)bodyBreak->objectEntries, objectEntriesSize, 0);
                bodyBreak->val = 1;
                return;
            }
        }
    ...
    if (bodyBreak->objectEntries != NULL) {
        ZeldaArena_FreeDebug(bodyBreak->objectEntries, "../z_actor.c", 7564);
    }
}
...
void BodyBreak_SetInfo(BodyBreak* bodyBreak, s32 limbIndex, s32 minLimbIndex, s32 maxLimbIndex, u32 count, Gfx** dList,
    ...
    if ((play->actorCtx.freezeFlashTimer == 0) && (bodyBreak->val > 0)) {
        if ((limbIndex >= minLimbIndex) && (limbIndex <= maxLimbIndex) && (*dList != NULL)) {
            ...
            bodyBreak->objectEntries[bodyBreak->val] = objectEntry;
            bodyBreak->val++;
        }
...
s32 BodyBreak_SpawnParts(Actor* actor, BodyBreak* bodyBreak, PlayState* play, s16 type) {
    ...
    while (bodyBreak->count > 0) {
        ...
        if (1) {
            if (bodyBreak->objectEntries[bodyBreak->count] > BODYBREAK_OBJECT_ENTRY_DEFAULT) {
                objectEntry = bodyBreak->objectEntries[bodyBreak->count];
            } else {
                objectEntry = actor->objectEntry;
            }
        }
    ...
    ZeldaArena_FreeDebug(bodyBreak->objectEntries, "../z_actor.c", 7680);
