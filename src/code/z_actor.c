...
void func_8002C124(TargetContext* targetCtx, PlayState* play) {
    ...
    if (targetCtx->unk_48 != 0) {
        ...
        if (!(player->stateFlags1 & PLAYER_STATE1_DIALOGUE_MESSAGE) || (actor != player->unk_664)) {
...
void func_8002C7BC(TargetContext* targetCtx, Player* player, Actor* actorArg, PlayState* play) {
    ...
    if (actorArg != NULL) {
        if (actorArg != targetCtx->targetedActor) {
            ...
            Audio_PlaySfxGeneralWithDefaultSettings1(lockOnSfxId);
        }
...
f32 func_8002DCE4(Player* player) {
    if (player->stateFlags1 & PLAYER_STATE1_ON_HORSE) {
        return 8.0f;
    } else if (player->stateFlags1 & PLAYER_STATE1_BEING_IN_WATER) {
...
s32 func_8002DD6C(Player* player) {
    return player->stateFlags1 & PLAYER_STATE1_FAR_RANGE_WEAPON_EQUIPPED;
}
...
s32 func_8002DDA8(PlayState* play) {
    ...
    return (player->stateFlags1 & PLAYER_STATE1_CARRYING_OBJECT) || func_8002DD78(player);
}

s32 func_8002DDE4(PlayState* play) {
    Player* player = GET_PLAYER(play);

    return player->stateFlags2 & PLAYER_STATE2_TOUCHING_GROUND;
}

s32 func_8002DDF4(PlayState* play) {
    Player* player = GET_PLAYER(play);

    return player->stateFlags2 & PLAYER_STATE2_CLIMBING_IDLE;
}
...
s32 func_8002DEEC(Player* player) {
    return (player->stateFlags1 & (PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_EXCLUSIVE_CUTSCENE)) || (player->csMode != PLAYER_CSMODE_NONE);
}
...
s32 Actor_OfferGetItem(Actor* actor, PlayState* play, s32 getItemId, f32 xzRange, f32 yRange) {
    ...
    if (!(player->stateFlags1 & (PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_SPIN_ATTACK_POSITIONING | PLAYER_STATE1_HANGING_AT_LEDGE | PLAYER_STATE1_CLIMBING_LEDGE |
                                 PLAYER_STATE1_MID_AIR_JUMP | PLAYER_STATE1_FREE_FALLING | PLAYER_STATE1_1ST_PERSON_VIEW | PLAYER_STATE1_CLIMBING_MODE)) &&
        Player_GetExplosiveHeld(player) < 0) {
        if ((((player->heldActor != NULL) || (actor == player->targetActor)) && (getItemId > GI_NONE) &&
             (getItemId < GI_MAX)) ||
            (!(player->stateFlags1 & (PLAYER_STATE1_CARRYING_OBJECT | PLAYER_STATE1_EXCLUSIVE_CUTSCENE)))) {
...
u32 Actor_SetRideActor(PlayState* play, Actor* horse, s32 mountSide) {
    ...
    if (!(player->stateFlags1 &
          (PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_CARRYING_OBJECT | PLAYER_STATE1_SPIN_ATTACK_POSITIONING | PLAYER_STATE1_HANGING_AT_LEDGE | PLAYER_STATE1_CLIMBING_LEDGE |
           PLAYER_STATE1_MID_AIR_JUMP | PLAYER_STATE1_FREE_FALLING | PLAYER_STATE1_1ST_PERSON_VIEW | PLAYER_STATE1_CLIMBING_MODE))) {
        player->rideActor = horse;
...
void Actor_PlaySfx(Actor* actor, u16 sfxId) {
    Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&actor->projectedPos, sfxId);
}
...
void func_8002F850(PlayState* play, Actor* actor) {
    ...
    Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&actor->projectedPos, NA_SE_EV_BOMB_BOUND);
    Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&actor->projectedPos, NA_SE_PL_WALK_GROUND + surfaceSfxOffset);
}
...
u32 sCategoryFreezeMasks[ACTORCAT_MAX] = {
    // ACTORCAT_SWITCH
    PLAYER_STATE1_DIALOGUE_MESSAGE | PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_EXCLUSIVE_ITEM_USAGE,
    // ACTORCAT_BG
    PLAYER_STATE1_DIALOGUE_MESSAGE | PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_EXCLUSIVE_ITEM_USAGE,
    // ACTORCAT_PLAYER
    0,
    // ACTORCAT_EXPLOSIVE
    PLAYER_STATE1_DIALOGUE_MESSAGE | PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_ACQUIRING_NEW_ITEM | PLAYER_STATE1_EXCLUSIVE_ITEM_USAGE,
    // ACTORCAT_NPC
    PLAYER_STATE1_GAME_OVER,
    // ACTORCAT_ENEMY
    PLAYER_STATE1_DIALOGUE_MESSAGE | PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_EXCLUSIVE_ITEM_USAGE | PLAYER_STATE1_EXCLUSIVE_CUTSCENE,
    // ACTORCAT_PROP
    PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_EXCLUSIVE_ITEM_USAGE,
    // ACTORCAT_ITEMACTION
    0,
    // ACTORCAT_MISC
    PLAYER_STATE1_DIALOGUE_MESSAGE | PLAYER_STATE1_GAME_OVER | PLAYER_STATE1_EXCLUSIVE_ITEM_USAGE | PLAYER_STATE1_EXCLUSIVE_CUTSCENE,
...
void Actor_UpdateAll(PlayState* play, ActorContext* actorCtx) {
    ...
    if ((player->stateFlags1 & PLAYER_STATE1_DIALOGUE_MESSAGE) && ((player->actor.textId & 0xFF00) != 0x600)) {
    ...
    if ((actor == NULL) || (player->unk_66C < 5)) {
        ...
        if (actorCtx->targetCtx.unk_4B != 0) {
            ...
            Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_SY_LOCK_OFF);
        }
    }
...
void func_80030ED8(Actor* actor) {
    if (actor->flags & ACTOR_FLAG_19) {
        ...
        Audio_PlaySfxGeneralWithDefaultSettings1(actor->sfx);
    } else if (actor->flags & ACTOR_FLAG_21) {
        Audio_PlaySfxGeneralWithDefaultSettings2(actor->sfx);
    } else if (actor->flags & ACTOR_FLAG_28) {
        func_800F4C58(&gSfxDefaultPos, NA_SE_SY_TIMER - SFX_FLAG, (s8)(actor->sfx - 1));
    } else {
        Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&actor->projectedPos, actor->sfx);
    }
}
...
s32 Actor_IsTargeted(PlayState* play, Actor* actor) {
    ...
    if ((player->stateFlags1 & PLAYER_STATE1_AIMING) && actor->isTargeted) {
        return true;
    } else {
        return false;
    }
