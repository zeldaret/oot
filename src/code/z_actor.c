...
void ActorShadow_DrawFoot(PlayState* play, Light* light, MtxF* arg2, s32 arg3, f32 arg4, f32 arg5, f32 arg6) {
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_actor.c", 1687),
              G_MTX_MODELVIEW | G_MTX_LOAD);
...
void Actor_SetObjectDependency(PlayState* play, Actor* actor) {
    gSegments[6] = VIRTUAL_TO_PHYSICAL(play->objectCtx.loadEntries[actor->objectLoadEntryIndex].segment);
}
    ...
    for (i = 0; i < ARRAY_COUNT(actorCtx->actorLists); i++, sp80++) {
        ...
        while (actor != NULL) {
            ...
            if (actor->init != NULL) {
                ...
                actor = actor->next;
            } else if (!Object_IsLoadEntryLoaded(&play->objectCtx, actor->objectLoadEntryIndex)) {
                Actor_Kill(actor);
                actor = actor->next;
...
void Actor_Draw(PlayState* play, Actor* actor) {
    ...
    gSPSegment(POLY_OPA_DISP++, 0x06, play->objectCtx.loadEntries[actor->objectLoadEntryIndex].segment);
    gSPSegment(POLY_XLU_DISP++, 0x06, play->objectCtx.loadEntries[actor->objectLoadEntryIndex].segment);
...
Actor* Actor_Spawn(ActorContext* actorCtx, PlayState* play, s16 actorId, f32 posX, f32 posY, f32 posZ,
    ...
    objectLoadEntryIndex = Object_GetLoadEntryIndex(&play->objectCtx, actorInit->objectId);
    ...
    if ((objectLoadEntryIndex < 0) ||
        ...
        osSyncPrintf(VT_COL(RED, WHITE) "データバンク無し！！<データバンク＝%d>(profilep->bank=%d)\n" VT_RST,
                     objectLoadEntryIndex, actorInit->objectId);
    ...
    if (actorInit->id == ACTOR_EN_PART) {
        actor->objectLoadEntryIndex = rotZ;
        rotZ = 0;
    } else {
        actor->objectLoadEntryIndex = objectLoadEntryIndex;
    }
...
void BodyBreak_Alloc(BodyBreak* bodyBreak, s32 count, PlayState* play) {
    u32 matricesSize;
    u32 dListsSize;
    u32 objectLoadEntryIndicesSize;
    ...
    if (bodyBreak->matrices != NULL) {
        ...
        if (bodyBreak->dLists != NULL) {
            objectLoadEntryIndicesSize = (count + 1) * sizeof(*bodyBreak->objectLoadEntryIndices);
            bodyBreak->objectLoadEntryIndices =
                ZeldaArena_MallocDebug(objectLoadEntryIndicesSize, "../z_actor.c", 7546);

            if (bodyBreak->objectLoadEntryIndices != NULL) {
                Lib_MemSet((u8*)bodyBreak->matrices, matricesSize, 0);
                Lib_MemSet((u8*)bodyBreak->dLists, dListsSize, 0);
                Lib_MemSet((u8*)bodyBreak->objectLoadEntryIndices, objectLoadEntryIndicesSize, 0);
                bodyBreak->val = 1;
                return;
            }
        }
    ...
    if (bodyBreak->objectLoadEntryIndices != NULL) {
        ZeldaArena_FreeDebug(bodyBreak->objectLoadEntryIndices, "../z_actor.c", 7564);
    }
}
...
void BodyBreak_SetInfo(BodyBreak* bodyBreak, s32 limbIndex, s32 minLimbIndex, s32 maxLimbIndex, u32 count, Gfx** dList,
    ...
    if ((play->actorCtx.freezeFlashTimer == 0) && (bodyBreak->val > 0)) {
        if ((limbIndex >= minLimbIndex) && (limbIndex <= maxLimbIndex) && (*dList != NULL)) {
            ...
            bodyBreak->objectLoadEntryIndices[bodyBreak->val] = objectLoadEntryIndex;
            bodyBreak->val++;
        }
...
s32 BodyBreak_SpawnParts(Actor* actor, BodyBreak* bodyBreak, PlayState* play, s16 type) {
    ...
    while (bodyBreak->count > 0) {
        ...
        if (1) {
            if (bodyBreak->objectLoadEntryIndices[bodyBreak->count] > BODYBREAK_OBJECT_LOAD_ENTRY_DEFAULT) {
                objectLoadEntryIndex = bodyBreak->objectLoadEntryIndices[bodyBreak->count];
            } else {
                objectLoadEntryIndex = actor->objectLoadEntryIndex;
            }
        }
        ...
        spawnedEnPart = (EnPart*)Actor_SpawnAsChild(&play->actorCtx, actor, play, ACTOR_EN_PART, mtx->xw,
                                                    mtx->yw, mtx->zw, 0, 0, objectLoadEntryIndex, type);
    ...
    ZeldaArena_FreeDebug(bodyBreak->objectLoadEntryIndices, "../z_actor.c", 7680);
...
void func_80033C30(Vec3f* arg0, Vec3f* arg1, u8 alpha, PlayState* play) {
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_actor.c", 8149),
              G_MTX_MODELVIEW | G_MTX_LOAD);
...
void Actor_DrawDoorLock(PlayState* play, s32 frame, s32 type) {
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_actor.c", 8314),
              G_MTX_MODELVIEW | G_MTX_LOAD);
...
Actor* func_800358DC(Actor* actor, Vec3f* spawnPos, Vec3s* spawnRot, f32* arg3, s32 timer, s16* unused,
    ...
    spawnedEnPart =
        (EnPart*)Actor_SpawnAsChild(&play->actorCtx, actor, play, ACTOR_EN_PART, spawnPos->x, spawnPos->y,
                                    spawnPos->z, spawnRot->x, spawnRot->y, actor->objectLoadEntryIndex, params);
    if (spawnedEnPart != NULL) {
        spawnedEnPart->actor.scale = actor->scale;
