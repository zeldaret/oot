...
void func_800A663C(GlobalContext *globalCtx, PSkinAwb *arg1, SkeletonHeader *skeletonHeaderSeg, AnimationHeader *arg3) {
    u8 nEntries;
    AvbEntry *thisEntry;
    SkeletonHeader *skeletonHeader;
    Skeleton *skeleton;
    SkeletonHeader *skeletonHeader2;
    SkeletonHeader *skeletonHeader3;
    u16 *nInBuf;
    s32 i;

    //skelAnime->skeleton
    skeletonHeader = SEGMENTED_TO_VIRTUAL(skeletonHeaderSeg); // TODO: somePtr is a pointer to a skeleton??
    arg1->somePtr = skeletonHeader;
    arg1->tblSize = arg1->somePtr->limbCount;
    skeletonHeader2 = SEGMENTED_TO_VIRTUAL(skeletonHeader->skeletonSeg);
    arg1->avbTbl = ZeldaArena_MallocDebug(arg1->tblSize * 0xC, "../z_skin_awb.c", 0xD4);
    if (arg1->avbTbl == NULL) {
        __assert((const char *) "pskin_awb->avb_tbl != NULL", "../z_skin_awb.c", 0xD6);
    }
    for (i = 0; i < arg1->tblSize; i++){
        thisEntry = &arg1->avbTbl[i];
        skeletonHeader3 = SEGMENTED_TO_VIRTUAL(skeletonHeader2->skeletonSeg);
        if ((skeletonHeader3->dListCount != 4) || (skeletonHeader3->nInBufSeg == 0)) {
            thisEntry->someFlag = 0;
            if (skeletonHeader2){}
            thisEntry->buf[0] = NULL;
            thisEntry->buf[1] = NULL;
        } else {
            nInBuf = ((u16*)SEGMENTED_TO_VIRTUAL(skeletonHeader3->nInBufSeg));
            thisEntry->someFlag = 0;
            thisEntry->buf[0] = ZeldaArena_MallocDebug((u32) (*nInBuf * 0x10), "../z_skin_awb.c", 0xEB);
            if (thisEntry->buf[0] == NULL) {
                __assert((const char *) "psavb->buf[0] != NULL", "../z_skin_awb.c", 0xED);
            }

            thisEntry->buf[1] = ZeldaArena_MallocDebug((u32) (*nInBuf * 0x10), "../z_skin_awb.c", 0xF0);
            if (thisEntry->buf[1] == NULL) {
                __assert((const char *) "psavb->buf[1] != NULL", "../z_skin_awb.c", 0xF2);
            }
            func_800A6460(globalCtx, arg1, i);
        }
    }
    SkelAnime_InitSkin(globalCtx, &arg1->skelAnime, skeletonHeaderSeg, arg3);
}
