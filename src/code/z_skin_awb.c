...
s32 func_800A6AC4(PSkinAwb *skin, MtxF *arg1, SkinActor *actor, s32 arg3) {
    ...
    SkinMatrix_SetScaleRotateYRPTranslate(&skin->mtx, actor->actor.scale.x, actor->actor.scale.y, actor->actor.scale.z, actor->actor.shape.rot.x, actor->actor.shape.rot.y, actor->actor.shape.rot.z, actor->actor.posRot.pos.x, actor->actor.posRot.pos.y + (actor->actor.shape.unk_08 * actor->actor.scale.y), actor->actor.posRot.pos.z);
