        ...
        if (this->actor.scale.y >= 0.0f) {
            if (!Player_HoldsHookshot(this) && ((hookedActor = this->heldActor) != NULL)) {
                if (this->stateFlags1 & 0x200) {
                    ...
                    Matrix_Get(&sp14C);
                    Matrix_MtxFToRotS(&sp14C, &hookedActor->world.rot, 0);
                    hookedActor->shape.rot = hookedActor->world.rot;
                } else if (this->stateFlags1 & 0x800) {
                    Vec3s spB8;

                    Matrix_Get(&sp14C);
                    Matrix_MtxFToRotS(&sp14C, &spB8, 0);
                ...
                Matrix_MtxFToRotS(&this->mf_9E0, &this->unk_3BC, 0);
            }
        }
    } else if (limbIndex == PLAYER_LIMB_R_HAND) {
        ...
        if (this->actor.scale.y >= 0.0f) {
            if ((this->heldItemActionParam == PLAYER_AP_HOOKSHOT) ||
                ...
                if (heldActor != NULL) {
                    ...
                    Matrix_Get(&sp44);
                    Matrix_MtxFToRotS(&sp44, &heldActor->world.rot, 0);
