...
s32 Player_OverrideLimbDrawGameplayCommon(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (limbIndex == PLAYER_LIMB_ROOT) {
        ...
        if (!LINK_IS_ADULT) {
            if (!(this->skelAnime.moveFlags & ANIM_FLAG_2) || (this->skelAnime.moveFlags & ANIM_FLAG_0)) {
                pos->x *= 0.64f;
                pos->z *= 0.64f;
            }

            if (!(this->skelAnime.moveFlags & ANIM_FLAG_2) || (this->skelAnime.moveFlags & ANIM_FLAG_UPDATE_Y)) {
                pos->y *= 0.64f;
            }
        }
