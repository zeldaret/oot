...
void Player_SetBootData(GlobalContext* globalCtx, Player* this) {
    ...
    if (currentBoots == PLAYER_BOOTS_NORMAL) {
        ...
        if (this->stateFlags1 & PLAYER_STATE_FLAG_1_27) {
...
s32 func_8008E9C4(Player* this) {
    return (this->stateFlags1 & PLAYER_STATE_FLAG_1_4);
}
...
void Player_SetModelsForHoldingShield(Player* this) {
    if ((this->stateFlags1 & PLAYER_STATE_FLAG_1_22) &&
...
void func_8008EDF0(Player* this) {
    this->unk_664 = NULL;
    this->stateFlags2 &= ~PLAYER_STATE_FLAG_2_13;
}
...
void func_8008EEAC(GlobalContext* globalCtx, Actor* actor) {
    ...
    this->unk_664 = actor;
    this->unk_684 = actor;
    this->stateFlags1 |= PLAYER_STATE_FLAG_1_16;
...
s32 func_8008EF30(GlobalContext* globalCtx) {
    ...
    return (this->stateFlags1 & PLAYER_STATE_FLAG_1_23);
}
...
s32 func_8008F2F8(GlobalContext* globalCtx) {
    ...
    } else if (this->stateFlags1 & PLAYER_STATE_FLAG_1_27) { // Swimming
        var = 2;
    } else {
        return 0;
    }
...
void func_8008F87C(GlobalContext* globalCtx, Player* this, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot,
    ...
    if ((this->actor.scale.y >= 0.0f) && !(this->stateFlags1 & PLAYER_STATE_FLAG_1_7) &&
...
void func_800906D4(GlobalContext* globalCtx, Player* this, Vec3f* newTipPos) {
    ...
    if (func_80090480(globalCtx, NULL, &this->swordInfo[0], &newTipPos[0], &newBasePos[0]) &&
        !(this->stateFlags1 & PLAYER_STATE_FLAG_1_22)) {
        EffectBlure_AddVertex(Effect_GetByIndex(this->swordEffectIndex), &this->swordInfo[0].tip,
                              &this->swordInfo[0].base);
    }

    if ((this->swordState > 0) && ((this->swordAnimation < 0x18) || (this->stateFlags2 & PLAYER_STATE_FLAG_2_17))) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (this->actor.scale.y >= 0.0f) {
            if (!Player_HoldsHookshot(this) && ((hookedActor = this->heldActor) != NULL)) {
                if (this->stateFlags1 & PLAYER_STATE_FLAG_1_9) {
                    Matrix_MultVec3f(&D_80126128, &hookedActor->world.pos);
                    Matrix_RotateZYX(0x69E8, -0x5708, 0x458E, MTXMODE_APPLY);
                    Matrix_Get(&sp14C);
                    Matrix_MtxFToYXZRotS(&sp14C, &hookedActor->world.rot, 0);
                    hookedActor->shape.rot = hookedActor->world.rot;
                } else if (this->stateFlags1 & PLAYER_STATE_FLAG_1_11) {
                    Vec3s spB8;
        ...
        if (this->rightHandType == 0xFF) {
            ...
            if ((this->stateFlags1 & PLAYER_STATE_FLAG_1_9) && (this->unk_860 >= 0) && (this->unk_834 <= 10)) {
                Vec3f sp90;
                f32 distXYZ;
