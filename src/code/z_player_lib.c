...
void Player_SetModelsForHoldingShield(Player* this) {
    if ((this->stateFlags1 & 0x400000) &&
        ...
        if (!Player_HoldsTwoHandedWeapon(this) && !Player_IsChildWithHylianShield(this)) {
            ...
            this->rightHandDLists = &sPlayerDListGroups[10][(void)0, gSaveContext.save.linkAge];
...
void Player_SetModels(Player* this, s32 modelGroup) {
    ...
    this->leftHandDLists = &sPlayerDListGroups[gPlayerModelTypes[modelGroup][1]][(void)0, gSaveContext.save.linkAge];
    this->rightHandDLists = &sPlayerDListGroups[gPlayerModelTypes[modelGroup][2]][(void)0, gSaveContext.save.linkAge];
    this->sheathDLists = &sPlayerDListGroups[gPlayerModelTypes[modelGroup][3]][(void)0, gSaveContext.save.linkAge];
    this->waistDLists = &sPlayerDListGroups[gPlayerModelTypes[modelGroup][4]][(void)0, gSaveContext.save.linkAge];
...
void func_8008F87C(GlobalContext* globalCtx, Player* this, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot,
    ...
    if ((this->actor.scale.y >= 0.0f) && !(this->stateFlags1 & 0x80) &&
        ...
        Matrix_JointPosition(&D_80126038[(void)0, gSaveContext.save.linkAge], &skelAnime->limbDrawTbl[shinLimbIndex]);
    ...
    if (func_8008FCC8(globalCtx, limbIndex, dList, pos, rot, actor) == 0) {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            if ((D_80160014 == 4) && (gSaveContext.save.info.playerData.swordHealth <= 0.0f)) {
                ...
                dLists = &D_80125E18[gSaveContext.save.linkAge];
                D_80160014 = 1;
            }
            ...
            *dList = dLists[sDListsLodOffset];
            ...
            if (D_80160018 == 10) {
                ...
                dLists = &D_80125E58[gSaveContext.save.linkAge];
                D_80160018 = 9;
            }
...
s32 func_800902F0(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* actor) {
    ...
    if (func_8008FCC8(globalCtx, limbIndex, dList, pos, rot, actor) == 0) {
        if (this->unk_6AD != 2) {
            ...
            *dList = D_80125F18[(void)0, gSaveContext.save.linkAge];
        } else if (limbIndex == PLAYER_LIMB_L_HAND) {
            *dList = D_80125F20[(void)0, gSaveContext.save.linkAge];
        } else if (limbIndex == PLAYER_LIMB_R_SHOULDER) {
            *dList = D_80125F28[(void)0, gSaveContext.save.linkAge];
        } else if (limbIndex == PLAYER_LIMB_R_FOREARM) {
            *dList = D_80125F30[(void)0, gSaveContext.save.linkAge];
        } else if (limbIndex == PLAYER_LIMB_R_HAND) {
            *dList = Player_HoldsHookshot(this) ? D_0602A738 : D_80125F38[(void)0, gSaveContext.save.linkAge];
        } else {
            *dList = NULL;
        }
    }
...
void func_80090D20(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* actor) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (this->itemActionParam == PLAYER_AP_STICK) {
            ...
            gSPDisplayList(oGfxCtx->polyXlu.p++, sBottleDLists[gSaveContext.save.linkAge]);
        ...
        if (this->rightHandType == 0xFF) {
            ...
            BowStringData* stringData = &sBowStringData[gSaveContext.save.linkAge]; // sp+0xB0
        ...
        if (limbIndex == PLAYER_LIMB_SHEATH) {
            ...
            Vec3f* vec = &D_801261E0[gSaveContext.save.linkAge];
...
u32 func_80091738(GlobalContext* globalCtx, u8* segment, SkelAnime* skelAnime) {
    ...
    SkelAnime_InitLinkAnimetion(globalCtx, skelAnime, gPlayerSkelHeaders[(void)0, gSaveContext.save.linkAge],
