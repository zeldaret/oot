        ...
        switch (play->sceneNum) {
            ...
            case SCENE_ICE_DOUKUTO:
                ...
                if (CHECK_BTN_ALL(play->state.input[0].press.button, BTN_L) && !Play_InCsMode(play)) {
                    ...
                    // clang-format off
                    if (!R_MINIMAP_DISABLED) { Audio_PlaySoundGeneral(NA_SE_SY_CAMERA_ZOOM_UP_2, &gSfxDefaultPos, 4,
                                                                      &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                                                                      &gSfxDefaultReverb);
                    } else {
                        Audio_PlaySoundGeneral(NA_SE_SY_CAMERA_ZOOM_DOWN_2, &gSfxDefaultPos, 4,
                                               &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                                               &gSfxDefaultReverb);
                    }
                    // clang-format on
