...
s16 Camera_XZAngle(Vec3f* to, Vec3f* from) {
    return DEG_TO_BINANG(RAD_TO_DEG(Math_FAtan2F(from->x - to->x, from->z - to->z)));
}
...
s16 func_80044ADC(Camera* camera, s16 yaw, s16 arg2) {
    ...
    temp_s0 = DEG_TO_BINANG(RAD_TO_DEG(Math_FAtan2F(phi_f16, sp30)));
    temp_s1 = DEG_TO_BINANG(RAD_TO_DEG(Math_FAtan2F(phi_f18, sp2C)));
...
s32 Camera_Normal1(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        norm1->pitchTarget = DEG_TO_BINANG(NEXTSETTING);
...
s32 Camera_Normal2(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        norm2->unk_1C = DEG_TO_BINANG(NEXTSETTING);
...
s32 Camera_Normal3(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        norm3->pitchTarget = DEG_TO_BINANG(NEXTSETTING);
...
s32 Camera_Parallel1(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        para1->pitchTarget = DEG_TO_BINANG(NEXTSETTING);
        para1->yawTarget = DEG_TO_BINANG(NEXTSETTING);
...
s32 Camera_Jump3(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14 || modeSwitch ||
        ...
        jump3->pitchTarget = DEG_TO_BINANG(NEXTSETTING);
...
s32 Camera_Battle1(Camera* camera) {
    ...
    if (anim->animTimer != 0) {
        ...
        anim->animTimer--;
    } else if (ABS(tmpAng1) > DEG_TO_BINANG(swingAngle)) {
        spFC = BINANG_TO_DEG(tmpAng1);
        ...
        tmpAng2 = tmpAng1 >= 0 ? DEG_TO_BINANG(temp_f14) : (-DEG_TO_BINANG(temp_f14));
        spB4.yaw = BINANG_ROT180((s16)(BINANG_ROT180(atToEyeNextDir.yaw) + tmpAng2));
    } else {
        spFC = 0.05f;
        spFC = (1 - camera->speedRatio) * spFC;
        tmpAng2 = tmpAng1 >= 0 ? DEG_TO_BINANG(swingAngle) : -DEG_TO_BINANG(swingAngle);
    ...
    if (!skipEyeAtCalc) {
        ...
        tmpAng1 = DEG_TO_BINANG(var2) - (s16)(playerToTargetDir.pitch * (0.5f + distRatio * (1.0f - 0.5f)));
        tmpAng1 += (s16)(var3);
    ...
    camera->roll = DEG_TO_BINANG(anim->roll);
...
s32 Camera_Battle4(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        batt4->pitchTarget = DEG_TO_BINANG(NEXTSETTING);
...
s32 Camera_KeepOn1(Camera* camera) {
    ...
    if (anim->unk_16 != 0) {
        ...
        anim->unk_16--;
    } else if (ABS(spE2) > DEG_TO_BINANG(spEC)) {
        spF4 = BINANG_TO_DEG(spE2);
        ...
        temp_f14 = SQ(spF4) / t1;
        spE0 = spE2 >= 0 ? (DEG_TO_BINANG(temp_f14)) : (-DEG_TO_BINANG(temp_f14));
        spD8.yaw = BINANG_ROT180((s16)(BINANG_ROT180(spB8.yaw) + spE0));
    } else {
        spF4 = 0.02f;
        spF4 = (1.0f - camera->speedRatio) * spF4;
        spE0 = spE2 >= 0 ? DEG_TO_BINANG(spEC) : -DEG_TO_BINANG(spEC);
        spD8.yaw = spB8.yaw - (s16)((spE0 - spE2) * spF4);
    }

    if (sp88 == 0) {
        spE2 = DEG_TO_BINANG((f32)(keep1->unk_14 + ((keep1->unk_18 - keep1->unk_14) * sp84)));
...
s32 Camera_KeepOn3(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        ...
        atToEyeAdj.pitch = DEG_TO_BINANG(swingAngle) + ((s16)(-(targetToPlayerDir.pitch * keep3->swingPitchAdj)));
        ...
        if (keep3->flags & 0x10) {
            if (BINANG_SUB(targetToPlayerDir.yaw, atToEyeNextDir.yaw) < 0) {
                atToEyeAdj.yaw = targetToPlayerDir.yaw + DEG_TO_BINANG(swingAngle);
            } else {
                atToEyeAdj.yaw = targetToPlayerDir.yaw - DEG_TO_BINANG(swingAngle);
            }
        } else if (keep3->flags & 0x20) {
            if (BINANG_SUB(targetToPlayerDir.yaw, atToEyeNextDir.yaw) < 0) {
                atToEyeAdj.yaw = BINANG_ROT180(targetToPlayerDir.yaw) - DEG_TO_BINANG(swingAngle);
            } else {
                atToEyeAdj.yaw = BINANG_ROT180(targetToPlayerDir.yaw) + DEG_TO_BINANG(swingAngle);
            }
    ...
    switch (camera->animState) {
        ...
        case 0x14:
            ...
            if (keep4->unk_1C & 2) {
                ...
                spA0 = (BINANG_SUB(sp9E, spA8.yaw) > 0) ? sp9E + DEG_TO_BINANG(keep4->unk_0C)
                                                        : sp9E - DEG_TO_BINANG(keep4->unk_0C);
                spCC[1] = camera->target;
                sp9C++;
...
s32 Camera_Data4(Camera* camera) {
    ...
    atOffset.yaw =
        (initParams->jfifId & 1) ? (DEG_TO_BINANG(camera->data2) + initParams->eyePosRot.rot.y) : eyeAtOffset.yaw;
    atOffset.pitch =
        (initParams->jfifId & 2) ? (DEG_TO_BINANG(camera->data3) + initParams->eyePosRot.rot.x) : eyeAtOffset.pitch;
...
s32 Camera_Unique1(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        uniq1->pitchTarget = DEG_TO_BINANG(NEXTSETTING);
...
s32 Camera_Unique9(Camera* camera) {
    ...
    if (atInitFlags == 1) {
        ...
        if (camera->target != NULL && camera->target->update != NULL) {
            ...
            if (atInitFlags & (s16)0x8080) {
                scratchSph.pitch = DEG_TO_BINANG(anim->curKeyFrame->atTargetInit.x);
                scratchSph.yaw = DEG_TO_BINANG(anim->curKeyFrame->atTargetInit.y);
    ...
    if (eyeInitFlags == 0x100) {
        ...
        if (camera->target != NULL && camera->target->update != NULL) {
            ...
            if (eyeInitFlags & (s16)0x8080) {
                scratchSph.pitch = DEG_TO_BINANG(anim->curKeyFrame->eyeTargetInit.x);
                scratchSph.yaw = DEG_TO_BINANG(anim->curKeyFrame->eyeTargetInit.y);
    ...
    if (anim->curKeyFrame->initFlags == 2) {
        ...
        anim->rollTarget = DEG_TO_BINANG(anim->curKeyFrame->rollTargetInit);
    }
...
s32 Camera_Special5(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        spec5->pitch = DEG_TO_BINANG(NEXTSETTING);
...
void Camera_UpdateDistortion(Camera* camera) {
    ...
    if (camera->distortionFlags != 0) {
        ...
        depthPhase += DEG_TO_BINANG(depthPhaseStep);
        screenPlanePhase += DEG_TO_BINANG(screenPlanePhaseStep);
...
Vec3s Camera_Update(Camera* camera) {
    ...
    if ((bgId = Quake_Calc(camera, &quake), bgId != 0) && (camera->setting != CAM_SET_TURN_AROUND)) {
        ...
        Camera_CalcUpFromPitchYawRoll(&viewUp, eyeAtAngle.pitch + quake.rotZ, eyeAtAngle.yaw + quake.unk_1A,
                                      camera->roll);
        viewFov = camera->fov + BINANG_TO_DEG(quake.zoom);
    } else {
        viewAt = camera->at;
        viewEye = camera->eye;
        OLib_Vec3fDiffToVecSphGeo(&eyeAtAngle, &viewEye, &viewAt);
    ...
    if (PREG(81)) {
        osSyncPrintf("dir  (%d) %d(%f) %d(%f) 0(0) \n", sUpdateCameraDirection, camera->inputDir.x,
                     BINANG_TO_DEG(camera->inputDir.x), camera->inputDir.y, BINANG_TO_DEG(camera->inputDir.y));
        osSyncPrintf("real (%d) %d(%f) %d(%f) 0(0) \n", sUpdateCameraDirection, camera->camDir.x,
                     BINANG_TO_DEG(camera->camDir.x), camera->camDir.y, BINANG_TO_DEG(camera->camDir.y));
    }
    ...
    if (value != NULL) {
        switch (param) {
            ...
            case 4:
                ...
                break;
            case 0x40:
                camera->roll = DEG_TO_BINANG(*(f32*)value);
                break;
            case 0x20:
