...
s32 Camera_Battle1(Camera* camera) {
    ...
    if (camera->target == NULL || camera->target->update == NULL) {
        if (camera->target == NULL) {
            osSyncPrintf(
                TE_COL(YELLOW, BLACK) "camera: warning: battle: target is not valid, change parallel\n" TE_RST);
        }
        camera->target = NULL;
...
s32 Camera_KeepOn1(Camera* camera) {
    ...
    if ((camera->target == NULL) || (camera->target->update == NULL)) {
        if (camera->target == NULL) {
            osSyncPrintf(
                TE_COL(YELLOW, BLACK) "camera: warning: keepon: target is not valid, change parallel\n" TE_RST);
        }
        camera->target = NULL;
        Camera_ChangeMode(camera, CAM_MODE_TARGET);
        return 1;
