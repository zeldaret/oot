...
s32 Camera_GetWaterBoxDataIdx(Camera* camera, f32* waterY) {
    ...
    *waterY = playerPosShape.pos.y;
    ...
    if (!(camera->player->stateFlags1 & PLAYER_STATE_FLAG_1_27)) {
...
s32 Camera_CalcAtForParallel(Camera* camera, VecSph* arg1, f32 arg2, f32* arg3, s16 arg4) {
    ...
    if (camera->playerGroundY == camera->playerPosRot.pos.y || camera->player->actor.gravity > -0.1f ||
        camera->player->stateFlags1 & PLAYER_STATE_FLAG_1_21) {
...
s32 Camera_Battle1(Camera* camera) {
    ...
    if (camera->player->stateFlags1 & PLAYER_STATE_FLAG_1_12) {
        // charging sword.
    ...
    if (RELOAD_PARAMS) {
        ...
        switch (*temp_s0) {
            ...
            case 12:
                ...
                keep4->unk_08 = -2.0f;
                keep4->unk_0C = 120.0f;
                keep4->unk_10 = player->stateFlags1 & PLAYER_STATE_FLAG_1_27 ? 0.0f : 20.0f;
                keep4->unk_1C = 0x3212;
                keep4->unk_1E = 0x1E;
                keep4->unk_18 = 50.0f;
                break;
...
s32 Camera_Unique0(Camera* camera) {
    ...
    if (player->stateFlags1 & PLAYER_STATE_FLAG_1_29) {
        anim->initalPos = playerPosRot->pos;
    }
    ...
    if (params->interfaceFlags & 1) {
        if (anim->animTimer > 0) {
            anim->animTimer--;
            anim->initalPos = playerPosRot->pos;
        } else if ((!(player->stateFlags1 & PLAYER_STATE_FLAG_1_29)) &&
...
s32 Camera_Unique9(Camera* camera) {
    ...
    if (anim->keyFrameTimer == 0) {
        ...
        if (anim->curKeyFrameIdx < ONEPOINT_CS_INFO(camera)->keyFrameCnt) {
            ...
            if (anim->curKeyFrame->unk_01 != 0xFF) {
                ...
                } else if (camera->player->stateFlags1 & PLAYER_STATE_FLAG_1_27 && player->currentBoots != PLAYER_BOOTS_IRON) {
    ...
    *eye = *eyeNext;
    ...
    if (anim->curKeyFrame->actionFlags & 0x40) {
        ...
        if (camera->player->stateFlags1 & PLAYER_STATE_FLAG_1_27 && player->currentBoots != PLAYER_BOOTS_IRON) {
...
s32 Camera_Demo5(Camera* camera) {
    ...
    if (camera->player->stateFlags1 & PLAYER_STATE_FLAG_1_27 && (player->currentBoots != PLAYER_BOOTS_IRON)) {
        ...
        player->stateFlags1 |= PLAYER_STATE_FLAG_1_29;
        // env frozen
        ...
        if (camera->target->category == ACTORCAT_PLAYER) {
            ...
            if (player->stateFlags1 & PLAYER_STATE_FLAG_1_11) {
...
s32 Camera_CheckWater(Camera* camera) {
    ...
    if (camera->unk_14C & 0x200) {
        if (player->stateFlags2 & PLAYER_STATE_FLAG_2_11) {
