...
s16 Camera_CalcDefaultYaw(Camera* camera, s16 arg1, s16 arg2, f32 arg3, f32 arg4);
...
s32 Camera_GetWaterBoxDataIdx(Camera* camera, f32* waterY) {
    ...
    *waterY = playerPosShape.pos.y;
    ...
    if (!func_8004213C(camera->globalCtx, &camera->globalCtx->colCtx, playerPosShape.pos.x, playerPosShape.pos.z, waterY, &waterBox)) {
...
*/
/*
s32 func_800460A8(Camera *camera, VecSph *eyeAtDir, Vec3f *targetPos, f32 arg3, f32 arg4, f32 *arg5, VecSph *targetPlayerGeo, s16 flags) {
    Vec3f* at = &camera->at;
    Vec3f posOffsetTarget;
    Vec3f atTarget;
    Vec3f sp80;
    Vec3f* floorNorm = &camera->floorNorm;
    VecSph targetPlayerDir;
    PosRot *playerPosRot = &camera->playerPosRot;
    f32 temp_f20;
    f32 phi_f16;
    f32 sp68;
    ...
    posOffsetTarget.x = 0.0f;
    posOffsetTarget.y = playerHeight + arg3;
    posOffsetTarget.z = 0.0f;
    if (PREG(76) && (flags & 1)) {
        posOffsetTarget.y -= Camera_CalcSlopeYAdj(&camera->floorNorm, playerPosRot->rot.y, eyeAtDir->yaw, OREG(9));
    }
    atTarget = playerPosRot->pos;
    atTarget.y += playerHeight;
    OLib_Vec3fDiffToVecSphGeo(targetPlayerGeo, &atTarget, targetPos);
    targetPlayerDir = *targetPlayerGeo;
    if (arg4 < targetPlayerDir.r) {
        targetPlayerDir.r = targetPlayerDir.r * PCT(OREG(38));
    } else{
        sp68 = OLib_ClampMaxDist((playerPosRot->pos.y - camera->playerGroundY) / playerHeight, 1.0f);
        targetPlayerDir.r = (targetPlayerDir.r * PCT(OREG(39))) - (targetPlayerDir.r * ((PCT(OREG(39)) - PCT(OREG(38)))) * (targetPlayerDir.r / arg4));
        targetPlayerDir.r -= targetPlayerDir.r * sp68 * sp68;
    }

    if (flags & 0x80) {
        targetPlayerDir.r = 0.2f * targetPlayerDir.r;
        camera->xzOffsetUpdateRate = 0.01f;
        camera->yOffsetUpdateRate = 0.01f;
    }
    
    OLib_VecSphGeoToVec3f(&sp80, &targetPlayerDir.r);
    
    if (PREG(89)) {
        osSyncPrintf("%f (%f %f %f) %f\n", targetPlayerDir.r / arg4, sp80.x, sp80.y, sp80.z, camera->atLERPStepScale);
    }
    
    posOffsetTarget.x += sp80.x;
    posOffsetTarget.y += sp80.y;
    posOffsetTarget.z += sp80.z;
    
    if (((camera->playerGroundY == camera->playerPosRot.pos.y) || (-0.1f < camera->player->actor.gravity)) || camera->player->stateFlags1 & 0x200000) {
        *arg5 = Camera_LERPCeilF(playerPosRot->pos.y, *arg5, PCT(OREG(43)), 0.1f);
        temp_f20 = playerPosRot->pos.y - *arg5;
        posOffsetTarget.y -= temp_f20;
        Camera_LERPCeilVec3f(&posOffsetTarget, &camera->posOffset, camera->yOffsetUpdateRate, camera->xzOffsetUpdateRate, 0.1f);
    } else {
        if (!(flags & 0x80)) {
            temp_f20 = playerPosRot->pos.y - *arg5;
            sp68 = OLib_Vec3fDistXZ(at, &camera->eye);
            Math_atan2f(temp_f20, sp68);
            phi_f16 = camera->fov * 0.4f;
            sp68 = Math_tanf(DEG_TO_RAD(phi_f16)) * sp68;
            if (sp68 < temp_f20) {
                *arg5 += temp_f20 - sp68;
                temp_f20 = sp68;
            } else if (temp_f20 < -sp68) {
                *arg5 += temp_f20 + sp68;
                temp_f20 = -sp68;
            }
            posOffsetTarget.y -= temp_f20;
            ...
            temp_f20 = playerPosRot->pos.y - *arg5;
            sp68 = Math_atan2f(temp_f20, OLib_Vec3fDistXZ(at, &camera->eye));
            
            if(DEG_TO_RAD(OREG(32)) < sp68){
                phi_f16 = 1.0f - sinf(sp68 - DEG_TO_RAD(OREG(32)));
            } else if(sp68 < DEG_TO_RAD(OREG(33))){
                phi_f16 = 1.0f - sinf(DEG_TO_RAD(OREG(33)) - sp68);
            } else {
                phi_f16 = 1.0f;
            }
                
            posOffsetTarget.y -= temp_f20 * phi_f16;
        }
        Camera_LERPCeilVec3f(&posOffsetTarget, &camera->posOffset, PCT(OREG(29)), PCT(OREG(30)), 0.1f);
        camera->yOffsetUpdateRate = PCT(OREG(29));
        camera->xzOffsetUpdateRate = PCT(OREG(30));
...
s32 func_800460A8(Camera *camera, VecSph *eyeAtDir, Vec3f *targetPos, f32 arg3, f32 arg4, f32 *arg5, VecSph *targetPlayerGeo, s16 flags) {
    ...
    if (PREG(76) != 0) {
        if ((flags & 1) != 0) {
            sp98.y = sp98.y - Camera_CalcSlopeYAdj(&camera->floorNorm, camera->playerPosRot.rot.y, eyeAtDir->yaw, OREG(9));
        }
    }
    ...
    if (arg4 < sp74.r) {
        sp74.r = PCT(OREG(38)) * sp74.r;
    } else {
        temp_f0_2 = OLib_ClampMaxDist((sp50->pos.y - camera->playerGroundY) / temp_f0, 1.0f);
        sp68 = (((PCT(OREG(39)) - PCT(OREG(38))) * sp74.r) * (sp74.r / arg4));
        phi_f16 = (sp74.r *  PCT(OREG(39))) - sp68;
        sp74.r = phi_f16 - (phi_f16 * temp_f0_2 * temp_f0_2);
    }
    ...
    if (camera->playerPosRot.pos.y == camera->playerGroundY || -0.1f < camera->player->actor.gravity || camera->player->stateFlags1 & 0x200000) {
        *arg5 = Camera_LERPCeilF(sp50->pos.y, *arg5, PCT(OREG(43)), 0.1f);
        ...
        if (!(flags & 0x80)) {
            ...
            sp68 = Math_tanf(DEG_TO_RAD(camera->fov * 0.4f)) * sp68;
            if (sp68 < phi_f20) {
                *arg5 = *arg5 + (phi_f20 - sp68);
                phi_f20 = sp68;
            } else if (phi_f20 < -sp68) {
                *arg5 = *arg5 + (phi_f20 + sp68);
                phi_f20 = -sp68;
            }
            sp98.y = sp98.y - phi_f20;
            ...
            phi_f20 = sp50->pos.y - *arg5;
            sp68 = Math_atan2f(phi_f20, OLib_Vec3fDistXZ(sp54, &camera->eye));
            if (DEG_TO_RAD(OREG(32)) < sp68) {
                phi_f16 = 1.0f - sinf(sp68 - DEG_TO_RAD(OREG(32)));
            } else if (DEG_TO_RAD(OREG(33)) > sp68) {
                phi_f16 = 1.0f - sinf(DEG_TO_RAD(OREG(33)) - sp68);
            } else {
                phi_f16 = 1.0f;
            }
...
s32 func_800466F8(Camera* camera, VecSph* eyeAtDir, f32 arg2, f32* arg3, s16 calcSlope) {
    ...
    if (player->rideActor->unk_1F0 & 4 ? 1 : 0) {
        ...
        *arg3 = Camera_LERPCeilF(horsePosRot.pos.y, *arg3, 0.1f, 0.2f);
        camera->atLERPStepScale = Camera_LERPCeilF(0.4f, camera->atLERPStepScale, 0.2f, 0.02f);
    } else {
        *arg3 = Camera_LERPCeilF(horsePosRot.pos.y, *arg3, 0.5f, 0.2f);
    }

    posOffsetTarget.x = 0.0f;
    posOffsetTarget.y = playerHeight + arg2;
    posOffsetTarget.z = 0.0f;
...
s32 Camera_Normal1(Camera *camera) {
    ...
    if(RELOAD_PARAMS){
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yOffsetInverse = (1.0f + PCT(R_CAM_YOFFSET_NORM) - PCT(R_CAM_YOFFSET_NORM) * (68.0f / yOffset));
        sp94 = yOffsetInverse * PCT(yOffset);
    ...
    if (norm1->interfaceFlags & 1) {
        ...
        anim->slopePitchAdj = Camera_LERPCeilS(t, 
                                    anim->slopePitchAdj, 
    ...
    if (norm1->interfaceFlags & 0x80) {
        ...
        func_800457A8(camera, &atEyeNextGeo, spA0, norm1->interfaceFlags & 1);
    }
    ...
    if ((camera->status == CAM_STATUS_ACTIVE) &&  (!(norm1->interfaceFlags & 0x10))) {
        ...
        if (anim->swing.unk_18 != 0) {
            camera->direction.y = Camera_LERPCeilS(camera->direction.y + BINANG_SUB(BINANG_ROT180(anim->swing.unk_16), camera->direction.y), camera->direction.y, 1.0f - (0.99f * sp98), 0xA);
        }
...
s32 Camera_Normal2(Camera *camera) {
    ...
    switch(camera->animState){
        ...
        case 0x14:
        case 0x19:
            sp64 = Camera_GetCamBGData(camera);
            // bg cam data pos
            Camera_Vec3sToVec3f(&temp_s1->unk_00, &BGCAM_POS(sp64));
            // bg cam data rot
            temp_s1->unk_20 = BGCAM_ROT(sp64).x;
            temp_s1->unk_22 = BGCAM_ROT(sp64).y;
            temp_s1->unk_24 = sp3C->pos.y;

            // bg cam data fov
            temp_s1->unk_1C = BGCAM_FOV(sp64) == -1 ? norm2->unk_14 :
                BGCAM_FOV(sp64) >= 0x169 ? PCT(BGCAM_FOV(sp64)) : BGCAM_FOV(sp64);
            
            // bg cam data jfifid
            temp_s1->unk_28 = BGCAM_JFIFID(sp64)== -1 ? 0 : BGCAM_JFIFID(sp64);
    ...
    if ((phi_a1 < 0x4000 && ABS(phi_a0) > phi_a1) || 
        (phi_a1 >= 0x4000 && ABS(phi_a0) < phi_a1)){
        ...
        if(temp_s1->unk_28 & 1){
            sp98.pitch = Camera_CalcDefaultPitch(camera, sp78.pitch, temp_s1->unk_20, 0);
        } else {
    ...
    if (!(temp_s1->unk_28 & 1)) {
        if (sp98.pitch >= 0xE39) {
            sp98.pitch += (BINANG_SUB(0xE38, sp98.pitch) >> 2);
        }

        if (sp98.pitch < 0) {
            sp98.pitch += (BINANG_SUB(-0x38E, sp98.pitch) >> 2);
        }
    }
    ...
    if (camera->status == CAM_STATUS_ACTIVE) {
        ...
        if ((camera->globalCtx->envCtx.skyDisabled == 0) || norm2->interfaceFlags & 0x10) {
            Camera_BGCheckInfo(camera, sp44, &spAC);
            *sp48 = spAC.pos;
        } else {
...
s32 Camera_Normal3(Camera *camera) {
    ...
    if (anim->swing.swingUpdateRateTimer != 0) {
        ...
        camera->pitchUpdateRateInv = Camera_LERPCeilF((f32)OREG(7) + (anim->swing.swingUpdateRateTimer * 2), camera->pitchUpdateRateInv, sp94, 0.1f);
        if(!sp98){}
        anim->swing.swingUpdateRateTimer--;
    } else {
    ...
    func_800466F8(camera, &sp74, norm3->yOffset, &anim->unk_20, 1);
    ...
    sp84.yaw = fabsf(sp98) > (150.0f * (1.0f - camera->speedRatio)) ?
        (s16)(sp74.yaw + sp98) : 
        sp74.yaw;
...
s32 Camera_Parallel1(Camera *camera) {
    ...
    if (!(para1->interfaceFlags & 0x80) && !sp6A) {
        func_80045C74(camera, &atToEyeNextDir, para1->unk_00, &anim->yTarget, para1->interfaceFlags & 1);
    }else {
...
s32 Camera_Jump1(Camera *camera) {
    ...
    if (RELOAD_PARAMS) {
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yOffsetInverse = (1.0f + PCT(R_CAM_YOFFSET_NORM)) - (PCT(R_CAM_YOFFSET_NORM) * (68.0f / playerYOffset));
...
s32 Camera_Jump3(Camera *camera) {  
    ...
    sp74 = Player_GetHeight(camera->player);
    func_8002EEE4(&playerPosRot2, &camera->player->actor);
    ...
    if (anim->swing.unk_18 != 0) {
        ...
        eyeDiffSph.pitch = Camera_CalcDefaultPitch(camera, eyeNextAtOffset.pitch, jump3->unk_20, 0);
    }
...
s32 Camera_Battle1(Camera *camera) {
    ...
    spE8 = 0;
    player = camera->player;
    sp64 = Player_GetHeight(player);
    if(RELOAD_PARAMS){
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 temp_f12 = (1.0f + PCT(OREG(46))) - (PCT(OREG(46)) * (68.0f / sp64));
        batt1->unk_00 = NEXTPCT * sp64 * temp_f12;
        ...
        batt1->unk_24 = NEXTPCT * sp64 * temp_f12;
    ...
    if(camera->target == NULL || camera->target->update == NULL){
        if(camera->target == NULL){
            osSyncPrintf((const char *) "\x1b[43;30mcamera: warning: battle: target is not valid, change parallel\n\x1b[m");
            if(anim){}
        }
        camera->target = NULL;
    ...
    if(camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14){   
        ...
        camera->animState++;
        if (anim->target->id > 0) {
            osSyncPrintf("camera: battle: target actor name \x1b[34m%d\x1b[m\n", anim->target->id);
        } else {
            osSyncPrintf("camera: battle: target actor name \x1b[41;37m%d\x1b[m\n", anim->target->id);
            camera->target = NULL;
            Camera_ChangeModeDefaultFlags(camera, CAM_MODE_PARALLEL);
            return 1;
        }
    ...
    if (anim->target != camera->target) { // 8518
        osSyncPrintf("camera: battle: change target %d -> \x1b[34m%d\x1b[m\n", anim->target->id, camera->target->id);
        camera->animState = 0;
        return true;
    }
    ...
    spAC.r = sp80 - ((spAC.r >= sp80 ? sp80 : spAC.r) * 0.5f);
    ...
    //sp8C = spA4.yaw;
    sp8C = BINANG_SUB(spAC.yaw, BINANG_ROT180(sp94.yaw));
    spB4.yaw = sp94.yaw;
    if (anim->unk_1A != 0) { //8738
        if (anim->unk_1A >= OREG(24)) {
            sp86 = anim->unk_1A - OREG(24);
            OLib_Vec3fDiffToVecSphGeo(&spA4, at, eye);
            spA4.yaw = BINANG_ROT180(sp8C);
            var3 = ((anim->unk_00 - spA4.r) * (1.0f / OREG(23)));
            sp8E = BINANG_SUB(anim->unk_16, spA4.yaw) * (1.0f / OREG(23));
            sp8C = BINANG_SUB(anim->unk_18, spA4.pitch);
            sp8C *= (1.0f / OREG(23));
            spB4.r = Camera_LERPCeilF((sp86 * var3) + spA4.r, sp9C.r, PCT(OREG(28)), 1.0f);
            if(1){
            spB4.yaw = Camera_LERPCeilS((sp8E * sp86) + spB4.yaw, sp9C.yaw, PCT(OREG(28)), 0xA);
            }
            spB4.pitch = Camera_LERPCeilS((sp8C * sp86) + spA4.pitch, sp9C.pitch, PCT(OREG(28)), 0xA);
        } else {
            spE8 = 1;
        }
        anim->unk_1A--;
    } else if (ABS(BINANG_SUB(spAC.yaw, BINANG_ROT180(sp94.yaw))) > DEGF_TO_BINANG(spF4)) { // 8970
        sp8E = BINANG_SUB(spAC.yaw, BINANG_ROT180(sp94.yaw));
        spFC = sp8C * 0.00549325f;
        temp_f2_2 = F32_LERPIMP(spF4, spF8, OLib_ClampMaxDist(spAC.r, spB4.r));
        temp_f12_2 = ((temp_f2_2 * temp_f2_2) - 2.0f) / (temp_f2_2 - 360.0f);
        var2 = ((temp_f12_2 * spFC) + (2.0f - (360.0f * temp_f12_2)));
        temp_f14 = SQ(spFC) / var2;

        spB4.yaw = BINANG_ROT180(BINANG_ROT180(sp94.yaw) + 
            (sp8E >= 0 ? DEGF_TO_BINANG(temp_f14) : (s16)-DEGF_TO_BINANG(temp_f14)));
    } else {
        var4 = ((1.0f - camera->speedRatio) * 0.05f);
        sp8C = ABS(DEGF_TO_BINANG(spF4));
        spB4.yaw = BINANG_SUB(sp94.yaw, (sp8C - (spAC.yaw - BINANG_ROT180(sp94.yaw))) * var4);
    
    }

    if (spE8 == 0) {
        sp8C = DEGF_TO_BINANG(F32_LERPIMP(sp7C, sp78, spE4));
        sp8E = spAC.pitch * batt1->unk_18;
        spB4.pitch = Camera_LERPCeilS(CLAMP(sp8E + (sp8C - (s16)(spA4.pitch * (0.5f + (spE4 * 0.5f)))), -0x2AA8, 0x2AA8), sp94.pitch, anim->unk_10, 0xA);
        Camera_Vec3fVecSphGeoAdd(eyeNext, at, &spB4);
        spBC.pos = *eyeNext;
        if (camera->status == CAM_STATUS_ACTIVE) {
            if (camera->globalCtx->envCtx.skyDisabled == 0 || batt1->interfaceFlags & 1) {
    ...
    anim->unk_04 = anim->unk_04 + ((((OREG(36) * camera->speedRatio) * (1.0f - spE4)) - anim->unk_04) * PCT(OREG(37)));
...
s32 Camera_Battle4(Camera *camera) {
    ...
    if (RELOAD_PARAMS) {
        ...
        batt4->yOffset = (NEXTPCT * yOffset) * yNormal;
        batt4->rTarget = (NEXTPCT * yOffset) * yNormal;
...
s32 Camera_KeepOn1(Camera *camera) {
    ...
    sp70 = Player_GetHeight(camera->player);
    if (camera->target == NULL || camera->target->update == NULL){
        if (camera->target == NULL) {
    ...
    if(RELOAD_PARAMS){
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yNormal = (1.0f + PCT(OREG(46))) - (PCT(OREG(46)) * (68.0f / sp70));
        keep1->unk_00 = NEXTPCT * sp70 * yNormal;
        ...
        keep1->unk_28 = NEXTPCT * sp70 * yNormal;
    ...
    switch(camera->paramFlags & 0x18){
        case 8:
            if (camera->player->actor.type == 2 && camera->target == camera->player->interactRangeActor) {
                PosRot sp54;
                func_8002EEE4(&sp54, &camera->player->actor);
                spC8.r = 60.0f;
                spC8.pitch = 0x2EE0;
                spC8.yaw = playerPosRot->rot.y;
...
cont:
            if (camera->playerGroundY == camera->playerPosRot.pos.y ||
                (-0.1f < camera->player->actor.gravity) ||
            ...
            func_800460A8(camera, &spB8, &camera->targetPosRot.pos, sp80 ? keep1->unk_28 : keep1->unk_00, sp104, &anim->unk_08, &spC8, (sp80 ? 0x80 : 0) | keep1->interfaceFlags);
    ...
    spEC = F32_LERPIMP(keep1->unk_0C, keep1->unk_10, (1.1f - sp84));
    spF0 = OREG(13) + spEC;
    spD8.r = camera->dist = Camera_LERPCeilF(spE8, camera->dist, PCT(OREG(11)), 2.0f);
    spE0 = BINANG_SUB(spD0.yaw, BINANG_ROT180(spB8.yaw));
    spD8.yaw = spB8.yaw;
    if (anim->unk_16 != 0) {
        if (anim->unk_16 >= OREG(24)) {
            ...
            temp_f0_8 = (anim->unk_00 - spC8.r) * (1.0f / OREG(23));
            spE2 = BINANG_SUB(anim->unk_12, spC8.yaw) * (1.0f / OREG(23));
            spE0 = BINANG_SUB(anim->unk_14, spC8.pitch) * (1.0f / OREG(23));
            spD8.r = Camera_LERPCeilF((sp82 * temp_f0_8) + spC8.r, spC0.r, PCT(OREG(28)), 1.0f);
            spD8.yaw = Camera_LERPCeilS((spE2 * sp82) + spC8.yaw, spC0.yaw, PCT(OREG(28)), 0xA);
            spD8.pitch = Camera_LERPCeilS((spE0 * sp82) + spC8.pitch, spC0.pitch, PCT(OREG(28)), 0xA);
        } else {
            sp88 = 1;
        }
        anim->unk_16--;
    } else if (ABS(spE0) > DEGF_TO_BINANG(spEC)) {
        spE2 = BINANG_SUB(spD0.yaw, BINANG_ROT180(spB8.yaw));
        spF4 = spE0 * 0.00549325f;
        temp_f2_2 = F32_LERPIMP(spEC, spF0, OLib_ClampMaxDist(spD0.r, spD8.r) / spD8.r);
        temp_f12_2 = ((temp_f2_2 * temp_f2_2) - 2.0f) / (temp_f2_2 - 360.0f);
        temp_f0_8 = ((temp_f12_2 * spF4) + (2.0f - (360.0f * temp_f12_2)));
        temp_f14 = SQ(spF4) / temp_f0_8;
        spD8.yaw = BINANG_ROT180(BINANG_ROT180(spB8.yaw) + (s16)((spE2 >= 0) ? 
            DEGF_TO_BINANG(temp_f14) : 
            (s16)-DEGF_TO_BINANG(temp_f14)));
    } else {
        temp_f0_8 = (1.0f - camera->speedRatio) * 0.02f;
        spE2 = spE0 >= 0 ? DEGF_TO_BINANG(spEC) : (s16)-DEGF_TO_BINANG(spEC);
        spD8.yaw = spB8.yaw - (s16)((spE2 - spE0) * temp_f0_8);
    }

    if (!sp88) {
        spD8.pitch = Camera_LERPCeilS(CLAMP((s16)(DEGF_TO_BINANG(F32_LERPIMP(keep1->unk_14, keep1->unk_18, sp84)) - (s16)(spC8.pitch * (0.5f + (sp84 * 0.5f))) + (s16)(spD0.pitch * keep1->unk_1C)), -0x3200, 0x3200),
            spB8.pitch, PCT(OREG(12)), 0xA);
        Camera_Vec3fVecSphGeoAdd(eyeNext, at, &spD8);
        sp8C.pos = *eyeNext;
        if (camera->status == 7) {
            if (camera->globalCtx->envCtx.skyDisabled == 0 || keep1->interfaceFlags) {
...
s32 Camera_KeepOn3(Camera *camera) {
    ...
    sp58 = Player_GetHeight(camera->player);
    if (camera->target == NULL || camera->target->update == NULL){
        if (camera->target == NULL){
            osSyncPrintf((const char *) "\x1b[43;30mcamera: warning: talk: target is not valid, change parallel\n\x1b[m");
        }
        camera->target = NULL;
        Camera_ChangeModeDefaultFlags(camera, CAM_MODE_PARALLEL);
        return 1;
    ...
    if(RELOAD_PARAMS){
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yNormal = (1.0f + PCT(OREG(46))) - (PCT(OREG(46)) * (68.0f / sp58));
        keep3->unk_00 = NEXTPCT * sp58 * yNormal;
    ...
    sp58 += keep3->unk_00;
    OLib_Vec3fDiffToVecSphGeo(&sp94, at, eye);
    OLib_Vec3fDiffToVecSphGeo(&sp8C, at, eyeNext);
    func_8002EEE4(&camera->targetPosRot, camera->target);
    func_8002EEE4(&sp6C, camera->player);
    spD0 = playerPosRot->pos;
    spD0.y += sp58;
    OLib_Vec3fDiffToVecSphGeo(&spA4, &spD0, &camera->targetPosRot.pos);
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        ...
        camera->animState++;
        anim->unk_0C = camera->target;
        temp_f0 = (keep3->unk_08 < spA4.r ? 1.0f : spA4.r / keep3->unk_08);
        anim->unk_1C = keep3->unk_28;
        spBC = ((1.0f - temp_f0) * spA4.r) / anim->unk_1C;
        sp9C.pitch = DEGF_TO_BINANG(F32_LERPIMP(keep3->unk_14, keep3->unk_18, temp_f0)) + (s16)-(spA4.pitch * keep3->unk_1C);
        temp_f14_2 = F32_LERPIMP(keep3->unk_0C, keep3->unk_10,  temp_f0);
        if (keep3->unk_2A & 0x10) {
            if (BINANG_SUB(spA4.yaw, sp8C.yaw) < 0) {
                sp9C.yaw = spA4.yaw + DEGF_TO_BINANG(temp_f14_2);
            } else {
                sp9C.yaw = spA4.yaw - DEGF_TO_BINANG(temp_f14_2);
            }
        } else if (keep3->unk_2A & 0x20) {
            if(BINANG_SUB(spA4.yaw, sp8C.yaw) < 0){
                sp9C.yaw = BINANG_ROT180(spA4.yaw) - DEGF_TO_BINANG(temp_f14_2);
            } else {
                sp9C.yaw = BINANG_ROT180(spA4.yaw) + DEGF_TO_BINANG(temp_f14_2);
            }
        } else if (ABS(BINANG_SUB(spA4.yaw, sp8C.yaw)) < 0x3FFF) {
            if (BINANG_SUB(spA4.yaw, sp8C.yaw) < 0) {
                sp9C.yaw = spA4.yaw + DEGF_TO_BINANG(temp_f14_2);
            } else {
                sp9C.yaw = spA4.yaw - DEGF_TO_BINANG(temp_f14_2);
        ...
        sp80 = sp9C.yaw;
        sp82 = sp9C.pitch;
        spA4.r = F32_LERP(spB8, spBC, 0.6f);
        spD0 = playerPosRot->pos;
        spD0.y += sp58;
        Camera_Vec3fVecSphGeoAdd(&anim->unk_10, &spD0, &spA4);
        sp84 = ARRAY_COUNT(D_8011D3B0);
        i = 0;
        spA4.r = spB8;
        sp9C.r = (spB8 * 0.5f) + keep3->unk_04 - sp8C.r + sp8C.r;
        Camera_Vec3fVecSphGeoAdd(&spC4, &anim->unk_10, &sp9C);
        if (!(keep3->unk_2A & 0x80)) {
            while(i < sp84){
                if (func_800626B0(camera->globalCtx, &camera->globalCtx->colChkCtx, &anim->unk_10, &spC4, &spAC, 2) || 
                    Camera_BGCheck(camera, &anim->unk_10, &spC4)) {
                    sp9C.yaw = D_8011D3B0[i] + sp80;
                    sp9C.pitch = D_8011D3CC[i] + sp82;
                    Camera_Vec3fVecSphGeoAdd(&spC4, &anim->unk_10, &sp9C);
                } else {
                    break;
                }
        ...
        anim->unk_00.y = BINANG_SUB(sp9C.yaw, sp8C.yaw) / temp_f0;
        anim->unk_00.z = BINANG_SUB(sp9C.pitch, sp8C.pitch) / temp_f0;
        anim->unk_00.x = (sp9C.r - sp8C.r) / temp_f0;
        return 1;
    }

    if (anim->unk_1C != 0) {
        at->x = F32_LERPIMPINV(at->x, anim->unk_10.x, anim->unk_1C);
        at->y = F32_LERPIMPINV(at->y, anim->unk_10.y, anim->unk_1C);
        at->z = F32_LERPIMPINV(at->z, anim->unk_10.z, anim->unk_1C);
        sp9C.r = ((anim->unk_00.x * anim->unk_1C) + sp8C.r) + 1.0f;
        sp9C.yaw = sp8C.yaw + (s16)(anim->unk_00.y * anim->unk_1C);
        sp9C.pitch = sp9C.pitch + (s16)(anim->unk_00.z * anim->unk_1C);
        Camera_Vec3fVecSphGeoAdd(eyeNext, at, &sp9C);
        *eye = *eyeNext;
...
s32 Camera_KeepOn4(Camera *camera) {
    ...
    sp88 = Player_GetHeight(camera->player);
    camera->unk_14C &= ~0x10;
    if(RELOAD_PARAMS){
        const f32 t = -0.5f;
    ...
    *sp44 = *sp3C;
    Camera_BGCheck(camera, sp40, sp44);
...
s32 Camera_Fixed2(Camera *camera) {
    ...
    if (RELOAD_PARAMS) {
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yNormal = (1.0f + PCT(R_CAM_YOFFSET_NORM)) - (PCT(R_CAM_YOFFSET_NORM) * (68.0f / yOffset));
        fixd2->yOffset = (NEXTPCT * yOffset) * yNormal;
 ...
 * First person view
*/
#define GEO_RATE(dst,s1,s2,rte) {\
(dst)->r = ((s2)->r - (s1)->r) * rte;\
(dst)->pitch = BINANG_SUB((s2)->pitch, (s1)->pitch) * rte;\
(dst)->yaw = BINANG_SUB((s2)->yaw, (s1)->yaw) * rte;\
}
s32 Camera_Subj3(Camera *camera) {
    ...
    sp3C = Player_GetHeight(camera->player);
    if (camera->globalCtx->view.unk_124 == 0) {
        camera->globalCtx->view.unk_124 = camera->thisIdx | 0x50;
        return 1;
    }
    ...
    sp84.yaw = (s16)(sp60.rot.y - 0x7FFF);
    sp98 = sp60.pos;
    sp98.y += subj3->eyeNextYOffset;
    Camera_Vec3fVecSphGeoAdd(&sp8C, &sp98, &sp84);
    OLib_Vec3fDiffToVecSphGeo((VecSph *) &sp7C, at, eye);
    sCameraInterfaceFlags = (s32) subj3->interfaceFlags;
    ...
    if (t != 0) {
        ...
        testAt.y = at->y;
        testAt.z = at->z;
        VEC3F_LERPIMPDST(at, &testAt, &sp98, (1.0f / t));
        GEO_RATE(&tsph2, &sp84, &tsph, (1.0f / OREG(23)));
        sp7C.r = Camera_LERPCeilF(sp84.r + (anim->animTimer * tsph2.r), sp7C.r, PCT(OREG(28)), 1.0f);
        sp7C.yaw = Camera_LERPCeilS(sp84.yaw + (anim->animTimer * tsph2.yaw), sp7C.yaw, PCT(OREG(28)), 0xA);
        sp7C.pitch = Camera_LERPCeilS(sp84.pitch + (anim->animTimer * tsph2.pitch), sp7C.pitch, PCT(OREG(28)), 0xA);
        ...
        *eye = *eyeNext;
        ...
        sp58 = Math_Sins((s16)(sp60.rot.y - 0x7FFF));
        temp_f0_3 = Math_Coss((s16)(sp60.rot.y - 0x7FFF));
        ...
        sp7C.yaw = (s16)(sp60.rot.x - 0x7FFF);
        Camera_Vec3fVecSphGeoAdd(eyeNext, at, &sp7C);
...
s32 Camera_Data4(Camera *camera)
{
    ...
    {
        values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        yNormal = (1.0f + PCT(R_CAM_YOFFSET_NORM)) - (PCT(R_CAM_YOFFSET_NORM) * (68.0f / yOffset));
        data4->yOffset = (NEXTPCT * yOffset) * yNormal;
...
s32 Camera_Unique1(Camera *camera) {
    ...
    if (RELOAD_PARAMS) {
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yNormal = (1.0f + (R_CAM_YOFFSET_NORM * 0.01f)) - ((R_CAM_YOFFSET_NORM * 0.01f) * (68.0f / yOffset));
        uniq1->yOffset = NEXTPCT * yOffset * yNormal;
        uniq1->distClampMin = NEXTPCT * yOffset * yNormal;
        uniq1->distClampMax = NEXTPCT * yOffset * yNormal;
...
s32 Camera_Unique2(Camera *camera) {
    ...
    if (RELOAD_PARAMS) {
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yNormal = ((1.0f + PCT(R_CAM_YOFFSET_NORM)) - (PCT(R_CAM_YOFFSET_NORM) * (68.0f / yOffset)));
        uniq2->yOffset = (NEXTPCT * yOffset) * yNormal;
...
s32 Camera_Unique3(Camera *camera) {
    ...
    if(RELOAD_PARAMS){
        values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        yNormal = (1.0f + PCT(R_CAM_YOFFSET_NORM)) - (PCT(R_CAM_YOFFSET_NORM) * (68.0f / yOffset));
        unkC->unk_00 = (NEXTPCT * yOffset) * yNormal;
...
s32 Camera_Unique0(Camera *camera) {
    ...
    Vec3s sceneCamRot;
    PosRot *playerPosRot = &camera->playerPosRot;
    PersonalizedUnique0* persUniq0 = (PersonalizedUnique0*)camera->paramData;
...
s32 Camera_Special9(Camera *camera) {
    ...
    f32 yNormal;
    PersonalizedDoor* doorc = (PersonalizedDoor*)camera->paramData;
    ...
    if(RELOAD_PARAMS){
        ...
        spec9->yOffset = NEXTPCT * playerYOffset * yNormal;
...
void Camera_PrintSettings(Camera *camera) {
    ...
    if (OREG(0) & 1 && camera->thisIdx == camera->globalCtx->activeCamera && !gDbgCamEnabled) {
        phi_a1 = 0;
        for(phi_v1 = sp58, phi_t0 = sp48; phi_v1 < &sp58[4]; phi_v1++, phi_t0++){
        ...
        *phi_t0 = '\0';
        ...
        phi_t0[camera->globalCtx->activeCamera] = 'a';
        func_8006376C(3, 0x16, 5, sp58);
        func_8006376C(3, 0x16, 1, phi_t0);
...
s32 Camera_ChangeDoorCam(Camera* camera, Actor* doorActor, s16 camDataIdx, f32 arg3, s16 timer1, s16 timer2, s16 timer3) {
    PersonalizeParams* params = (PersonalizeParams*)camera->paramData;
