...
s16 Camera_XZAngle(Vec3f* to, Vec3f* from) {
    return DEGF_TO_BINANG(RADF_TO_DEGF(MathF_Atan2(from->x - to->x, from->z - to->z)));
}
...
s16 func_80044ADC(Camera* camera, s16 yaw, s16 arg2) {
    ...
    temp_s0 = DEGF_TO_BINANG(RADF_TO_DEGF(MathF_Atan2(phi_f16, sp30)));
    temp_s1 = DEGF_TO_BINANG(RADF_TO_DEGF(MathF_Atan2(phi_f18, sp2C)));
...
s32 func_800458D4(Camera* camera, VecSph* eyeAtDir, f32 arg2, f32* arg3, s16 arg4) {
    ...
    eyeAtAngle = MathF_Atan2(deltaY, OLib_Vec3fDistXZ(&camera->at, &camera->eye));
    ...
    if (camera->playerGroundY == camera->playerPosRot.pos.y || camera->player->actor.gravity > -0.1f ||
        ...
        *arg3 = Camera_LERPCeilF(playerPosRot->pos.y, *arg3, PCT(OREG(43)), 0.1f);
        ...
        if (!PREG(75)) {
            ...
            phi_f16 = sp54;
            MathF_Atan2(phi_f20, sp54);
            temp_f2 = MathF_Tan(DEG_TO_RAD(camera->fov * 0.4f)) * phi_f16;
            if (temp_f2 < phi_f20) {
            ...
            phi_f20 = playerPosRot->pos.y - *arg3;
            temp_f2 = MathF_Atan2(phi_f20, OLib_Vec3fDistXZ(at, eye));
    ...
    if (camera->playerGroundY == camera->playerPosRot.pos.y || camera->player->actor.gravity > -0.1f ||
        ...
        *yPosOffset = Camera_LERPCeilF(playerPosRot->pos.y, *yPosOffset, PCT(OREG(43)), 0.1f);
        ...
        if (!(flags & FLG_OFFGROUND)) {
            ...
            phi_f16 = eyeAtDist;
            MathF_Atan2(yPosDelta, eyeAtDist);
            temp_f0_2 = MathF_Tan(DEG_TO_RAD(camera->fov * 0.4f)) * phi_f16;
            ...
            yPosDelta = playerPosRot->pos.y - *yPosOffset;
            temp_f0_2 = MathF_Atan2(yPosDelta, OLib_Vec3fDistXZ(at, &camera->eye));
