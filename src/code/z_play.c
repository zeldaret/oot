...
void func_800BC490(GlobalContext* globalCtx, s16 point) {
    ...
    if ((YREG(15) != 0x10) && (gSaveContext.cutsceneIndex < 0xFFF0)) {
        Audio_PlaySfxGeneral((point == 1) ? NA_SE_SY_CAMERA_ZOOM_DOWN : NA_SE_SY_CAMERA_ZOOM_UP, &gSfxPosScreenCenter,
                             4, &gSfxFreqOrVolDefaultVal, &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    }
    ...
    if (FrameAdvance_Update(&globalCtx->frameAdvCtx, &input[1])) {
        ...
        if (globalCtx->transitionMode) {
            ...
            switch (globalCtx->transitionMode) {
                ...
                case 13:
                    Audio_PlaySfxGeneral(NA_SE_EV_SAND_STORM - SFX_FLAG, &gSfxPosScreenCenter, 4,
