...
void Audio_ProcessSeqCmd(u32 cmd) {
    ...
    switch (op) {
        ...
        case SEQ_CMD_SET_PLAYER_VOLUME:
        ...
        case SEQ_CMD_SET_CHANNEL_VOLUME:
...
void Audio_SetVolumeScale(u8 playerIndex, u8 scaleIndex, u8 targetVol, u8 volFadeTimer) {
    ...
    if (volFadeTimer != 0) {
        ...
        SEQCMD_SET_PLAYER_VOLUME_NOW(playerIndex, volFadeTimer, volScale);
    }
}
...
void Audio_UpdateActiveSequences(void) {
    ...
    for (playerIndex = 0; playerIndex < 4; playerIndex++) {
        ...
        if (gActiveSeqs[playerIndex].setupCmdNum != 0) {
            ...
            // Only process setup commands if `playerIndex` if no longer playing
