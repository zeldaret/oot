    ...
    if (this != NULL) {
        ...
        if (!(this->flags & 2)) {
            ...
            if (!(fabsf(scale) < 0.008f)) {
                ...
                SkinMatrix_Translate(&sp110, sp160.x, sp160.y, sp160.z);
...
void EffectBlure_DrawSmooth(EffectBlure* this, GraphicsContext* gfxCtx) {
    ...
    SkinMatrix_Translate(&spDC, this->elements[0].p2.x, this->elements[0].p2.y, this->elements[0].p2.z);
    SkinMatrix_Scale(&sp9C, 0.1f, 0.1f, 0.1f);
    ...
    for (i = 0; i < this->numElements - 1; i++) {
        ...
        if (this->flags & 4) {
            ...
            if (fabsf(scale) > 0.0005f) {
                ...
                SkinMatrix_Translate(&sp154, sp1B0.x, sp1B0.y, sp1B0.z);
