...
s32 __osLeoInterrupt(void) {
    ...
    if (stat & LEO_STATUS_MECHANIC_INTR) {
        ...
        IO_WRITE(ASIC_BM_CTL, info->bmCtlShadow | LEO_BM_CLR_MECHANIC_INTR);
    ...
    if (info->cmdType == 1) {
        if (!(stat & 0x40000000)) {
            if ((info->sectorNum + 1) != (info->transferMode * 0x55)) {
                blockInfo->errStatus = LEO_SENSE_NO_REFERENCE_POSITION_FOUND;
            ...
            info->sectorNum += 1;
            __osEPiRawStartDma(__osDiskHandle, OS_WRITE, ASIC_SECTOR_BUFF, blockInfo->dramAddr, blockInfo->sectorSize);
        }
        return 1;
    }
    if (info->cmdType == 0) {
        if (info->transferMode == 3) {
            ...
            if (!(stat & 0x40000000)) {
                blockInfo->errStatus = LEO_SENSE_UNRECOVERED_READ_ERROR;
                __osLeoAbnormalResume();
                return 1;
            }
        } else {
        ...
        if (stat & 0x10000000) {
            if (info->sectorNum != 0x57) {
                ...
                __osEPiRawStartDma(__osDiskHandle, OS_READ, ASIC_C2_BUFF, blockInfo->C2Addr, blockInfo->sectorSize * 4);
            }
            return 1;
        }
        if ((info->sectorNum == -1) && (info->transferMode == 2) && (info->blockNum == 1)) {
            ...
            info->block[0].errStatus = LEO_SENSE_NO_ADDITIONAL_SENSE_INFOMATION;
            __osLeoResume();
        }
        info->sectorNum += 1;
        if (stat & 0x40000000) {
            ...
            __osEPiRawStartDma(__osDiskHandle, OS_READ, ASIC_SECTOR_BUFF, blockInfo->dramAddr, blockInfo->sectorSize);
            blockInfo->errStatus = LEO_SENSE_NO_ADDITIONAL_SENSE_INFOMATION;
            return 1;
        }
        if (info->sectorNum < 0x55) {
...
void __osLeoAbnormalResume(void) {
    ...
    IO_WRITE(ASIC_BM_CTL, info->bmCtlShadow | LEO_BM_RESET);
    while (((pi_stat = IO_READ(PI_STATUS_REG)) & (PI_STATUS_DMA_BUSY | PI_STATUS_IO_BUSY)) != 0) {}
