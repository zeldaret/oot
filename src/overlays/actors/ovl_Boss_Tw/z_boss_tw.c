    ...
    if (fabsf(beamDistFromPlayer.x) < 20.0f && fabsf(beamDistFromPlayer.y) < 50.0f && beamDistFromPlayer.z > 100.0f &&
        ...
        if (sTwinrovaPtr->timers[2] == 0) {
            ...
            if (this->actor.params == TW_KOTAKE) {
                ...
                Player_PlaySfx(player, player->ageProperties->sfxIdOffsetForAdultLink + NA_SE_VO_LI_DEMO_DAMAGE);
            }
        }
...
s32 BossTw_CheckBeamReflection(BossTw* this, PlayState* play) {
    ...
    if (player->stateFlags1 & PLAYER_STATE1_USING_SHIELD &&
        ...
        if (fabsf(vec.x) < 30.0f && fabsf(vec.y) < 70.0f && vec.z > 100.0f && vec.z <= this->beamDist) {
            ...
            if (sBeamDivertTimer == 0) {
                ...
                this->timers[0] = 10;
                Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_IT_SHIELD_REFLECT_MG2);
            }
            ...
            case 0:
                if (this->timers[0] != 0) {
                    ...
                    if (beamReflection == 1) {
                        ...
                        this->beamShootState = 1;
                        Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&player->actor.projectedPos, NA_SE_IT_SHIELD_REFLECT_MG);
...
void BossTw_TwinrovaMergeCS(BossTw* this, PlayState* play) {
    ...
    switch (this->csState1) {
        ...
        case 2:
            ...
            if (this->timers[3] == 16) {
                Player_PlaySfx(player, player->ageProperties->sfxIdOffsetForAdultLink + NA_SE_VO_LI_SURPRISE);
            }
            ...
            if (this->timers[1] == 8) {
                ...
                Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_TWINROBA_YOUNG_WINK);
            }
            if (this->timers[2] == 4) {
                sEnvType = 0;
...
void BossTw_TwinrovaIntroCS(BossTw* this, PlayState* play) {
    ...
    if (this->csSfxTimer > 220 && this->csSfxTimer < 630) {
        Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_TWINROBA_UNARI - SFX_FLAG);
    }

    if (this->csSfxTimer == 180) {
        Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&D_8094A7D0, NA_SE_EN_TWINROBA_LAUGH);
        Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&D_8094A7D0, NA_SE_EN_TWINROBA_LAUGH2);
    ...
    switch (this->csState2) {
        ...
        case 1:
            ...
            if (this->work[CS_TIMER_1] == 180) {
                Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_TWINROBA_APPEAR_MS);
            }
...
void BossTw_DeathCSMsgSfx(BossTw* this, PlayState* play) {
    ...
    if (this->work[CS_TIMER_2] > 440 && this->work[CS_TIMER_2] < 860) {
        Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_TWINROBA_FIGHT - SFX_FLAG);
    }
        ...
        case 0:
            ...
            if (this->work[CS_TIMER_1] >= 55) {
                ...
                if (this->work[CS_TIMER_1] == 135) {
                    ...
                    Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_TWINROBA_TRANSFORM);
                    for (i = 0; i < 100; i++) {
...
void BossTw_TwinrovaUpdate(Actor* thisx, PlayState* play2) {
    ...
    if (sFreezeState == 1) {
        ...
        Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&player->actor.projectedPos, NA_SE_VO_LI_FREEZE);
        Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&player->actor.projectedPos, NA_SE_PL_FREEZE);
...
void BossTw_ShieldChargeDraw(BossTw* this, PlayState* play) {
    ...
    if (temp_t0 == 1) {
        Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_IT_SHIELD_CHARGE_LV1 & ~SFX_FLAG);
    } else if (temp_t0 == 2) {
        Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_IT_SHIELD_CHARGE_LV2 & ~SFX_FLAG);
    } else if (temp_t0 == 3) {
        Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_IT_SHIELD_CHARGE_LV3 & ~SFX_FLAG);
    }
