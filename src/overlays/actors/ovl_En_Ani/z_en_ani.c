...
void EnAni_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_InitFlex(globalCtx, &this->skelAnime, &D_060000F0, &D_060076EC, this->jointTbl, this->morphTbl, 0x10);
...
void func_809B0994(EnAni* this, GlobalContext* globalCtx) {
    if (globalCtx->csCtx.npcActions[0]->action == 4) {
        SkelAnime_ChangeAnim(&this->skelAnime, &D_060070F0, 1.0f, 0.0f, SkelAnime_GetLastFrame(&D_060070F0), 2, -4.0f);
        this->unk_2AA++;
...
void func_809B0A6C(EnAni* this, GlobalContext* globalCtx) {
    ...
    if (globalCtx->csCtx.npcActions[0]->action == 2) {
        SkelAnime_ChangeAnim(&this->skelAnime, &D_060067B8, 1.0f, 0.0f, SkelAnime_GetLastFrame(&D_060067B8), 2, 0.0f);
