...
void EnTp_Head_ApproachPlayer(EnTp* this, GlobalContext* globalCtx) {
    ...
    Audio_PlaySfxGeneral(NA_SE_EN_TAIL_FLY - SFX_FLAG, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
...
void EnTp_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params <= TAILPASARAN_HEAD) {
        ...
        if (this->actor.shape.rot.z == 0) {
            Audio_PlaySfxGeneral(NA_SE_EN_TAIL_CRY, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
        }
