...
void EnTp_UpdateDamage(EnTp* this, PlayState* play) {
    ...
    if ((this->collider.base.acFlags & AC_HIT) && (this->actionIndex >= TAILPASARAN_ACTION_TAIL_FOLLOWHEAD)) {
        ...
        if (this->actor.colChkInfo.damageEffect_CollisionCheckInfo != TAILPASARAN_DMGEFF_NONE) {
            if (this->actor.colChkInfo.damageEffect_CollisionCheckInfo == TAILPASARAN_DMGEFF_DEKUNUT) {
                phi_s4 = 1;
            }
            ...
            if (this->actor.colChkInfo.health == 0) {
                ...
                if (head->actor.params <= TAILPASARAN_HEAD) {
                    EnTp_SetupDie(head);
                    head->damageEffect = this->actor.colChkInfo.damageEffect_CollisionCheckInfo;
