...
void EnTp_Init(Actor* thisx, PlayState* play2) {
    ...
    if (this->actor.params <= TAILPASARAN_HEAD) {
        ...
        this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE | ACTOR_FLAG_DISABLE_UPDATE_CULLING;
        ...
        for (i = 0; i <= 6; i++) {
            ...
            if (next != NULL) {
                ...
                if (i == 2) {
                    next->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE | ACTOR_FLAG_DISABLE_UPDATE_CULLING;
                    next->unk_150 = 1; // Why?
                }
