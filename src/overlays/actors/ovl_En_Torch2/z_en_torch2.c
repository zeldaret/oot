...
void EnTorch2_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    sAttackState = sStaggerTimer = sStaggerCount = 0;
    sLastSwordAnim = 0;
    sAlpha = 95;
...
void EnTorch2_Update(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    switch (sActionState) {
        ...
        case ENTORCH2_ATTACK:
            ...
            if ((this->swordQuads[0].base.acFlags & 0x80) || (this->swordQuads[1].base.acFlags & 0x80)) {
                ...
                if ((gSaveContext.health < 0x50) && (sAttackState != 0)) {
                    sAttackState = 0;
                    sStaggerTimer = 50;
                }
            }
            if ((sAttackState != 0) && this->swordState) {
            ...
            if (sDodgeRollState != 0) {
                ...
                if ((player->swordState || (player->actor.velocity.y > -3.0f)) &&
                    ...
                    sStickAngle = (globalCtx->gameplayFrames % 2)
                    ...
                    if (func_800354B4(globalCtx, this, 120.0f, 0x7FFF, 0x7FFF, this->actor.posRot.rot.y)) {
                        if ((player->swordAnimation == 12) && (this->actor.xzDistFromLink < 90.0f)) {
                            if (!this->swordState && (sAttackState == 0) && (player->invincibilityTimer == 0) &&
        ...
        case ENTORCH2_DAMAGE:
            ...
            if ((this->invincibilityTimer > 0) && (this->actor.posRot.pos.y < (this->actor.groundY - 160.0f))) {
                ...
                if (func_8002DB6C(this, &sSpawnPoint) > 800.0f) {
    ...
    sInput.press.in.button = phi_a2 & pad54;
     phi_v0 = phi_a2;
    ...
    sInput.press.in.x = sInput.press.in.x + (sInput.cur.in.x - sInput.prev.in.x);
    sInput.press.in.y = sInput.press.in.y + (sInput.cur.in.y - sInput.prev.in.y);
    ...
    if ((sAttackState != 0) && (sAttackState == 1)) {
        if (!this->swordState) {
            sAttackState = 0;
        } else {
            sAttackState = 2;
...
s32 EnTorch2_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx,
                  Gfx** gfx) {
...
void EnTorch2_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (sAlpha == 255) {
        ...
        oGfxCtx->polyOpa.p = SkelAnime_DrawSV2(globalCtx, this->skelAnime.skeleton, this->skelAnime.limbDrawTbl,
                                               this->skelAnime.dListCount, &EnTorch2_OverrideLimbDraw, &EnTorch2_PostLimbDraw, &this->actor,
                                               oGfxCtx->polyOpa.p);
    } else {
