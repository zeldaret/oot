...
void BgJyaIronobj_SpawnPillarParticles(BgJyaIronobj* this, GlobalContext* globalCtx, EnIk* enIk) {
    ...
    for (j = 0; j < 32; j++) {
        ...
        EffectSsKakera_Spawn(globalCtx, &pos, &vel, &pos, -0x15E, unkArg5, D_80899500[j & 7], 4, 0,
                             D_808994E0[j & 7], 0, 5, D_808994F0[j & 7], -1, 0x16C, gObjectJyaIronDL_000880);
        if (Rand_ZeroOne() < 0.26f) {
            func_80033480(globalCtx, &pos, 200.0f, 1, D_808994E0[j & 7] * 4 + 0x3C, D_808994E0[j & 7] * 4 + 0x50, 1);
        }
    }
}
...
void BgJyaIronobj_SpawnThoneParticles(BgJyaIronobj* this, GlobalContext* arg1, EnIk* enIk) {
    ...
    for (j = 0; j < 32; j++) {
        ...
        vel.y = Rand_ZeroOne() * 8.0f - 3.0f;
        vel.z = 2.0f * (Rand_ZeroOne() * 6.0f - 3.0f) + (Rand_ZeroOne() * coss * 8.0f * temp_f22);
        EffectSsKakera_Spawn(arg1, &pos, &vel, &pos, -0x15E, unkArg5, D_80899530[j & 7], 4, 0, D_80899510[j & 7], 0,
                             5, D_80899520[j & 7], -1, 0x16C, gObjectJyaIronDL_000880);
        if (Rand_ZeroOne() < 0.26f) {
            func_80033480(arg1, &pos, 200.0f, 1, D_80899510[j & 7] * 4 + 0x3C, D_80899510[j & 7] * 4 + 0x50, 1);
        }
    }
}
