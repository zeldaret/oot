...
void func_80A76C14(EnIk* this) {
    if (Animation_OnFrame(&this->skelAnime, 1.0f)) {
        Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_WAKEUP, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    } else if (Animation_OnFrame(&this->skelAnime, 33.0f)) {
        Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_WALK, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    } else if (Animation_OnFrame(&this->skelAnime, 68.0f) || Animation_OnFrame(&this->skelAnime, 80.0f)) {
        Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_ARMOR_DEMO, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    } else if (Animation_OnFrame(&this->skelAnime, 107.0f)) {
        Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_FINGER_DEMO, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    } else if (Animation_OnFrame(&this->skelAnime, 156.0f)) {
        Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_ARMOR_DEMO, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    } else if (Animation_OnFrame(&this->skelAnime, 188.0f)) {
        Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_WAVE_DEMO, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    }
}

void func_80A76DDC(EnIk* this, GlobalContext* globalCtx, Vec3f* pos) {
    Audio_PlaySfxGeneral(NA_SE_EN_TWINROBA_TRANSFORM, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                         &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
}
...
void func_80A772A4(EnIk* this) {
    Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_STAGGER_DEMO, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                         &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
}
...
void func_80A772EC(EnIk* this, GlobalContext* globalCtx) {
    ...
    Audio_PlaySfxGeneral(NA_SE_EN_IRONNACK_DEAD, &D_80A78FA0, 4, &gSfxFreqOrVolDefaultVal, &gSfxFreqOrVolDefaultVal,
                         &gSfxReverbAddNone);
}
