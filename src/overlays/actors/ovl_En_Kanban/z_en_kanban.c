...
void EnKanban_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionState != ENKANBAN_SIGN) {
        ...
        gSPMATRIX_SET_NEW(POLY_OPA_DISP++, play->state.gfxCtx, "../z_en_kanban.c", 1715);
        for (i = 0; i < ARRAY_COUNT(sPartFlags); i++) {
        ...
        gSPMATRIX_SET_NEW(POLY_OPA_DISP++, play->state.gfxCtx, "../z_en_kanban.c", 1725);
        if (this->partFlags == 0xFFFF) {
        ...
        if (this->cutMarkAlpha != 0) {
            ...
            gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_en_kanban.c", 1773);
    ...
    if ((this->actor.projectedPos.z <= 400.0f) && (this->actor.projectedPos.z > 0.0f) &&
        ...
        if ((this->bounceX != 0) || (this->bounceZ != 0)) {
            ...
            gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_en_kanban.c", 1833);
