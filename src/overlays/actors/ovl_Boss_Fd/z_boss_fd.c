...
void func_808CB718(BossFd* this, GlobalContext* globalCtx) {
    ...
    pitchToTarget = (s16)(Math_atan2f(dy, sqrtf(dx * dx + dz * dz)) * 10430.378f);
    ...
    if (this->introState2 != 0) {
        ...
        switch (this->introState2) {
            case 1: // switch
                ...
                if ((fabsf(player2->actor.posRot.pos.z) < 80.0f) &&
                    ...
                    if (gSaveContext.eventChkInf[7] & 8) {
                        ...
                        this->animationTimers[0] = 0;
                        this->holeIndex = 1;
                        holePosition1 = &D_808D192C[(u8)this->holeIndex];
            ...
            case 6: // switch 1
                ...
                if ((this->animationTimers[3] >= 191) && !(gSaveContext.eventChkInf[7] & 8)) {
                    Audio_PlaySoundGeneral(NA_SE_EN_DODO_K_ROLL - SFX_FLAG, &this->actor.projectedPos, 4, &D_801333E0,
                                           &D_801333E0, &D_801333E8);
                }
                ...
                if ((this->animationTimers[3] == 130) && !(gSaveContext.eventChkInf[7] & 8)) {
                    ...
                    switch (this->introState1) {
                        ...
                        case 2:
                            ...
                            if (this->animationTimers[5] == 0) {
                                ...
                                this->holeIndex = 7;
                                this->targetPosition.x = D_808D192C[this->holeIndex].x;
                                this->targetPosition.y = (D_808D192C[this->holeIndex].y + 200.0f) + 50.0f;
                                this->targetPosition.z = D_808D192C[this->holeIndex].z;
                                this->introState1 = 3;
                            }
    ...
    switch (this->actionState) {
        case 0: // Cases 0 - 3 deal with flying in and out of holes
            ...
            break;
        case 1:
            if ((this->animationTimers[0] == 0) && (sqrtf(dx * dx + dy * dy + dz * dz) < 100.0f)) {
                this->actionState = 2;
        ...
        case 50: // Flying up to the ceiling to drop rocks
            this->flightSpeed = 8;
            this->targetPosition.x = 0.0f; // float regalloc problems
        ...
        case 51: // Flying around the ceiling dropping rocks
            ...
            this->flightWobbleAmplitude = 200.0f;
            this->maxTurnRate = 10000.0f;
            this->targetPosition.x = D_808D1998[this->ceilingTargetIndex].x;
            this->targetPosition.y = D_808D1998[this->ceilingTargetIndex].y + 900.0f;
            this->targetPosition.z = D_808D1998[this->ceilingTargetIndex].z;
            if (this->animationTimers[0] == 0) {
                this->animationTimers[0] = 25;
        ...
        case 100: // Chasing Link and breathing fire at him
            ...
            this->flightWobbleAmplitude = 0.0f;
            if (!(this->animationTimers[0] & 0x3F) && (this->animationTimers[0] < 450)) {
                this->roarTimer = 40;
                if (func_808CB67C(this)) {
                    this->fireBreathTimer = 20;
                }
            }
        ...
        case 200: // Start of death cutscene
            if (sqrtf(dx * dx + dz * dz) < 50.0f) {
                this->animationTimers[0] = 0;
            }
            if (this->animationTimers[0] == 0) {
                this->animationTimers[0] = (s16)Math_Rand_ZeroFloat(10.0f) + 10;
                do {
                ...
                } while (!(sqrtf((temp_x * temp_x) + (temp_z * temp_z)) > 100.0f));
            }
            this->flightWobbleAmplitude = 200.0f;
        ...
        case 201: // Skin burning off scene
            ...
            Math_SmoothScaleMaxF(&this->bodyPulse, 0.3f, 1.0f, 0.05f);
            if ((this->animationTimers[0] == 0) && ((this->movementTimer % 4) == 0)) {
                ...
                if (this->actor.posRot.pos.y <= 110.0f) {
                    ...
                    if (this->ceilingTargetIndex == 0) {
                        ...
                        for (i1 = 0; i1 < 15; i1++) {
                            ...
                            func_808CAF5C(this->particles, &sp12C, &sp144, &sp138, Math_Rand_ZeroFloat(100.0f) + 300);
                        }
                    }
                } else {
    ...
    if (!this->stopFlag) {
        ...
        this->bodySegsRot[i4].x = (this->actor.posRot.rot.x / 32768.0f) * M_PI;
        this->bodySegsRot[i4].y = (this->actor.posRot.rot.y / 32768.0f) * M_PI;
        this->bodySegsRot[i4].z = (this->actor.posRot.rot.z / 32768.0f) * M_PI;

        this->leadManeSeg++;
        if (this->leadManeSeg >= 30) {
            this->leadManeSeg = 0;
        }
        i4 = this->leadManeSeg; // s0
        ...
        this->fireManeRot[i4].x = (this->actor.posRot.rot.x / 32768.0f) * M_PI;
        this->fireManeRot[i4].y = (this->actor.posRot.rot.y / 32768.0f) * M_PI;
        this->fireManeRot[i4].z = (this->actor.posRot.rot.z / 32768.0f) * M_PI;
...
void func_808CE01C(BossFd* this, GlobalContext* globalCtx) {
    ...
    if (this->roarTimer != 0) {
        ...
        phi_f0_2 = (this->varianceTimer & 0x10) ? 0.0f : 1000.0f;
        phi_f2 = 500.0f;
    }
    ...
    if (sp106 != 0) {
        ...
        spAC = (this->actor.posRot.rot.y / 32768.0f) * M_PI;
        spB0 = (((-this->actor.posRot.rot.x) / 32768.0f) * M_PI) + 0.3f;
...
static Color_RGBA8_n D_808D1A58 = { 255, 255, 0, 255 };
static Color_RGBA8_n D_808D1A5C = { 255, 10, 0, 255 };
...
void BossFd_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    phi_f2 = (this->varianceTimer & 0x10) ? 30.0f : 158.0f;
    Math_SmoothScaleMaxF(&this->bodyTex2Opacity, phi_f2, 1.0f, 8.0f);
    if (this->skinSegments == 0) {
        this->headTex2Opacity = this->bodyTex2Opacity;
    } else {
        phi_f0_1 = (this->varianceTimer & 4) ? 0.0f : 255.0f;
        Math_SmoothScaleMaxF(&this->headTex2Opacity, phi_f0_1, 1.0f, 64.0f);
    }

    phi_f0_3 = (this->varianceTimer & 8) ? 128.0f : 255.0f;
    phi_f22 = ((this->varianceTimer + 3) & 8) ? 128.0f : 255.0f;
    phi_f20 = ((this->varianceTimer + 6) & 8) ? 128.0f : 255.0f;
...
void func_808CF448(BossFd* this, GlobalContext* globalCtx) {
    ...
    Player* player = PLAYER;
    Color_RGB8 sp8C[4] = { { 255, 128, 0 }, { 255, 0, 0 }, { 255, 255, 0 }, { 255, 0, 0} };
    Vec3f diff;
    s16 cInd;
    s16 i1;
    s16 i2;
        ...
        if (part->type != 0) {
            ...
            if (part->type == 1) {
                ...
                if (part->timer2 >= 8) {
                    part->timer2 = 8;
                    part->type = 0;
                } else if ((part->timer1 & 1) || (Math_Rand_ZeroOne() < 0.3f)) {
                    part->timer2++;
                }
            } else if (part->type == 4) {
...
void func_808CF898(BossFdParticle* ptr, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 180; i++, ptr++) {
        if (ptr->type == 4) {
            ...
            gDPSetPrimColor(oGfxCtx->polyXlu.p++, 0, 0, 255, 255, 0, ptr->opacity);
            Matrix_Translate(ptr->pos.x, ptr->pos.y, ptr->pos.z, 0);
            Matrix_Scale(ptr->scale, ptr->scale, ptr->scale, 1);
            func_800D1FD4(&globalCtx->mf_11DA0);
...
void func_808D08F8(GlobalContext* globalCtx, BossFd* this) {
    ...
    osSyncPrintf("SK\n");
    {
        ...
        gDPSetPrimColor(oGfxCtx->polyXlu.p++, 0, 0, 255, this->centerManeColor, 0x00, 255);
        Matrix_Push();
        ...
        Matrix_Pull();
        gDPSetPrimColor(oGfxCtx->polyXlu.p++, 0, 0, 255, this->rightManeColor, 0x00, 255);
        Matrix_Push();
        ...
        Matrix_Pull();
        gDPSetPrimColor(oGfxCtx->polyXlu.p++, 0, 0, 255, this->leftManeColor, 0x00, 255);
        Matrix_Push();
        spA4.x *= -1.0f;
