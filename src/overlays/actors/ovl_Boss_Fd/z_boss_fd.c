...
void func_808D02DC(GlobalContext* globalCtx, BossFd* this, Vec3f* arg2, Vec3f* arg3, f32* arg4,
                   u8 mode); // Draw Mane
...
#ifdef NON_MATCHING
//Obscenely long. See file for details.
...
void func_808CF898(BossFdParticle *particle, GlobalContext *globalCtx) {
    ...
    for (i = 0; i < 180; i++, ptr++) {
        if (ptr->unk_24 == 1) {
            ...
            gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(gfxCtx, "../z_boss_fd.c", 0xFCE), G_MTX_NOPUSH | G_MTX_LOAD |
G_MTX_MODELVIEW); gSPDisplayList(gfxCtx->polyXlu.p++, D_0600A900);
        }
    }
    ...
    for (i = 0; i < 180; i++, ptr++) {
        if (ptr->unk_24 == 2) {
            ...
            gSPMatrix(gfxCtx->polyOpa.p++, Matrix_NewMtx(gfxCtx, "../z_boss_fd.c", 0xFE4), G_MTX_NOPUSH | G_MTX_LOAD |
G_MTX_MODELVIEW); gSPDisplayList(gfxCtx->polyOpa.p++, D_0600D420);
        }
    }
    ...
    for (i = 0; i < 180; i++, ptr++) {
        if (ptr->unk_24 == 3) {
            ...
            gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(gfxCtx, "../z_boss_fd.c", 0x1008), G_MTX_NOPUSH | G_MTX_LOAD |
G_MTX_MODELVIEW); gSPSegment(gfxCtx->polyXlu.p++, 0x08, SEGMENTED_TO_VIRTUAL(D_808D1A9C[ptr->unk_2C]));
    ...
    for (i = 0; i < 180; i++, ptr++) {
        if (ptr->unk_24 == 4) {
            ...
            gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(gfxCtx, "../z_boss_fd.c", 0x103A), G_MTX_NOPUSH | G_MTX_LOAD |
G_MTX_MODELVIEW); gSPSegment(gfxCtx->polyXlu.p++, 0x08, SEGMENTED_TO_VIRTUAL(D_808D1A9C[ptr->unk_2C]));
            gSPDisplayList(gfxCtx->polyXlu.p++, D_0600B3C8);
        }
    }
    ...
    for (i = 0; i < 180; i++, ptr++) {
        if (ptr->unk_24 == 5) {
            ...
            gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(gfxCtx, "../z_boss_fd.c", 0x1060), G_MTX_NOPUSH | G_MTX_LOAD |
G_MTX_MODELVIEW); gSPDisplayList(gfxCtx->polyXlu.p++, D_0600D6E8);
        }
    }
