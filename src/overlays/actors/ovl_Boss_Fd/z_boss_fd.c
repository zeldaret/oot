        ...
        case BOSSFD_DROP_ROCKS:
            ...
            if (this->timers[1] == 0) {
                ...
                if (this->actor.bgCheckFlags & BGCHECKFLAG_CEILING) {
                    ...
                    this->timers[1] = 10;
                    Audio_PlaySfxGeneral(NA_SE_EV_EXPLOSION, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
...
void BossFd_Effects(BossFd* this, GlobalContext* globalCtx) {
    ...
    if (this->work[BFD_ROAR_TIMER] != 0) {
        if (this->work[BFD_ROAR_TIMER] == 37) {
            Audio_PlaySfxGeneral(NA_SE_EN_VALVAISA_ROAR, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
        }
        jawAngle = 6000.0f;
        jawSpeed = 1300.0f;
    ...
    if (breathOpacity != 0) {
        ...
        Audio_PlaySfxGeneral(NA_SE_EN_VALVAISA_FIRE - SFX_FLAG, &sFireAudioVec, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
...
void BossFd_CollisionCheck(BossFd* this, GlobalContext* globalCtx) {
    ...
    if (headCollider->info.bumperFlags & BUMP_HIT) {
        ...
        Audio_PlaySfxGeneral(NA_SE_EN_VALVAISA_DAMAGE1, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
    }
}
