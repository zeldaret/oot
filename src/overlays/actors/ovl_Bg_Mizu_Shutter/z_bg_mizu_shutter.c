...
void BgMizuShutter_Init(BgMizuShutter* thisx, GlobalContext* globalCtx) {
    ...
    if ((SIZE_PARAM == BGMIZUSHUTTER_SMALL) || (SIZE_PARAM == BGMIZUSHUTTER_LARGE)) {
        ...
        Matrix_RotateY(BINANG_TO_RAD_ALT(this->dyna.actor.world.rot.y), MTXMODE_NEW);
        Matrix_RotateX(BINANG_TO_RAD_ALT(this->dyna.actor.world.rot.x), MTXMODE_APPLY);
        Matrix_RotateZ(BINANG_TO_RAD_ALT(this->dyna.actor.world.rot.z), MTXMODE_APPLY);
