...
void EnNb_SetupRaisedArmTransition(EnNb* this, s32 animFinished) {
    ...
    if (animFinished) {
        SkelAnime_ChangeAnim(&this->skelAnime, animation, 1.0f, 0.0f, SkelAnime_GetLastFrame(animation), 0, 0.0f);
...
void EnNb_SetupKidnap(EnNb* this) {
    ...
    SkelAnime_ChangeAnim(&this->skelAnime, animation, 1.0f, 0.0f, SkelAnime_GetLastFrame(animation), 2, -8.0f);
...
void func_80AB26DC(EnNb* this, GlobalContext* globalCtx) {
    s32 pad;
    AnimationHeader* animation = &D_06008BD0;
    f32 frames = SkelAnime_GetLastFrame(animation);
...
void func_80AB2FE4(EnNb* this, GlobalContext* globalCtx) {
    ...
    gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 0, 255);
    gSPSegment(POLY_OPA_DISP++, 0x0C, &D_80116280[2]);
    SkelAnime_DrawFlexOpa(globalCtx, skelAnime->skeleton, skelAnime->jointTbl, skelAnime->dListCount, func_80AB2FC0,
                          NULL, &this->actor);
...
void EnNb_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_InitFlex(globalCtx, &this->skelAnime, &D_060181C8, NULL, this->jointTbl, this->morphTbl, 19);
