...
u16 EnGo_GetTextID(PlayState* play, Actor* thisx) {
    ...
    switch (PARAMS_GET_NOSHIFT(thisx->params, 4, 0xF)) {
        case 0x90:
...
void EnGo_ChangeAnim(EnGo* this, s32 index) {
    Animation_Change(&this->skelAnime, sAnimationInfo[index].animation,
                     sAnimationInfo[index].playSpeed * (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90 ? 0.5f : 1.0f), 0.0f,
...
s32 EnGo_IsActorSpawned(EnGo* this, PlayState* play) {
    if (((this->actor.params) & 0xF0) == 0x90) {
        return true;
               ...
               PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x10) {
        return true;
    } else if (play->sceneNum == SCENE_SPOT18 && LINK_IS_ADULT && PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x00) {
        return true;
    } else if (play->sceneNum == SCENE_SPOT16 && LINK_IS_CHILD &&
               (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x20 || PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x30 ||
                PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x40)) {
        return true;
    } else if (play->sceneNum == SCENE_SPOT18 && LINK_IS_CHILD &&
               (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x50 || PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x60 ||
                PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x70)) {
        return true;
    } else {
        return false;
    }
...
f32 EnGo_GetGoronSize(EnGo* this) {
    switch (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF)) {
        case 0x00:
            return 10.0f;
        case 0x20:
        case 0x30:
...
s32 EnGo_IsCameraModified(EnGo* this, PlayState* play) {
    ...
    if (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90) {
...
s32 EnGo_FollowPath(EnGo* this, PlayState* play) {
    ...
    if (PARAMS_GET(this->actor.params, 0, 0xF) == 15) {
        return false;
    }

    path = &play->setupPathList[PARAMS_GET(this->actor.params, 0, 0xF)];
    ...
    if ((SQ(xDist) + SQ(zDist)) < 600.0f) {
        ...
        if (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) != 0x00) {
            return true;
...
s32 EnGo_SetMovedPos(EnGo* this, PlayState* play) {
    ...
    if (PARAMS_GET(this->actor.params, 0, 0xF) == 0xF) {
        return false;
    } else {
        path = &play->setupPathList[PARAMS_GET(this->actor.params, 0, 0xF)];
        pointPos = SEGMENTED_TO_VIRTUAL(path->points);
...
void func_80A3F908(EnGo* this, PlayState* play) {
    ...
    if (this->actionFunc == EnGo_BiggoronActionFunc || this->actionFunc == EnGo_GoronLinkRolling ||
        ...
        if ((PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90) && (isUnkCondition == true)) {
...
void EnGo_Init(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) && (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) != 0x90)) {
...
void EnGo_CurledUp(EnGo* this, PlayState* play) {
    if ((DECR(this->unk_210) == 0) && EnGo_IsCameraModified(this, play)) {
        ...
        this->skelAnime.playSpeed *= PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90 ? 0.5f : 1.0f;
...
void EnGo_WakeUp(EnGo* this, PlayState* play) {
    ...
    if (this->skelAnime.playSpeed != 0.0f) {
        Math_SmoothStepToF(&this->skelAnime.playSpeed, (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90 ? 0.5f : 1.0f) * 0.5f, 0.1f,
                           1000.0f, 0.1f);
...
void func_80A405CC(EnGo* this, PlayState* play) {
    ...
    Math_SmoothStepToF(&this->skelAnime.playSpeed, PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90 ? 0.5f : 1.0f, 0.1f, 1000.0f,
                       0.1f);
    ...
    if (!(frame < lastFrame)) {
        ...
        if ((PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x40) && !GET_INFTABLE(INFTABLE_EB)) {
...
void EnGo_BiggoronActionFunc(EnGo* this, PlayState* play) {
    if ((PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90) && (this->unk_1E0.unk_00 == 2)) {
        if (gSaveContext.bgsFlag) {
            this->unk_1E0.unk_00 = 0;
        } else {
            if (INV_CONTENT(ITEM_TRADE_ADULT) == ITEM_EYEDROPS) {
    ...
    } else if ((PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0) && (this->unk_1E0.unk_00 == 2)) {
        EnGo_SetupAction(this, EnGo_GetItem);
...
void func_80A408D8(EnGo* this, PlayState* play) {
    ...
    if (this->skelAnime.playSpeed != 0.0f) {
        Math_SmoothStepToF(&this->skelAnime.playSpeed, (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0x90 ? 0.5f : 1.0f) * -1.0f,
                           0.1f, 1000.0f, 0.1f);
        frame = this->skelAnime.curFrame;
        frame += this->skelAnime.playSpeed;
        if (frame >= 12.0f) {
...
void EnGo_GetItem(EnGo* this, PlayState* play) {
    ...
    if (Actor_HasParent(&this->actor, play)) {
        ...
        if (PARAMS_GET_NOSHIFT(this->actor.params, 4, 0xF) == 0) {
