...
void EnGo_DrawCurledUp(EnGo* this, PlayState* play) {
    ...
    gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_go.c", 2326), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnGo_DrawRolling(EnGo* this, PlayState* play) {
    ...
    Matrix_RotateZYX((s16)(play->state.frames * ((s16)this->actor.speed * 1400)), 0, this->actor.shape.rot.z,
                     MTXMODE_APPLY);
    gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_go.c", 2368), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnGo_DrawEffects(EnGo* this, PlayState* play) {
    ...
    for (i = 0; i < EN_GO_EFFECT_COUNT; i++, dustEffect++) {
        ...
        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_go.c", 2664), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
