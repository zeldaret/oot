    ...
    switch (PARAMS_GET_NOSHIFT(thisx->params, 4, 4)) {
        ...
        case 0x20:
            if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
                return 0x3027;
            } else if (GET_EVENTCHKINF(EVENTCHKINF_BOMBED_DODONGOS_CAVERN_ENTRANCE)) {
                return 0x3021;
            } else if (GET_INFTABLE(INFTABLE_SPOKE_TO_DODONGOS_CAVERN_ENTRANCE_GORON)) {
                return 0x302A;
            } else {
                return 0x3008;
            }
        ...
        case 0x50:
            if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
                return 0x3027;
            } else if (GET_INFTABLE(INFTABLE_SPOKE_TO_GC_ENTRANCE_GORON)) {
                return 0x3015;
            } else {
                return 0x3014;
            }
        case 0x60:
            if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
                return 0x3027;
            } else if (GET_INFTABLE(INFTABLE_SPOKE_TO_GC_ISLAND_GORON)) {
                return 0x3017;
            } else {
                return 0x3016;
            }
        case 0x70:
            if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
                return 0x3027;
            } else if (GET_INFTABLE(INFTABLE_SPOKE_TO_DARUNIA_DOOR_GORON)) {
                return 0x3019;
            } else {
                return 0x3018;
            }
                ...
                switch (thisx->textId) {
                    case 0x300A:
                        if (play->msgCtx.choiceIndex == 0) {
                            if (CUR_UPG_VALUE(UPG_STRENGTH) != 0 || GET_INFTABLE(INFTABLE_SPOKE_TO_DMT_BOMB_FLOWER_GORON)) {
                                thisx->textId = 0x300B;
                            } else {
                                thisx->textId = 0x300C;
                            }
                    ...
                    case 0x3034:
                        if (play->msgCtx.choiceIndex == 0) {
                            if (GET_INFTABLE(INFTABLE_SPOKE_TO_GORON_LINK_ABOUT_DRAGON)) {
                                thisx->textId = 0x3033;
                            } else {
                                thisx->textId = 0x3035;
                            }
                        } else if (GET_INFTABLE(INFTABLE_SPOKE_TO_GORON_LINK_ABOUT_DRAGON)) {
                            thisx->textId = 0x3036;
                        } else {
                            thisx->textId = 0x3033;
                        }
...
void func_80A405CC(EnGo* this, PlayState* play) {
    ...
    if (!(frame < lastFrame)) {
        ...
        if ((PARAMS_GET_NOSHIFT(this->actor.params, 4, 4) == 0x40) && !GET_INFTABLE(INFTABLE_SPOKE_TO_DMT_BOMB_FLOWER_GORON)) {
