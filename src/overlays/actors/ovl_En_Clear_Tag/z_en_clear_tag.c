...
void EnClearTag_Update(Actor* thisx, GlobalContext* globalCtx2) {
            ...
            case CLEAR_TAG_STATE_LASER:
                ...
                if (this->actor.bgCheckFlags & 9 || hasAtHit || this->timers[CLEAR_TAG_TIMER_LASER_DEATH] == 0) {
                    ...
                    if (this->timers[CLEAR_TAG_TIMER_LASER_DEATH] != 0) {
                        AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 20, NA_SE_EN_FANTOM_THUNDER_GND);
                    }
                }
                break;
        }
    ...
    if (this->shouldExplode) {
        ...
        AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 40, NA_SE_IT_BOMB_EXPLOSION);
