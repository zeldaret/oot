...
void EnChanger_OpenChests(EnChanger* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        unopenedChestItemType = openedChestSide = this->chestOpened; // Required to use the right registers

        switch (openedChestSide) {
            case CHEST_LEFT:
                ...
                if (this->rightChestGetItemId == GI_DOOR_KEY) {
                    Actor_Spawn(&play->actorCtx, play, ACTOR_EN_EX_ITEM, xPos, yPos, zPos, 0, 0, 0, EXITEM_SMALL_KEY_CHEST);
                    Flags_SetSwitch(play, 0x32);
                } else {
                    unopenedChestItemType = (s16)(this->rightChestGetItemId - GI_RUPEE_GREEN_LOSE) + EXITEM_GREEN_RUPEE_CHEST;
                    // "Open right treasure (chest)"
                    PRINTF(VT_FGCOL(GREEN) "☆☆☆☆☆ 右宝開く ☆☆☆☆☆ %d\n" VT_RST, unopenedChestItemType);
                    Actor_Spawn(&play->actorCtx, play, ACTOR_EN_EX_ITEM, xPos, yPos, zPos, 0, 0, 0, unopenedChestItemType);
                }
                break;
            case CHEST_RIGHT:
                ...
                if (this->leftChestGetItemId == GI_DOOR_KEY) {
                    ...
                    unopenedChestItemType = (s16)(this->leftChestGetItemId - GI_RUPEE_GREEN_LOSE) + EXITEM_GREEN_RUPEE_CHEST;
