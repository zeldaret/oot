...
void func_80AB6F04(EnNiw* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_WATER) {
        ...
        if (this->actor.yDistUnderWater > 15.0f) {
...
void EnNiw_Update(Actor* thisx, PlayState* play) {
    ...
    if ((thisx->bgCheckFlags & BGCHECKFLAG_WATER) && thisx->yDistUnderWater > 15.0f &&
        this->actionFunc != func_80AB6F04 && thisx->params != 0xD && thisx->params != 0xE && thisx->params != 0xA) {
        thisx->velocity.y = 0.0f;
        thisx->gravity = 0.0f;
        Math_Vec3f_Copy(&pos, &thisx->world.pos);
        pos.y += thisx->yDistUnderWater;
        this->timer4 = 30;
