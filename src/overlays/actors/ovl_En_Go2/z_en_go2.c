...
u16 EnGo2_GetTextIdGoronCityLink(GlobalContext* globalCtx, EnGo2* this) {
    ...
    } else if (CHECK_OWNED_EQUIP(EQUIP_TUNIC, INV_EQUIP_TUNIC_GORON)) {
...
s32 EnGo2_IsCameraModified(EnGo2* this, GlobalContext* globalCtx) {
    ...
    if ((this->actor.params & 0x1F) == GORON_FIRE_GENERIC || (this->actor.params & 0x1F) == GORON_CITY_ROLLING_BIG ||
        ...
        return true;
    } else if (!CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && CHECK_OWNED_EQUIP(EQUIP_TUNIC, INV_EQUIP_TUNIC_GORON)) {
        return true;
    } else {
        return false;
    }
...
void EnGo2_SelectGoronWakingUp(EnGo2* this) {
    switch (this->actor.params & 0x1F) {
        ...
        case GORON_DMT_BIGGORON:
            ...
            break;
        case GORON_CITY_LINK:
            if (!CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && CHECK_OWNED_EQUIP(EQUIP_TUNIC, INV_EQUIP_TUNIC_GORON)) {
                EnGo2_WakingUp(this);
                break;
            }
        default:
