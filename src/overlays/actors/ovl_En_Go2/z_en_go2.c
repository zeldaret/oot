...
u16 EnGo2_GetTextIdGoronCityRollingBig(PlayState* play, EnGo2* this) {
    if (GET_INFTABLE(INFTABLE_UPGRADED_BOMB_BAG)) {
        return 0x3013;
        ...
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x3012) {
                    ...
                    SET_INFTABLE(INFTABLE_UPGRADED_BOMB_BAG);
                    return NPC_TALK_STATE_ACTION;
                } else {
                    return NPC_TALK_STATE_ACTION;
                }
...
s16 EnGo2_UpdateTalkStateGoronDmtBombFlower(PlayState* play, EnGo2* this) {
    switch (Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_CLOSING:
            if ((this->actor.textId == 0x300B) && !GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_300B)) {
                SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_300B);
                return NPC_TALK_STATE_ACTION;
            } else {
...
u16 EnGo2_GetTextIdGoronDmtDcEntrance(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return GET_EVENTCHKINF(EVENTCHKINF_DC_BOULDER_DESTROYED) ? 0x3021
               : GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3008) ? 0x302A
                                                                 : 0x3008;
    }
}
...
s16 EnGo2_UpdateTalkStateGoronDmtDcEntrance(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3008) {
            SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3008);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityEntrance(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3014) ? 0x3015 : 0x3014;
    }
}
...
s16 EnGo2_UpdateTalkStateGoronCityEntrance(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3014) {
            SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3014);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityIsland(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3016) ? 0x3017 : 0x3016;
    }
}
...
s16 EnGo2_UpdateTalkStateGoronCityIsland(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3016) {
            SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3016);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityLowestFloor(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return CUR_UPG_VALUE(UPG_STRENGTH) != 0                  ? 0x302C
               ...
               : GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3018) ? 0x3019
                                                                 : 0x3018;
    }
}
...
u16 EnGo2_GetTextIdGoronCityLink(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) {
        return GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3041) ? 0x3042 : 0x3041;
    } else if (CHECK_OWNED_EQUIP(EQUIP_TYPE_TUNIC, EQUIP_INV_TUNIC_GORON)) {
        return GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3037) ? 0x3038 : 0x3037;
    } else if (GET_INFTABLE(INFTABLE_STOPPED_GORON_LINK)) {
        this->unk_20C = 0;
        this->dialogState = TEXT_STATE_NONE;
        return GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3032) ? 0x3033 : 0x3032;
    } else {
        return 0x3030;
    }
}
    ...
    switch (EnGo2_GetDialogState(this, play)) {
        case TEXT_STATE_CLOSING:
            switch (this->actor.textId) {
                case 0x3036:
                    ...
                    return NPC_TALK_STATE_ACTION;
                case 0x3037:
                    SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3037);
                    FALLTHROUGH;
                default:
                    return NPC_TALK_STATE_IDLE;
            }
        case TEXT_STATE_CHOICE:
            if (Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x3034) {
                    if (play->msgCtx.choiceIndex == 0) {
                        this->actor.textId = GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3035) ? 0x3033 : 0x3035;
                        if (this->actor.textId == 0x3035) {
                            Audio_StopSfxById(NA_SE_EN_GOLON_CRY);
                        }
                    } else {
                        this->actor.textId = GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3035) ? 0x3036 : 0x3033;
        ...
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(play)) {
                switch (this->actor.textId) {
                    case 0x3035:
                        SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3035);
                        FALLTHROUGH;
                    case 0x3032:
                    case 0x3033:
...
s16 EnGo2_UpdateTalkStateGoronCityStairwell(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x300E) {
            SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_300E);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityLostWoods(PlayState* play, EnGo2* this) {
    if (!LINK_IS_ADULT) {
        if (Flags_GetSwitch(play, 0x1C)) {
            ...
            return GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3024) ? 0x3025 : 0x3024;
        }
    } else {
        return 0x3043;
    }
...
s16 EnGo2_UpdateTalkStateGoronCityLostWoods(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3024) {
            SET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3024);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
    ...
    if (PARAMS_GET_S(this->actor.params, 0, 5) == GORON_DMT_BIGGORON) {
        if (gSaveContext.save.info.playerData.bgsFlag) {
            ...
            if (func_8002F368(play) == EXCH_ITEM_BROKEN_GORONS_SWORD) {
                if (GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_3054)) {
                    textId = 0x3055;
                } else {
                    textId = 0x3054;
                }
...
void EnGo2_SetupRolling(EnGo2* this, PlayState* play) {
    if (PARAMS_GET_S(this->actor.params, 0, 5) == GORON_CITY_ROLLING_BIG ||
        ...
        this->actor.speed = GET_INFTABLE(INFTABLE_UPGRADED_BOMB_BAG) ? 6.0f : 3.6000001f;
    } else {
...
void EnGo2_Init(Actor* thisx, PlayState* play) {
    ...
    switch (PARAMS_GET_S(this->actor.params, 0, 5)) {
        ...
        case GORON_DMT_BIGGORON:
            ...
            break;
        case GORON_DMT_BOMB_FLOWER:
            if (GET_INFTABLE(INFTABLE_TALKED_TO_GORON_MSG_300B)) {
                Path_CopyLastPoint(this->path, &this->actor.world.pos);
                this->actor.home.pos = this->actor.world.pos;
            }
            FALLTHROUGH;
