...
u16 EnGo2_GetTextIdGoronDmtBiggoron(PlayState* play, EnGo2* this) {
    ...
    if (gSaveContext.bgsFlag) {
        ...
        player->exchangeItemId = EXCH_ITEM_BROKEN_BGS;
        return 0x3053;
    }
}
...
void EnGo2_BiggoronSetTextId(EnGo2* this, PlayState* play, Player* player) {
    ...
    if ((this->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        ...
        } else if (INV_CONTENT(ITEM_TRADE_ADULT) <= ITEM_BROKEN_BGS) {
            if (func_8002F368(play) == EXCH_ITEM_BROKEN_BGS) {
...
void func_80A454CC(EnGo2* this) {
    switch (this->actor.params & 0x1F) {
        ...
        case GORON_DMT_FAIRY_HINT:
            ...
            break;
        case GORON_DMT_BIGGORON:
            if (INV_CONTENT(ITEM_TRADE_ADULT) >= ITEM_BROKEN_BGS && INV_CONTENT(ITEM_TRADE_ADULT) <= ITEM_EYE_DROPS) {
...
void EnGo2_Init(Actor* thisx, PlayState* play) {
    ...
    switch (this->actor.params & 0x1F) {
        ...
        case GORON_DMT_BIGGORON:
            ...
            this->actor.flags &= ~ACTOR_FLAG_0;
            if ((INV_CONTENT(ITEM_TRADE_ADULT) >= ITEM_BROKEN_BGS) &&
