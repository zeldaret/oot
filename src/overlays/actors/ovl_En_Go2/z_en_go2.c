...
EVENTCHKINF_BOMBED_DODONGOS_CAVERN_ENTRANCE - DC entrance boulder blown up as child

InfTable

INFTABLE_B4
INFTABLE_SPOKE_TO_DODONGOS_CAVERN_ENTRANCE_GORON - Talked to DMT Goron at DC entrance (Before DC is opened as child)
INFTABLE_SPOKE_TO_GC_STAIRWELL_GORON - Talked to GC Goron in bottom level stairwell
INFTABLE_SPOKE_TO_LOST_WOODS_SHORTCUT_GORON - Talked to GC Goron at LW entrance (Before LW shortcut is opened)
INFTABLE_SPOKE_TO_DMT_BOMB_FLOWER_GORON - Talked to DMT Goron at Bomb Flower with goron bracelet
INFTABLE_SPOKE_TO_GC_ENTRANCE_GORON - Talked to Goron at GC Entrance (Before goron ruby is obtained)
...
INFTABLE_ACQUIRED_BOMB_BAG_UPGRADE_FROM_HOT_RODDER_GORON - Bomb bag upgrade obtained from rolling Goron
...
u16 EnGo2_GetTextIdGoronCityRollingBig(PlayState* play, EnGo2* this) {
    if (GET_INFTABLE(INFTABLE_ACQUIRED_BOMB_BAG_UPGRADE_FROM_HOT_RODDER_GORON)) {
        return 0x3013;
        ...
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x3012) {
                    ...
                    SET_INFTABLE(INFTABLE_ACQUIRED_BOMB_BAG_UPGRADE_FROM_HOT_RODDER_GORON);
                    return NPC_TALK_STATE_ACTION;
                } else {
                    return NPC_TALK_STATE_ACTION;
                }
...
s16 EnGo2_UpdateTalkStateGoronDmtBombFlower(PlayState* play, EnGo2* this) {
    switch (Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_CLOSING:
            if ((this->actor.textId == 0x300B) && !GET_INFTABLE(INFTABLE_SPOKE_TO_DMT_BOMB_FLOWER_GORON)) {
                SET_INFTABLE(INFTABLE_SPOKE_TO_DMT_BOMB_FLOWER_GORON);
                return NPC_TALK_STATE_ACTION;
            } else {
...
u16 EnGo2_GetTextIdGoronDmtRollingSmall(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
        ...
        return GET_EVENTCHKINF(EVENTCHKINF_BOMBED_DODONGOS_CAVERN_ENTRANCE) ? 0x3026 : 0x3009;
    }
}
...
u16 EnGo2_GetTextIdGoronDmtDcEntrance(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return GET_EVENTCHKINF(EVENTCHKINF_BOMBED_DODONGOS_CAVERN_ENTRANCE) ? 0x3021 : GET_INFTABLE(INFTABLE_SPOKE_TO_DODONGOS_CAVERN_ENTRANCE_GORON) ? 0x302A : 0x3008;
    }
}
...
s16 EnGo2_UpdateTalkStateGoronDmtDcEntrance(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3008) {
            SET_INFTABLE(INFTABLE_SPOKE_TO_DODONGOS_CAVERN_ENTRANCE_GORON);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityEntrance(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return GET_INFTABLE(INFTABLE_SPOKE_TO_GC_ENTRANCE_GORON) ? 0x3015 : 0x3014;
    }
}
...
s16 EnGo2_UpdateTalkStateGoronCityEntrance(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3014) {
            SET_INFTABLE(INFTABLE_SPOKE_TO_GC_ENTRANCE_GORON);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityIsland(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return GET_INFTABLE(INFTABLE_SPOKE_TO_GC_ISLAND_GORON) ? 0x3017 : 0x3016;
    }
}
...
s16 EnGo2_UpdateTalkStateGoronCityIsland(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3016) {
            SET_INFTABLE(INFTABLE_SPOKE_TO_GC_ISLAND_GORON);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityLowestFloor(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        ...
        return CUR_UPG_VALUE(UPG_STRENGTH) != 0 ? 0x302C
               ...
               : GET_INFTABLE(INFTABLE_SPOKE_TO_DARUNIA_DOOR_GORON)      ? 0x3019
                                                : 0x3018;
    }
}
...
s16 EnGo2_UpdateTalkStateGoronCityLowestFloor(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3018) {
            SET_INFTABLE(INFTABLE_SPOKE_TO_DARUNIA_DOOR_GORON);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityLink(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) {
        ...
        return GET_INFTABLE(INFTABLE_SPOKE_TO_GORON_LINK_ABOUT_GORONS) ? 0x3038 : 0x3037;
    } else if (GET_INFTABLE(INFTABLE_STOPPED_GORON_LINKS_ROLLING)) {
        this->unk_20C = 0;
    ...
    switch (EnGo2_GetDialogState(this, play)) {
        case TEXT_STATE_CLOSING:
            switch (this->actor.textId) {
                case 0x3036:
                    ...
                    return NPC_TALK_STATE_ACTION;
                case 0x3037:
                    SET_INFTABLE(INFTABLE_SPOKE_TO_GORON_LINK_ABOUT_GORONS);
                    FALLTHROUGH;
                default:
                    return NPC_TALK_STATE_IDLE;
            }
        case TEXT_STATE_CHOICE:
            if (Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x3034) {
                    if (play->msgCtx.choiceIndex == 0) {
                        this->actor.textId = GET_INFTABLE(INFTABLE_SPOKE_TO_GORON_LINK_ABOUT_DRAGON) ? 0x3033 : 0x3035;
                        if (this->actor.textId == 0x3035) {
                            Audio_StopSfxById(NA_SE_EN_GOLON_CRY);
                        }
                    } else {
                        this->actor.textId = GET_INFTABLE(INFTABLE_SPOKE_TO_GORON_LINK_ABOUT_DRAGON) ? 0x3036 : 0x3033;
        ...
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(play)) {
                switch (this->actor.textId) {
                    case 0x3035:
                        SET_INFTABLE(INFTABLE_SPOKE_TO_GORON_LINK_ABOUT_DRAGON);
                        FALLTHROUGH;
                    case 0x3032:
                    case 0x3033:
...
u16 EnGo2_GetTextIdGoronCityStairwell(PlayState* play, EnGo2* this) {
    return !LINK_IS_ADULT ? GET_INFTABLE(INFTABLE_SPOKE_TO_GC_STAIRWELL_GORON) ? 0x3022 : 0x300E : 0x3043;
}

s16 EnGo2_UpdateTalkStateGoronCityStairwell(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x300E) {
            SET_INFTABLE(INFTABLE_SPOKE_TO_GC_STAIRWELL_GORON);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityLostWoods(PlayState* play, EnGo2* this) {
    if (!LINK_IS_ADULT) {
        if (Flags_GetSwitch(play, 0x1C)) {
            ...
            return GET_INFTABLE(INFTABLE_SPOKE_TO_LOST_WOODS_SHORTCUT_GORON) ? 0x3025 : 0x3024;
        }
    } else {
        return 0x3043;
    }
...
s16 EnGo2_UpdateTalkStateGoronCityLostWoods(PlayState* play, EnGo2* this) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3024) {
            SET_INFTABLE(INFTABLE_SPOKE_TO_LOST_WOODS_SHORTCUT_GORON);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
void EnGo2_SetupRolling(EnGo2* this, PlayState* play) {
    if (PARAMS_GET_S(this->actor.params, 0, 5) == GORON_CITY_ROLLING_BIG ||
        ...
        this->actor.speed = GET_INFTABLE(INFTABLE_ACQUIRED_BOMB_BAG_UPGRADE_FROM_HOT_RODDER_GORON) ? 6.0f : 3.6000001f;
    } else {
...
void EnGo2_Init(Actor* thisx, PlayState* play) {
    ...
    switch (PARAMS_GET_S(this->actor.params, 0, 5)) {
        ...
        case GORON_MARKET_BAZAAR:
            ...
            break;
        case GORON_CITY_LINK:
            if (GET_INFTABLE(INFTABLE_GORON_CITY_DOORS_UNLOCKED)) {
        ...
        case GORON_DMT_BIGGORON:
            ...
            break;
        case GORON_DMT_BOMB_FLOWER:
            if (GET_INFTABLE(INFTABLE_SPOKE_TO_DMT_BOMB_FLOWER_GORON)) {
                Path_CopyLastPoint(this->path, &this->actor.world.pos);
                this->actor.home.pos = this->actor.world.pos;
            }
            FALLTHROUGH;
    ...
    if ((Message_GetState(&play->msgCtx) == TEXT_STATE_DONE) && Message_ShouldAdvance(play)) {
        ...
        switch (this->getItemId) {
            case GI_CLAIM_CHECK:
                ...
                return;
            case GI_TUNIC_GORON:
                SET_INFTABLE(INFTABLE_GORON_CITY_DOORS_UNLOCKED);
                EnGo2_GetItemAnimation(this, play);
                return;
...
void EnGo2_GoronLinkStopRolling(EnGo2* this, PlayState* play) {
    ...
    if (Message_GetState(&play->msgCtx) != TEXT_STATE_CLOSING) {
        ...
        SET_INFTABLE(INFTABLE_STOPPED_GORON_LINKS_ROLLING);
        this->trackingMode = NPC_TRACKING_NONE;
        this->unk_211 = false;
        this->isAwake = false;
        this->actionFunc = EnGo2_CurledUp;
