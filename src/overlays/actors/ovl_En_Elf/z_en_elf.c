...
void func_80A0461C(EnElf* this, GlobalContext* globalCtx) {
    ...
    if (globalCtx->csCtx.state != CS_STATE_IDLE) {
        ...
        if ((player->stateFlags1 & PLAYER_STATE_FLAG_1_10) || ((YREG(15) & 0x10) && func_800BC56C(globalCtx, 2))) {
            temp = 12;
            this->unk_2C0 = 100;
        } else if (arrowPointedActor == NULL || arrowPointedActor->category == ACTORCAT_NPC) {
            if (arrowPointedActor != NULL) {
                this->unk_2C0 = 100;
                player->stateFlags2 |= PLAYER_STATE_FLAG_2_20;
                temp = 0;
            } else {
                switch (this->unk_2A8) {
                    case 0:
        ...
        switch (temp) {
            case 0:
                if (!(player->stateFlags2 & PLAYER_STATE_FLAG_2_20)) {
                    temp = 7;
                    if (this->unk_2C7 == 0) {
                        Audio_PlayActorSound2(&this->actor, NA_SE_EV_NAVY_VANISH);
                    }
...
void EnElf_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->unk_2A8 != 8) && !(this->fairyFlags & 8)) {
        if (!(player->stateFlags1 & PLAYER_STATE_FLAG_1_20) || (kREG(90) < this->actor.projectedPos.z)) {
