    ...
    if ((s32)this->skelAnime.curFrame == 28) {
        ...
        if (this->timer != 0) {
            ...
            if (this->timer == 10) {
                ...
                Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_R, 120, COLORFILTER_XLUFLAG_OPA, 8);
            }
        }
    }
    if ((s32)this->skelAnime.curFrame < 28) {
...
void EnDodongo_Death(EnDodongo* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame < 35.0f) {
        ...
        Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_R, 120, COLORFILTER_XLUFLAG_OPA, 4);
    }
    if (SkelAnime_Update(&this->skelAnime)) {
        if (this->timer == 0) {
