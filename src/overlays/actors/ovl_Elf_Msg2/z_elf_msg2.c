...
s32 ElfMsg2_KillCheck(ElfMsg2* this, GlobalContext* globalCtx) {
    ...
    if (0x00 < this->actor.posRot.rot.y && this->actor.posRot.rot.y < 0x41 && Flags_GetSwitch(globalCtx, this->actor.posRot.rot.y - 1)) {
...
/**
 * Runs while navi text is up. Handles the killing of the actor upon closing the text box, can also set a switch flag from params.
 */
...
/**
 * Runs while navi text is not up. If text box flag is set, unset it and proceed to ElfMsg2_Read 
 */
void ElfMsg2_CheckForRead(ElfMsg2 *this, GlobalContext *globalCtx) {
