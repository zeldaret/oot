...
void EnWallmas_DrawXlu(EnWallmas* this, PlayState* play) {
    ...
    gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_wallmas.c", 1421), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnWallMas_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    if (limbIndex == 2) {
        ...
        gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_wallmas.c", 1489), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
