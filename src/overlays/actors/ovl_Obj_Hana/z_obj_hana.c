...
void ObjHana_Init(Actor* thisx, PlayState* play) {
    ...
    s16 type = PARAMS_GET(this->actor.params, 0, 2);
...
void ObjHana_Destroy(Actor* thisx, PlayState* play) {
    ...
    if (sHanaParams[PARAMS_GET(this->actor.params, 0, 2)].radius >= 0) {
...
void ObjHana_Update(Actor* thisx, PlayState* play) {
    ...
    if (sHanaParams[PARAMS_GET(this->actor.params, 0, 2)].radius >= 0 && this->actor.xzDistToPlayer < 400.0f) {
        CollisionCheck_SetOC(play, &play->colChkCtx, &this->collider.base);
    }
}

void ObjHana_Draw(Actor* thisx, PlayState* play) {
    Gfx_DrawDListOpa(play, sHanaParams[PARAMS_GET(thisx->params, 0, 2)].dList);
}
