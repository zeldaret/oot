...
void BossGanon_IntroCutscene(BossGanon* this, PlayState* play) {
    ...
    switch (this->csState) {
        ...
        case 6:
            ...
            Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EV_TRIFORCE_MARK);
            play->envCtx.lightBlend = 0.0f;
            FALLTHROUGH;
        case 7:
            this->envLightMode = 6;
...
void BossGanon_DeathAndTowerCutscene(BossGanon* this, PlayState* play) {
    ...
    switch (this->csState) {
        ...
        case 8:
            ...
            Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EV_EARTHQUAKE - SFX_FLAG);
...
void BossGanon_Wait(BossGanon* this, PlayState* play) {
    ...
    Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&this->actor.projectedPos, NA_SE_EN_FANTOM_FLOAT - SFX_FLAG);
}
        ...
        if ((this->actionFunc == BossGanon_HitByLightBall) || (this->actionFunc == BossGanon_ChargeBigMagic)) {
            ...
            if (!(acHitInfo->toucher.dmgFlags & DMG_HOOKSHOT)) {
                ...
                if ((s8)this->actor.colChkInfo.health <= 0) {
                    ...
                    Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&sZeroVec, NA_SE_EN_LAST_DAMAGE);
                    SEQCMD_STOP_SEQUENCE(SEQ_PLAYER_BGM_MAIN, 1);
...
void BossGanon_Update(Actor* thisx, PlayState* play2) {
    ...
    if ((this->unk_2E6 != 0) || (this->unk_2E8 != 0)) {
        ...
        if (this->unk_2E8 != 0) {
            Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&player->actor.projectedPos, NA_SE_PL_SPARK - SFX_FLAG);
    ...
    if (this->unk_1C2 == 0) {
        ...
        if (sqrtf(SQ(xDiff) + SQ(zDiff) + SQ(yDiff)) < 40.0f) {
            ...
            if (this->actor.params == 0xC8) {
                Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_GANON_DAMAGE2);
                Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_GANON_DD_THUNDER);
    ...
    for (i = 0; i < BOSSGANON_EFFECT_COUNT; i++, eff++) {
        if (eff->type != GDF_EFF_NONE) {
            ...
            if (eff->type == GDF_EFF_WINDOW_SHARD) {
                ...
                if (i == 0) {
                    Audio_PlaySfxGeneralWithDefaultSettings1(NA_SE_EN_GANON_WAVE_GND - SFX_FLAG);
                }
