...
static s16 sSubCamId = CAM_ID_ACTIVE;
...
void func_808A3E54(BgMoriHineri* this, GlobalContext* globalCtx) {
    ...
    if (globalCtx->activeCamId == sSubCamId) {
        if (sSubCamId != CAM_ID_MAIN) {
            ...
            this->actionFunc = func_808A3D58;
            sSubCamId = CAM_ID_ACTIVE;
        }
    }
    if ((sSubCamId >= CAM_ID_SUB_FIRST) && ((GET_ACTIVE_CAM(globalCtx)->eye.z - this->dyna.actor.world.pos.z) < 1100.0f)) {
