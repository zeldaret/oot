...
u16 func_80AA19A0(GlobalContext* globalCtx, Actor* thisx) {
    ...
    if (gSaveContext.save.nightFlag == 1) {
        if (gSaveContext.save.info.infTable[8] & 0x1000) {
            return 0x2052;
        } else if (gSaveContext.save.info.infTable[8] & 0x4000) {
            return 0x2051;
        } else {
            return 0x2050;
        }
...
s16 func_80AA1A38(GlobalContext* globalCtx, Actor* thisx) {
    ...
    switch (func_8010BDBC(&globalCtx->msgCtx)) {
        case 2:
            switch (thisx->textId) {
                case 0x2051:
                    gSaveContext.save.info.infTable[8] |= 0x1000;
                    ret = 2;
                    break;
                case 0x2053:
                    gSaveContext.save.info.infTable[8] |= 0x2000;
                    ret = 0;
                    break;
                default:
                    ret = 0;
...
u16 func_80AA1B58(EnMa2* this, GlobalContext* globalCtx) {
    ...
    if ((!(gSaveContext.save.info.eventChkInf[1] & 0x100)) && (globalCtx->sceneNum == SCENE_MALON_STABLE) &&
        (gSaveContext.save.nightFlag == 0) && (this->actor.shape.rot.z == 5)) {
        return 1;
    }
    if ((!(gSaveContext.save.info.eventChkInf[1] & 0x100)) && (globalCtx->sceneNum == SCENE_SPOT20) &&
        (gSaveContext.save.nightFlag == 1) && (this->actor.shape.rot.z == 6)) {
        return 2;
    }
    if ((!(gSaveContext.save.info.eventChkInf[1] & 0x100)) || (globalCtx->sceneNum != SCENE_SPOT20)) {
        return 0;
    }
    if ((this->actor.shape.rot.z == 7) && (gSaveContext.save.nightFlag == 0)) {
        return 3;
    }
    if ((this->actor.shape.rot.z == 8) && (gSaveContext.save.nightFlag == 1)) {
        return 3;
    }
    return 0;
}
...
void EnMa2_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (func_80AA1B58(this, globalCtx)) {
        ...
        case 2:
            ...
            break;
        case 3:
            if (gSaveContext.save.info.infTable[8] & 0x2000) {
...
void func_80AA20E4(EnMa2* this, GlobalContext* globalCtx) {
    ...
    if (globalCtx->msgCtx.unk_E3EE >= 4) {
        ...
        this->unk_208 = 0x1E;
        gSaveContext.save.info.infTable[8] |= 0x4000;
