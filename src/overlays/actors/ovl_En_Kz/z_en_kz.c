...
u16 EnKz_GetTextNoMaskChild(GlobalContext* globalCtx, EnKz* this) {
    ...
    if (CHECK_QUEST_ITEM(QUEST_ZORA_SAPPHIRE)) {
        return 0x402B;
    } else if (gSaveContext.save.info.eventChkInf[3] & 8) {
        return 0x401C;
    } else {
...
u16 EnKz_GetTextNoMaskAdult(GlobalContext* globalCtx, EnKz* this) {
    ...
    if (INV_CONTENT(ITEM_TRADE_ADULT) >= ITEM_FROG) {
        if (!(gSaveContext.save.info.infTable[19] & 0x200)) {
    ...
    switch (func_8010BDBC(&globalCtx->msgCtx)) {
        case 6:
            ...
            switch (this->actor.textId) {
                case 0x4012:
                    gSaveContext.save.info.infTable[19] |= 0x200;
                    ret = 2;
                    break;
                case 0x401B:
...
void func_80A9CB18(EnKz* this, GlobalContext* globalCtx) {
    ...
    if (func_80A9C95C(globalCtx, this, &this->unk_1E0.unk_00, 340.0f, EnKz_GetText, func_80A9C6C0) != 0) {
        if ((this->actor.textId == 0x401A) && !(gSaveContext.save.info.eventChkInf[3] & 8)) {
        ...
        if (LINK_IS_ADULT) {
            ...
            if (gSaveContext.save.info.infTable[19] & 0x200) {
                ...
                this->actor.textId = CHECK_OWNED_EQUIP(EQUIP_TUNIC, PLAYER_TUNIC_ZORA) ? 0x401F : 0x4012;
                player->actor.textId = this->actor.textId;
            }
        }
    }
...
void EnKz_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.save.info.eventChkInf[3] & 8) {
        EnKz_SetMovedPos(this, globalCtx);
    }

    if (LINK_IS_ADULT) {
        if (!(gSaveContext.save.info.infTable[19] & 0x100)) {
...
void EnKz_Mweep(EnKz* this, GlobalContext* globalCtx) {
    ...
    if ((EnKz_FollowPath(this, globalCtx) == 1) && (this->waypoint == 0)) {
        ...
        EnKz_SetMovedPos(this, globalCtx);
        gSaveContext.save.info.eventChkInf[3] |= 8;
...
void EnKz_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (LINK_IS_ADULT && !(gSaveContext.save.info.infTable[19] & 0x100)) {
        gSaveContext.save.info.infTable[19] |= 0x100;
    }
