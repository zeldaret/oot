...
void EnFz_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->isActive) {
        ...
        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_fz.c", 1183), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnFz_DrawEffects(EnFz* this, PlayState* play) {
    ...
    for (i = 0; i < EN_FZ_EFFECT_COUNT; i++) {
        if (effect->type > 0) {
            ...
            gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(gfxCtx, "../z_en_fz.c", 1424), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
