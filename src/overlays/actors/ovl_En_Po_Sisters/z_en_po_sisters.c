...
#ifdef NON_MATCHING
// Small reordering around unk_196 decrement
...
void func_80ADB338(EnPoSisters* this, GlobalContext* globalCtx) {
    ...
    if (this->unk_195 == 0) {
        ...
        if (this->unk_19C == 0) {
            this->actor.shape.rot.y = this->actor.yawTowardsLink + 0x4000 * (s32)(Math_Rand_ZeroOne() * 4.0f);
...
void func_80ADB51C(EnPoSisters* this, GlobalContext* globalCtx) {
    ...
    if (this->unk_19A == 0) {
        ...
        if (this->unk_195 != 0) {
            ...
            Math_ApproxUpdateScaledS(&this->actor.shape.rot.y, this->actor.attachedA->shape.rot.y + ((s16)(this->unk_195 * 0x4000) * phi_v0), phi_a2);
    ...
    if (this->collider.base.acFlags & 2) {
        ...
        if (this->unk_195 != 0) {
            ...
            if (Math_Rand_ZeroOne() < 0.2f) {
                ...
                Item_DropCollectible(globalCtx, &pos, 8);
            }
        } else if (this->collider.base.type != 9 && this->actor.colChkInfo.damageEffect == 0) {
            if (this->actor.colChkInfo.damage == 0 && this->unk_194 == 0) {
...
void EnPoSisters_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_en_po_sisters.c", 0xBAD);
    func_80ADC55C(this);
    ...
    gSPSegment(gfxCtx->polyXlu.p++, 0x08, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 0x20, 0x40, 1, 0, (globalCtx->gameplayFrames * -20) % 512, 0x20, 0x80));
    gDPSetEnvColor(gfxCtx->polyXlu.p++, temp_s1->r, temp_s1->g, temp_s1->b, temp_s1->a);
    if (this->actionFunc == func_80ADB17C) {
        if (this->unk_19A < 0x20) {
            phi_s5 = ((0x20 - this->unk_19A) * 0xFF) / 0x20;
            phi_f20 = 0.0056000003f;
        } else {
            phi_s5 = (this->unk_19A * 0xFF - 0x1FE0) / 0x20;
            phi_f20 = 0.0027f;
        }
    ...
    Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_en_po_sisters.c", 0xC43);
}
