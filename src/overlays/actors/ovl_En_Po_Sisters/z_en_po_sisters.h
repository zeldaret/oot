...
typedef enum EnPoSisFlags {
    ...
    EN_POESIS_FLAG_UPDATEMASK=(EN_POESIS_FLAG_ACCOL|EN_POESIS_FLAG_ROTATE|
        EN_POESIS_FLAG_VANISH|EN_POESIS_FLAG_HOVER|EN_POESIS_FLAG_BGCHECK),
    EN_POESIS_FLAG_TORCH=1<<5, // manipulate torch flames
    EN_POESIS_FLAG_SPIN=1<<6, // the real Meg spins as a tell
    EN_POESIS_FLAG_NOMTXF=1<<7, //don't read or write torchMtx
} EnPoSisFlags;

#define EN_POESIS_DECOY_PARAM 1<<10 // param for Meg's decoys
#define EN_POESIS_INTRO_PARAM 1<<12 // param for Poe Sisters when first entering foyer
