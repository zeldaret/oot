...
void EnFd_DrawEffectsFlames(EnFd* this, PlayState* play) {
    ...
    for (i = 0; i < EN_FD_EFFECT_COUNT; i++, eff++) {
        ...
        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_fd.c", 2006), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnFd_DrawEffectsDots(EnFd* this, PlayState* play) {
    ...
    for (i = 0; i < EN_FD_EFFECT_COUNT; i++, eff++) {
        ...
        Matrix_ReplaceRotation(&play->billboardMtxF);
        Matrix_Scale(eff->scale, eff->scale, 1.0f, MTXMODE_APPLY);
        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_fd.c", 2064), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
