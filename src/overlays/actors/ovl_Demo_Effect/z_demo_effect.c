...
void DemoEffect_MoveJewelSpherical(f32 degrees, f32 frameDivisor, Vec3f startPos, Vec3f endPos, f32 radius,
    ...
    ySpherical = (this->actor.world.pos.y * cosf(BINANG_TO_RAD_ALT(rotation.x))) -
                 (sinf(BINANG_TO_RAD_ALT(rotation.x)) * this->actor.world.pos.z);
    xzSpherical = (this->actor.world.pos.z * cosf(BINANG_TO_RAD_ALT(rotation.x))) +
                  (sinf(BINANG_TO_RAD_ALT(rotation.x)) * this->actor.world.pos.y);

    this->actor.world.pos.x =
        (xPos * cosf(BINANG_TO_RAD_ALT(rotation.y))) - (sinf(BINANG_TO_RAD_ALT(rotation.y)) * xzSpherical);
    this->actor.world.pos.y = ySpherical;
    this->actor.world.pos.z =
        (xzSpherical * cosf(BINANG_TO_RAD_ALT(rotation.y))) + (sinf(BINANG_TO_RAD_ALT(rotation.y)) * xPos);
...
void DemoEffect_DrawBlueOrb(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_80093D84(globalCtx->state.gfxCtx);
    Matrix_Mult(&globalCtx->billboardMtxF, MTXMODE_APPLY);
    Matrix_RotateZ(BINANG_TO_RAD_ALT(this->blueOrb.rotation), MTXMODE_APPLY);
    ...
    if (gSaveContext.entranceIndex != 0x0400 || globalCtx->csCtx.frames < 885) {
        ...
        if (this->triforceSpot.triforceSpotOpacity != 0) {
            ...
            if (this->triforceSpot.triforceSpotOpacity < 250) {
                ...
                Matrix_RotateY(BINANG_TO_RAD_ALT(this->triforceSpot.rotation), MTXMODE_APPLY);
