...
void EnDaiku_Init(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET(this->actor.params, 0, 2) == 0 && GET_EVENTCHKINF(EVENTCHKINF_CARPENTERS_FREE(0))) {
        isFree = true;
    } else if (PARAMS_GET(this->actor.params, 0, 2) == 1 && GET_EVENTCHKINF(EVENTCHKINF_CARPENTERS_FREE(1))) {
        isFree = true;
    } else if (PARAMS_GET(this->actor.params, 0, 2) == 2 && GET_EVENTCHKINF(EVENTCHKINF_CARPENTERS_FREE(2))) {
        isFree = true;
    } else if (PARAMS_GET(this->actor.params, 0, 2) == 3 && GET_EVENTCHKINF(EVENTCHKINF_CARPENTERS_FREE(3))) {
        isFree = true;
    }
    ...
    if (play->sceneId == SCENE_GERUDOWAY) {
        ...
        if (PARAMS_GET(this->actor.params, 0, 2) == 1 || PARAMS_GET(this->actor.params, 0, 2) == 3) {
...
void EnDaiku_InitEscape(EnDaiku* this, PlayState* play) {
    ...
    gSaveContext.eventChkInf[EVENTCHKINF_CARPENTERS_FREE_INDEX] |=
        EVENTCHKINF_CARPENTERS_FREE_MASK(PARAMS_GET(this->actor.params, 0, 2));

    this->actor.gravity = -1.0f;
    this->escapeSubCamTimer = sEscapeSubCamParams[PARAMS_GET(this->actor.params, 0, 2)].maxFramesActive;
    EnDaiku_InitSubCamera(this, play);

    exitLoop = false;
    path = &play->setupPathList[PARAMS_GET(this->actor.params, 4, 4)];
    while (!exitLoop) {
...
void EnDaiku_InitSubCamera(EnDaiku* this, PlayState* play) {
    ...
    eyePosDeltaLocal.x = sEscapeSubCamParams[PARAMS_GET(this->actor.params, 0, 2)].eyePosDeltaLocal.x;
    eyePosDeltaLocal.y = sEscapeSubCamParams[PARAMS_GET(this->actor.params, 0, 2)].eyePosDeltaLocal.y;
    eyePosDeltaLocal.z = sEscapeSubCamParams[PARAMS_GET(this->actor.params, 0, 2)].eyePosDeltaLocal.z;
...
void EnDaiku_Draw(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET(thisx->params, 0, 2) == 0) {
        gDPSetEnvColor(POLY_OPA_DISP++, 170, 10, 70, 255);
    } else if (PARAMS_GET(thisx->params, 0, 2) == 1) {
        gDPSetEnvColor(POLY_OPA_DISP++, 170, 200, 255, 255);
    } else if (PARAMS_GET(thisx->params, 0, 2) == 2) {
        gDPSetEnvColor(POLY_OPA_DISP++, 0, 230, 70, 255);
    } else if (PARAMS_GET(thisx->params, 0, 2) == 3) {
...
void EnDaiku_PostLimbDraw(PlayState* play, s32 limb, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limb == 15) { // head
        ...
        gSPDisplayList(POLY_OPA_DISP++, hairDLists[PARAMS_GET(this->actor.params, 0, 2)]);
    }
