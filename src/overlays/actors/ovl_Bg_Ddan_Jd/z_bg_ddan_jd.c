...
void BgDdanJd_Idle(BgDdanJd* this, PlayState* play) {
    ...
    if (this->ySpeed == DEFAULT_Y_SPEED && this->bg.actor.params < 0x40 &&
        Flags_GetSwitch(play, this->bg.actor.params)) {
        this->ySpeed = SHORTCUT_Y_SPEED;
        this->state = STATE_GO_MIDDLE_FROM_BOTTOM;
        this->idleTimer = 0;
        this->bg.actor.world.pos.y = this->bg.actor.home.pos.y + MOVE_HEIGHT_MIDDLE;
        OnePointCutscene_Init(play, 3060, -99, &this->bg.actor, CAM_ID_MAIN);
    }
    if (this->idleTimer == 0) {
        this->idleTimer = IDLE_FRAMES;
        if (this->state == STATE_GO_BOTTOM) {
            this->state = STATE_GO_MIDDLE_FROM_BOTTOM;
            this->targetY = this->bg.actor.home.pos.y + MOVE_HEIGHT_MIDDLE;
            ...
            if (this->ySpeed != DEFAULT_Y_SPEED) {
                ...
                this->targetY = this->bg.actor.home.pos.y + MOVE_HEIGHT_TOP;
            } else {
                this->state = STATE_GO_BOTTOM;
                this->targetY = this->bg.actor.home.pos.y;
            }
...
void BgDdanJd_MoveEffects(BgDdanJd* this, PlayState* play) {
    ...
    dustPos.y = this->bg.actor.home.pos.y;
    if (play->gameplayFrames & 1) {
        dustPos.x = this->bg.actor.world.pos.x + 65.0f;
        dustPos.z = Rand_CenteredFloat(110.0f) + this->bg.actor.world.pos.z;
        func_80033480(play, &dustPos, 5.0f, 1, 20, 60, 1);
        dustPos.x = this->bg.actor.world.pos.x - 65.0f;
        dustPos.z = Rand_CenteredFloat(110.0f) + this->bg.actor.world.pos.z;
        func_80033480(play, &dustPos, 5.0f, 1, 20, 60, 1);
    } else {
        dustPos.x = Rand_CenteredFloat(110.0f) + this->bg.actor.world.pos.x;
        dustPos.z = this->bg.actor.world.pos.z + 65.0f;
        func_80033480(play, &dustPos, 5.0f, 1, 20, 60, 1);
        dustPos.x = Rand_CenteredFloat(110.0f) + this->bg.actor.world.pos.x;
        dustPos.z = this->bg.actor.world.pos.z - 65.0f;
        func_80033480(play, &dustPos, 5.0f, 1, 20, 60, 1);
...
void BgDdanJd_Move(BgDdanJd* this, PlayState* play) {
    ...
    if (this->ySpeed == DEFAULT_Y_SPEED && this->bg.actor.params < 0x40 &&
        ...
        this->actionFunc = BgDdanJd_Idle;
        OnePointCutscene_Init(play, 3060, -99, &this->bg.actor, CAM_ID_MAIN);
    } else if (Math_StepToF(&this->bg.actor.world.pos.y, this->targetY, this->ySpeed)) {
        Audio_PlayActorSfx2(&this->bg.actor, NA_SE_EV_PILLAR_MOVE_STOP);
        this->actionFunc = BgDdanJd_Idle;
    }
