...
void EnSw_Init(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET_NOSHIFT(thisx->params, 15, 1)) {
        phi_v0 = PARAMS_GET2(thisx->params - 0x8000, 13, 3) + 1;
        thisx->params = PARAMS_GET2(thisx->params, 0, 13) | (phi_v0 << 0xD);
    }

    if (PARAMS_GET2(thisx->params, 13, 3) > 0) {
        phi_v0 = PARAMS_GET2(thisx->params, 8, 5) - 1;
        thisx->params = (thisx->params & ~(0x1F << 8)) | (phi_v0 << 8);
    }

    // Check to see if this gold skull token has already been retrieved.
    if (GET_GS_FLAGS(PARAMS_GET2(thisx->params, 8, 5)) & PARAMS_GET2(thisx->params, 0, 8)) {
    ...
    if (PARAMS_GET2(thisx->params, 13, 3) >= 3) {
        Audio_PlaySoundGeneral(NA_SE_SY_CORRECT_CHIME, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                               &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
    }

    switch (PARAMS_GET2(thisx->params, 13, 3)) {
        case 3:
        case 4:
            this->unk_360 = 1;
    ...
    } else if (PARAMS_GET2(thisx->params, 13, 3) == 0) {
...
s32 func_80B0C9F0(EnSw* this, PlayState* play) {
    ...
    if (this->actor.xyzDistToPlayerSq < SQ(400.0f) && PARAMS_GET2(this->actor.params, 13, 3) == 0 &&
    ...
    if (this->unk_392 == 0) {
        if ((this->collider.base.acFlags & AC_HIT) || phi_v1) {
            ...
            if (PARAMS_GET2(this->actor.params, 13, 3) != 0) {
...
void func_80B0CBE8(EnSw* this, PlayState* play) {
    if ((PARAMS_GET2(this->actor.params, 13, 3) > 0) && (this->actionFunc != func_80B0D590)) {
        if (this->unk_392 != 0) {
            this->unk_392--;
        }
    } else {
...
void func_80B0CEA8(EnSw* this, PlayState* play) {
    if (!(this->actor.scale.x < 0.0139999995f)) {
        ...
        if (!(Math_Vec3f_DistXYZ(&this->actor.world.pos, &activeCam->eye) >= 380.0f)) {
            Audio_PlayActorSound2(&this->actor, PARAMS_GET2(this->actor.params, 13, 3) > 0 ? NA_SE_EN_STALGOLD_ROLL
...
void func_80B0D364(EnSw* this, PlayState* play) {
    if (PARAMS_GET2(this->actor.params, 13, 3) == 4) {
        this->unk_38C = 0;
...
void func_80B0D590(EnSw* this, PlayState* play) {
    ...
    if (PARAMS_GET2(this->actor.params, 13, 3) == 2) {
        if (this->actor.scale.x < 0.0139999995f) {
