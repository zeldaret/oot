...
// subtype:     (this->bg.actor.params >> 4 & 7)
// switch flag: (this->bg.actor.params >> 8 & 0x3F)
// frozen:      this->bg.actor.params >> 7 & 1
...
void ObjSwitch_InitJntSphCollider(ObjSwitch* this, PlayState* play, ColliderJntSphInit* colliderJntSphInit) {
    ...
    Collider_SetJntSph(play, colliderJntSph, &this->bg.actor, colliderJntSphInit, this->jntSph.items);
    Matrix_SetTranslateRotateYXZ(this->bg.actor.world.pos.x,
                                 this->bg.actor.world.pos.y + this->bg.actor.shape.yOffset * this->bg.actor.scale.y,
                                 this->bg.actor.world.pos.z, &this->bg.actor.shape.rot);
    Matrix_Scale(this->bg.actor.scale.x, this->bg.actor.scale.y, this->bg.actor.scale.z, MTXMODE_APPLY);
...
void ObjSwitch_InitTrisCollider(ObjSwitch* this, PlayState* play, ColliderTrisInit* colliderTrisInit) {
    ...
    Collider_SetTris(play, colliderTris, &this->bg.actor, colliderTrisInit, this->tris.items);

    for (i = 0; i < 2; i++) {
        for (j = 0; j < 3; j++) {
            ObjSwitch_RotateY(&pos[j], &colliderTrisInit->elements[i].dim.vtx[j], this->bg.actor.home.rot.y);
            Math_Vec3f_Sum(&pos[j], &this->bg.actor.world.pos, &pos[j]);
        }
...
void ObjSwitch_SetOn(ObjSwitch* this, PlayState* play) {
    ...
    if (Flags_GetSwitch(play, (this->bg.actor.params >> 8 & 0x3F))) {
        ...
        if (subType == OBJSWITCH_SUBTYPE_ONCE || subType == OBJSWITCH_SUBTYPE_SYNC) {
            OnePointCutscene_AttentionSetSfx(play, &this->bg.actor, NA_SE_SY_CORRECT_CHIME);
        } else {
            OnePointCutscene_AttentionSetSfx(play, &this->bg.actor, NA_SE_SY_TRE_BOX_APPEAR);
        }
...
void ObjSwitch_SetOff(ObjSwitch* this, PlayState* play) {
    ...
    if (Flags_GetSwitch(play, (this->bg.actor.params >> 8 & 0x3F))) {
        ...
        if ((this->bg.actor.params >> 4 & 7) == OBJSWITCH_SUBTYPE_TOGGLE) {
            OnePointCutscene_AttentionSetSfx(play, &this->bg.actor, NA_SE_SY_TRE_BOX_APPEAR);
            this->cooldownOn = true;
        }
    }
...
void ObjSwitch_Init(Actor* thisx, PlayState* play) {
    ...
    switchFlagSet = Flags_GetSwitch(play, (this->bg.actor.params >> 8 & 0x3F));
    type = (this->bg.actor.params & 7);
    ...
    if (type == OBJSWITCH_TYPE_FLOOR || type == OBJSWITCH_TYPE_FLOOR_RUSTY) {
        this->bg.actor.world.pos.y = this->bg.actor.home.pos.y + 1.0f;
    }

    Actor_SetFocus(&this->bg.actor, sHeights[type]);
    ...
    if (type == OBJSWITCH_TYPE_CRYSTAL_TARGETABLE) {
        ...
        this->bg.actor.targetMode = 4;
    }

    this->bg.actor.colChkInfo.mass = MASS_IMMOVABLE;

    if ((this->bg.actor.params >> 7 & 1) && (ObjSwitch_SpawnIce(this, play) == NULL)) {
        osSyncPrintf(VT_FGCOL(RED));
        osSyncPrintf("Error : 氷発生失敗 (%s %d)\n", "../z_obj_switch.c", 732);
        osSyncPrintf(VT_RST);
        this->bg.actor.params &= ~0x80;
    }

    if (this->bg.actor.params >> 7 & 1) {
...
void ObjSwitch_FloorUpInit(ObjSwitch* this) {
    this->bg.actor.scale.y = 33.0f / 200.0f;
    this->actionFunc = ObjSwitch_FloorUp;
}

void ObjSwitch_FloorUp(ObjSwitch* this, PlayState* play) {
    if ((this->bg.actor.params & 7) == OBJSWITCH_TYPE_FLOOR_RUSTY) {
        ...
        switch (this->bg.actor.params >> 4 & 7) {
            ...
            case OBJSWITCH_SUBTYPE_TOGGLE:
                if ((this->bg.interactFlags & DYNA_INTERACT_PLAYER_ON_TOP) &&
...
void ObjSwitch_FloorPress(ObjSwitch* this, PlayState* play) {
    if ((this->bg.actor.params >> 4 & 7) == OBJSWITCH_SUBTYPE_HOLD_INVERTED || !this->cooldownOn ||
        func_8005B198() == this->bg.actor.category || this->cooldownTimer <= 0) {
        this->bg.actor.scale.y -= 99.0f / 2000.0f;
        if (this->bg.actor.scale.y <= 33.0f / 2000.0f) {
            ObjSwitch_FloorDownInit(this);
            Audio_PlayActorSfx2(&this->bg.actor, NA_SE_EV_FOOT_SWITCH);
            func_800AA000(this->bg.actor.xyzDistToPlayerSq, 120, 20, 10);
        }
    }
}

void ObjSwitch_FloorDownInit(ObjSwitch* this) {
    this->bg.actor.scale.y = 33.0f / 2000.0f;
    this->releaseTimer = 6;
...
void ObjSwitch_FloorDown(ObjSwitch* this, PlayState* play) {
    switch (this->bg.actor.params >> 4 & 7) {
        ...
        case OBJSWITCH_SUBTYPE_HOLD:
        case OBJSWITCH_SUBTYPE_HOLD_INVERTED:
            if (!func_800435B4(&this->bg) && !Player_InCsMode(play)) {
                if (this->releaseTimer <= 0) {
                    ObjSwitch_FloorReleaseInit(this);
                    if ((this->bg.actor.params >> 4 & 7) == OBJSWITCH_SUBTYPE_HOLD) {
                        ObjSwitch_SetOff(this, play);
                    } else {
                        ObjSwitch_SetOn(this, play);
                    }
...
s32 ObjSwitch_EyeIsHit(ObjSwitch* this) {
    ...
    if ((this->tris.col.base.acFlags & AC_HIT) && !(this->unk_17F & AC_HIT)) {
        ...
        if (collidingActor != NULL) {
            yawDiff = collidingActor->world.rot.y - this->bg.actor.shape.rot.y;
...
void ObjSwitch_EyeOpen(ObjSwitch* this, PlayState* play) {
    if (ObjSwitch_EyeIsHit(this) || (this->bg.actor.params >> 7 & 1)) {
...
void ObjSwitch_EyeClosing(ObjSwitch* this, PlayState* play) {
    if (!this->cooldownOn || func_8005B198() == this->bg.actor.category || this->cooldownTimer <= 0) {
        this->eyeTexIndex++;
...
void ObjSwitch_EyeOpening(ObjSwitch* this, PlayState* play) {
    if ((this->bg.actor.params >> 4 & 7) != OBJSWITCH_SUBTYPE_TOGGLE || !this->cooldownOn ||
        func_8005B198() == this->bg.actor.category || this->cooldownTimer <= 0) {
        this->eyeTexIndex--;
...
void ObjSwitch_CrystalTurnOn(ObjSwitch* this, PlayState* play) {
    if (!this->cooldownOn || func_8005B198() == this->bg.actor.category || this->cooldownTimer <= 0) {
        ...
        Audio_PlayActorSfx2(&this->bg.actor, NA_SE_EV_DIAMOND_SWITCH);
    }
}
...
void ObjSwitch_Update(Actor* thisx, PlayState* play) {
    ...
    switch (this->bg.actor.params & 7) {
        case OBJSWITCH_TYPE_FLOOR:
        case OBJSWITCH_TYPE_FLOOR_RUSTY:
            this->unk_17F = this->bg.interactFlags;
            break;
        case OBJSWITCH_TYPE_EYE:
...
void ObjSwitch_DrawFloor(ObjSwitch* this, PlayState* play) {
    ...
    Gfx_DrawDListOpa(play, floorSwitchDLists[(this->bg.actor.params >> 4 & 7)]);
}
...
void ObjSwitch_DrawEye(ObjSwitch* this, PlayState* play) {
    ...
    s32 pad;
    s32 subType = (this->bg.actor.params >> 4 & 7);
...
void ObjSwitch_Draw(Actor* thisx, PlayState* play) {
    ...
    sDrawFuncs[(this->bg.actor.params & 7)](this, play);
}
