...
void EnFw_Run(EnFw* this, PlayState* play) {
    ...
    if (this->explosionTimer != 0) {
        ...
        if (this->actor.colorFilterTimer == 0) {
            Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_R, 200, COLORFILTER_XLUFLAG_OPA, this->explosionTimer);
            this->explosionTimer--;
        }
        ...
        if (!(this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) || this->actor.velocity.y > 0.0f) {
            Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_R, 200, COLORFILTER_XLUFLAG_OPA, this->damageTimer);
            return;
        }
