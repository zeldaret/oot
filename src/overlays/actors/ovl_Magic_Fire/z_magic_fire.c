...
void MagicFire_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->action == DF_ACTION_EXPAND_SLOWLY) {
        this->collider.elem.atDmgInfo.damage_ColliderElementDamageInfoAT = this->actionTimer + 25;
    } else if (this->action == DF_ACTION_STOP_EXPANDING) {
        this->collider.elem.atDmgInfo.damage_ColliderElementDamageInfoAT = this->actionTimer;
    }
    Collider_UpdateCylinder(&this->actor, &this->collider);
