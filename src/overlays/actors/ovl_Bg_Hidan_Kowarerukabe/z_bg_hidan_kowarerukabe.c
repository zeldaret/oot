...
void BgHidanKowarerukabe_InitDynaPoly(BgHidanKowarerukabe* this, PlayState* play) {
    ...
    if (collisionHeaders[PARAMS_GET(this->dyna.actor.params, 0, 8)] != NULL) {
        DynaPolyActor_Init(&this->dyna, 0);
        CollisionHeader_GetVirtual(collisionHeaders[PARAMS_GET(this->dyna.actor.params, 0, 8)], &colHeader);
...
void BgHidanKowarerukabe_InitColliderSphere(BgHidanKowarerukabe* this, PlayState* play) {
    ...
    this->collider.elements[0].dim.modelSphere.radius = sphereRadii[PARAMS_GET(this->dyna.actor.params, 0, 8)];
    this->collider.elements[0].dim.modelSphere.center.y = sphereYPositions[PARAMS_GET(this->dyna.actor.params, 0, 8)];
}
...
void BgHidanKowarerukabe_Init(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET(this->dyna.actor.params, 0, 8) < CRACKED_STONE_FLOOR ||
        PARAMS_GET(this->dyna.actor.params, 0, 8) > LARGE_BOMBABLE_WALL) {
...
void BgHidanKowarerukabe_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPDisplayList(POLY_OPA_DISP++, sBreakableWallDLists[PARAMS_GET(this->dyna.actor.params, 0, 8)]);
