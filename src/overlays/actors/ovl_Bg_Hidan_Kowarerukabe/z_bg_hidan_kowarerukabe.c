...
void BgHidanKowarerukabe_InitDynaPoly(BgHidanKowarerukabe* this, PlayState* play) {
    ...
    if (collisionHeaders[PARAMS_GET(this->dyna.actor.params, 0, 0xFF)] != NULL) {
        DynaPolyActor_Init(&this->dyna, DPM_UNK);
        CollisionHeader_GetVirtual(collisionHeaders[PARAMS_GET(this->dyna.actor.params, 0, 0xFF)], &colHeader);
...
void BgHidanKowarerukabe_InitColliderSphere(BgHidanKowarerukabe* this, PlayState* play) {
    ...
    this->collider.elements[0].dim.modelSphere.radius = sphereRadii[PARAMS_GET(this->dyna.actor.params, 0, 0xFF)];
    this->collider.elements[0].dim.modelSphere.center.y = sphereYPositions[PARAMS_GET(this->dyna.actor.params, 0, 0xFF)];
}

void BgHidanKowarerukabe_OffsetActorYPos(BgHidanKowarerukabe* this) {
    static f32 actorYPosOffsets[] = { 0.7f, 0.0f, 0.0f };

    this->dyna.actor.world.pos.y = actorYPosOffsets[PARAMS_GET(this->dyna.actor.params, 0, 0xFF)] + this->dyna.actor.home.pos.y;
}
...
void BgHidanKowarerukabe_Init(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET(this->dyna.actor.params, 0, 0xFF) < CRACKED_STONE_FLOOR ||
        PARAMS_GET(this->dyna.actor.params, 0, 0xFF) > LARGE_BOMBABLE_WALL) {
...
void BgHidanKowarerukabe_Update(Actor* thisx, PlayState* play) {
    ...
    if (Actor_GetCollidedExplosive(play, &this->collider.base) != NULL) {
        ...
        if (PARAMS_GET(this->dyna.actor.params, 0, 0xFF) == 0) {
...
void BgHidanKowarerukabe_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPDisplayList(POLY_OPA_DISP++, sBreakableWallDLists[PARAMS_GET(this->dyna.actor.params, 0, 0xFF)]);
