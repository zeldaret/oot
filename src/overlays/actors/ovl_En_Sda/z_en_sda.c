...
void func_80AF9C70(u8* shadowTexture, Player* player, PlayState* play) {
    ...
    gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_sda.c", 860), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    ...
    for (phi_s1 = 0; phi_s1 < KREG(78); phi_s1++) {
        ...
        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_sda.c", 877), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_XLU_DISP++, D_80AFA3F8);
    }
    PRINTF("SDA D 2\n");
