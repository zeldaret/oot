...
void func_80A1A9A0(EnFr* this); // EnFr_OrientUnderwater
void func_80A1AC0C(EnFr* this); // EnFr_DrawIdle
...
void func_80A1B19C(EnFr* this); // EnFr_DecrementBlinkTimerUpdate

// Animation Functions
void func_80A1B22C(EnFr* this, GlobalContext* globalCtx); // EnFr_InitJumpingOutOfWater
...
void func_80A1BB34(EnFr* this, GlobalContext* globalCtx); // EnFr_UpdateActive
...
//
// D_80A1CFF0.flags system
//
//
// D_80A1CFF0.flags = 0x01
//      - Activate frogs, frogs will jump out of the water
// 
// D_80A1CFF0.flags = 0x01 to 0x0B:
...
// Song Flags for gSaveContext.eventChkInf[13]
u16 D_80A1D008[] = { 
    0x0002,   // ZL            (second bit)
...
// u8 Frog Index
// Frog to Index for Song Flag (D_80A1D008) Mapping 
u8 D_80A1D018[] = { 
    0x02,   // Frog 0: (Yellow - Middle)
...
s32 D_80A1D020[] = { 
    0x00000003,     // Song 0 = ZL    = Frog 3 = (Purple - Back Left)
...
// Sets the colour of the frogs (POLY_OPA_DISP)
EnFrColor D_80A1D054[5] = {  
    {0xC8, 0xAA, 0x00, 0xFF},   // frog 0 (Yellow - Middle)
...
//      D_80A1D068[frog].xzDist is magnitude of xz distance frog travels
//      D_80A1D068[frog].yaw is rot around y-axis of jumping back into water
//      D_80A1D068[frog].yDist is change in y distance frog travels
EnFrLogSpotToFromWater D_80A1D068[6] = {
...
s16 D_80A1D0B0[] = { 
    40, // frames to play the first note
...
// Used as part of D_80A1CFF0.flags
u8 D_80A1D0C4[] = { 
    0x05,       // frog 0 (Yellow - Middle)
...
f32 D_80A1D0CC[] = { 
    180.0,      // 1st incremental scale
    210.0,      // 2nd incremental scale
    240.0,      // 3rd incremental scale
    270.0 };    // Final scale

// Audio Pitch for frog croak while jumping
// Small Frog (before playing child song)
u8 D_80A1D0DC[] = {  
    0x05,       // frog 0 (Yellow - C-Down Ocarina) 
...
// Audio Pitch for frog croak while jumping
// Large Frog (after playing child song)
u8 D_80A1D0E4[] = { 
    0xF9,       // frog 0 (Yellow - C-Down Ocarina) 
...
// Indexed by this->jumpCounter
// Jumping Order when when child song is played (jump in celebration)
// Starts at index 1,
// Loops around to index 0 after index 7
// Eleven total jumps, once every 4 frames
u8 D_80A1D0EC[] = { 
    0x01,       // frog 1 (Blue)
    0x00,       // frog 0 (Yellow)
    0x02,       // frog 2 (Red)
...
// Segmented Addressing System for POLY_OPA_DISP
s32 D_80A1D0FC[] = { 
    0x060059A0, 
    0x06005BA0 };
...
void func_80A1A9A0(EnFr* this) {
    ...
    xzDist = D_80A1D068[this->actor.params].xzDist; //80.0
    this->xzDistToLogSpot = xzDist;
    vec1.z = xzDist;

    Matrix_RotateY(D_80A1D068[this->actor.params].yaw, MTXMODE_NEW); // C * PI
    Matrix_MultVec3f(&vec1, &vec2);

    this->actor.posRot.pos.x = this->posLogSpot.x + vec2.x;
    this->actor.posRot.pos.z = this->posLogSpot.z + vec2.z;
    this->actor.posRot.pos.y = D_80A1D068[this->actor.params].yDist + this->posLogSpot.y; // -80.0

    yaw = (s16)(D_80A1D068[this->actor.params].yaw * (32768.0f / M_PI)) + 32768; 
...
void EnFr_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params == 0) {
        ...
        // [sound 28E3]
        // Young boy, you did grrreat! Perrrrrfect! We are stuffed, ribbit!
        // Would you take this token of our gratitude?
...
// EnFr_InitJumpingOutOfWater
void func_80A1B22C(EnFr* this, GlobalContext* globalCtx) {
    AnimationHeader* objSegChangeAnime = &D_060007BC; // 0x060007BC

    // Indicator for frog to start jumping out of water
    // Note that the frogs jump out one at a time
    // With the order determined by D_80A1D0C4
    if (D_80A1CFF0.flags == D_80A1D0C4[this->actor.params - 1]) {
...
void EnFr_JumpingOutOfWater(EnFr* this, GlobalContext* globalCtx) {
    ...
    vec1.y = 0.0f;
    vec1.x = 0.0f;          
    vec1.z = this->xzDistToLogSpot; // xzDist
...
void EnFr_ChooseJumpFromLogSpot(EnFr* this, GlobalContext* globalCtx) {
    ...
    if (D_80A1CFF0.flags == 0xC) { 
        this->actor.posRot.rot.y = (32768.0f / M_PI) * D_80A1D068[this->actor.params].yaw;
        SkelAnime_ChangeAnim(&this->skelAnime, &D_060007BC, 1.0f, 0.0f,
                             SkelAnime_GetFrameCount(&D_060007BC), 2, 0.0f);
...
void EnFr_JumpingBackIntoWater(EnFr* this, GlobalContext* globalCtx) {
    ...
    y_underwater = D_80A1D068[this->actor.params].yDist + this->posLogSpot.y;
...
void func_80A1B8D0(EnFr *this, GlobalContext *globalCtx) {
    if (this->isGrowing) {
        ...
        Math_SmoothScaleMaxF(&this->scale, D_80A1D0CC[this->growingScaleIndex], 2.0f, 25.0f);
        if (this->scale >= D_80A1D0CC[this->growingScaleIndex]) {
            this->scale = D_80A1D0CC[this->growingScaleIndex];
...
void func_80A1BB34(EnFr* this, GlobalContext* globalCtx) {
...
s32 EnFr_InitJumpingUp(EnFr *this, s32 frogIndex) {
    ...
    frog = D_80A1CFF0.frogs[frogIndex];
    //frog->actor.id
    if (frog && frog->isJumpingUp == false) {
        if (frog->growingScaleIndex == 3) {
            audio_pitch = D_80A1D0E4[frogIndex]; 
        } else { 
            audio_pitch = D_80A1D0DC[frogIndex];
        }
        if (this->songIndex == 6) { 
...
void EnFr_ActivateCheckFrogSong(EnFr* this, GlobalContext* globalCtx) {
    ...
    if (D_80A1CFF0.flags == 0xB) {
        ...
        if ((gSaveContext.eventChkInf[13] & 2)           // ZL (Purple Frog)
            ...
            this->songIndex = 6;
            // [sound 28E4]
            // Look at us, we're all huge frogs!! We are the world famous Fabulous Five Froggish Tenors!! Rrrribbit!!
            // Make us jump so we can eat the bugs flying above us, ribbit! OK? Then, ready, set, go
...
void EnFr_ChildSong(EnFr *this, GlobalContext *globalCtx) {
    ...
    if (this->jumpCounter < 48) {
        ...
        if (!(this->jumpCounter & 3)) { 
            ...
            EnFr_InitJumpingUp(this, D_80A1D0EC[(this->jumpCounter >> 2) & 7]); 
        }
    } 
        ...
        if (songIndex == 5) {
   	        //[sound 28E3]
            //Wow, that melody is so cool! Ribbit!
            //Siiiiingin' in the raaaaaain, Oh what a feeling! Rrrrrribbbit!!
            //Please take this as a token of our froggish gratitude, ribbit! All right. See you, ribbit!
            this->actor.textId = 0x40AA;
            EnFr_InitReward(this, globalCtx, false);
        }

        // ZL, Epona, Saria, Suns, SoT
        else if ((gSaveContext.eventChkInf[13] & D_80A1D008[songIndex]) == 0) {
            frog = D_80A1CFF0.frogs[D_80A1D020[songIndex]];
            ...
            // [sound 28E4]
            // Young lad, you play the Ocarina well... Mmmmm...! That melody is so fine...ribbit! We all should practice it, ribbit!
            // Take Rupees as a souvenir! If you come up with another nice melody, please drop by and play it...Ribbit ribbit! 
...
void EnFr_ChildSongFirstTime(EnFr* this, GlobalContext* globalCtx) {
    EnFr* frog;
    frog = D_80A1CFF0.frogs[D_80A1D020[this->songIndex]];
...
s32 EnFr_IsFrogSongComplete(EnFr *this, GlobalContext *globalCtx) {
    ...
    if (this->ocarinaNote == (*temp_msgCtx).unk_E410) {
        ...
        this->frogSongTimer = D_80A1D0B0[index];
    }
    return false;
}
    ...
    if (this->frogSongTimer == 0) {
        ...
        if (globalCtx->msgCtx.msgMode == 0x33) {
            ...
            if (EnFr_IsFrogSongComplete(this, globalCtx)) {
                // [sound 28E3]
                // Young boy, you did grrreat! Perrrrrfect! We are stuffed, ribbit!
                // Would you take this token of our gratitude? Take Rupees as a souvenir!
                // If you come up with another nice melody, please drop by and play it...Ribbit ribbit!
                this->actor.textId = 0x40AC;
                EnFr_InitReward(this, globalCtx, false);
            }    
        }
...
void EnFr_SetReward(EnFr *this, GlobalContext *globalCtx) {
    ...
    if ((songIndex >= 0) && (songIndex < 5)) {
        if (!(gSaveContext.eventChkInf[13] & D_80A1D008[songIndex])) {
            gSaveContext.eventChkInf[13] |= D_80A1D008[songIndex];
...
void EnFr_Deactivate(EnFr *this, GlobalContext *globalCtx) {
    ...
    for(frogIndex=0;frogIndex<5;frogIndex++){
        ...
        frog_loop1 = D_80A1CFF0.frogs[frogIndex];
    ...
    for(frogIndex=0;frogIndex<5;frogIndex++){
        frog_loop2 = D_80A1CFF0.frogs[frogIndex];
        // Translation: There are no frogs!?
        if (!frog_loop2) {
            osSyncPrintf(VT_COL(RED, WHITE));
            osSyncPrintf("%s[%d]カエルがいない！？\n", "../z_en_fr.c", 1618);
...
void EnFr_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if ((limbIndex == 7) || (limbIndex == 8)) {
        ...
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_fr.c", 0x6CA), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnFr_Draw(Actor* thisx, GlobalContext *globalCtx) {
    ...
    Lights_PointNoGlowSetInfo(&this->lightInfo, this->posButterflyLight.x, this->posButterflyLight.y, this->posButterflyLight.z, 255, 255, 255, NoGlowRadius);
    gDPSetEnvColor(POLY_OPA_DISP++, D_80A1D054[frogindex].red, D_80A1D054[frogindex].green, D_80A1D054[frogindex].blue, 255);
    gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(D_80A1D0FC[this->eyeTexIndex])); 
    gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(D_80A1D0FC[this->eyeTexIndex])); 
    SkelAnime_DrawFlexOpa(globalCtx, this->skelAnime.skeleton, this->skelAnime.limbDrawTbl, this->skelAnime.dListCount,
    EnFr_OverrideLimbDraw, EnFr_PostLimbDraw, &this->actor);

    if (this->isButterflyDrawn) {
        Matrix_Translate(this->posButterfly.x, this->posButterfly.y, this->posButterfly.z, MTXMODE_NEW);
        Matrix_Scale(0.015f, 0.015f, 0.015f, MTXMODE_APPLY);
        Matrix_RotateRPY(this->actor.shape.rot.x, this->actor.shape.rot.y, this->actor.shape.rot.z, MTXMODE_APPLY);
        SkelAnime_DrawOpa(globalCtx, this->skelAnimeButterfly.skeleton, this->skelAnimeButterfly.limbDrawTbl, NULL, NULL, NULL);
    }
