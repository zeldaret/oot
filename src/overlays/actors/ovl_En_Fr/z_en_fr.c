...
void EnFr_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (Object_IsLoaded(&globalCtx->objectCtx, this->objBankIndex)) {
        ...
        // frog
        SkelAnime_InitFlex(globalCtx, &this->skelAnime, &object_fr_Skel_00B498, &object_fr_Anim_001534, this->jointTable, this->morphTable,
                           24);
        // butterfly
...
void EnFr_SetupJumpingOutOfWater(EnFr* this, GlobalContext* globalCtx) {
    if (sEnFrPointers.flags == sTimerJumpingOutOfWater[this->actor.params - 1]) {
        Animation_Change(&this->skelAnime, &object_fr_Anim_0007BC, 1.0f, 0.0f, Animation_GetLastFrame(&object_fr_Anim_0007BC), 2, 0.0f);
...
void EnFr_OrientOnLogSpot(EnFr* this, GlobalContext* globalCtx) {
    ...
    if ((rotYRemaining == 0) && (this->skelAnime.curFrame == this->skelAnime.endFrame)) {
        ...
        Animation_Change(&this->skelAnime, &object_fr_Anim_001534, 1.0f, 0.0f, Animation_GetLastFrame(&object_fr_Anim_001534), 0, 0.0f);
    }
}
...
void EnFr_JumpingUp(EnFr* this, GlobalContext* globalCtx) {
    ...
    if (EnFr_IsBelowLogSpot(this, &yDistToLogSpot)) {
        this->isJumpingUp = false;
        this->actor.gravity = 0.0f;
        Animation_Change(&this->skelAnime, &object_fr_Anim_0011C0, 1.0f, 0.0f, Animation_GetLastFrame(&object_fr_Anim_0011C0), 0, 0.0f);
        this->actionFunc = EnFr_ChooseJumpFromLogSpot;
