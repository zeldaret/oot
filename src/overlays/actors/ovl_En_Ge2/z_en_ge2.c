...
void func_80A32BD0(EnGe2* this, s32 arg1) {
    static EnGe2ActionFunc D_80A3433C[] = {
    ...
    static AnimationHeader* D_80A34360[] = {
        0x06009ED4, 0x060098AC, 0x060098AC, 0x060011F4, 0x060098AC, 0x06008D60, 0x060098AC, 0x060098AC, 0x060098AC,
    };
    static u8 D_80A34384[] = {
        0x00, 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x02,
    };

    this->actionFunc = D_80A3433C[arg1];
    SkelAnime_ChangeAnim(&this->skelAnime, D_80A34360[arg1], 1.0f, 0.0f,
                         SkelAnime_GetFrameCount(&D_80A34360[arg1]->genericHeader), D_80A34384[arg1], -8.0f);
    this->unk_2F4 &= ~2;
}
...
void EnGe2_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_InitSV(globalCtx, &this->skelAnime, &D_06008968, NULL, this->limbDrawTable, this->transitionDrawTable,
                     22);
...
s32 func_80A32F74(GlobalContext* globalCtx, EnGe2* this, Vec3f* arg2, s16 arg3, f32 arg4) {
    ...
    if (func_8003E0B8(&globalCtx->colCtx, arg2, &player->unk_95C, &vec, &collisionPoly, 0)) {
        return 0;
    }
...
void func_80A3334C(EnGe2* this, GlobalContext* globalCtx) {
    ...
    static Color_RGB8 D_80A343A8 = { 0xFF, 0xFF, 0xFF };
    static Color_RGB8 D_80A343AC = { 0xFF, 0x96, 0x00 };
    ...
    this->actor.flags &= ~1;
    if (this->unk_2F4 & 2) {
        frames = globalCtx->state.frames * 10240;
        sp38.x = (Math_Coss(frames) * 5.0f) + this->actor.posRot2.pos.x;
        sp38.y = this->actor.posRot2.pos.y + 10.0f;
        sp38.z = (Math_Sins(frames) * 5.0f) + this->actor.posRot2.pos.z;
        func_80028BB0(globalCtx, &sp38, &D_80A34390, &D_80A3439C, &D_80A343A8, &D_80A343AC, 1000, 16);
    }
}
...
void func_80A33600(EnGe2* this, GlobalContext* globalCtx) {
    u8 temp_v0 = func_80A32ECC(globalCtx, this);
    ...
    if ((this->unk_2F4 & 4) || (this->unk_2F4 & 8)) {
        ...
        if (this->collider.base.acFlags & 2) {
            ...
            if (((this->actor.params & 0xFF) == 1) && (this->actor.xzDistFromLink < 100.0f)) {
                // I spy with my little eye, a Link
                osSyncPrintf(VT_FGCOL(GREEN) "発見!!!!!!!!!!!!\n" VT_RST);
                func_80A33B7C(this, globalCtx);
            }
        }
...
void func_80A3402C(Actor* thisx, GlobalContext* globalCtx) {
    ...
    // Yes they're both required Fig, I tried
    if (globalCtx) {};
...
s32 EnGe2_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnGe2* this = THIS;
...
void EnGe2_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
...
void EnGe2_Draw(Actor* thisx, GlobalContext* globalCtx) {
    static UNK_TYPE D_80A343BC[] = { 0x06004F78, 0x06005578, 0x06005BF8, 0x00000000, 0x00000000 };
    ...
    SkelAnime_DrawSV(globalCtx, skelAnime->skeleton, skelAnime->limbDrawTbl, skelAnime->dListCount,
                     EnGe2_OverrideLimbDraw, EnGe2_PostLimbDraw, &this->actor);
