...
void EnDekubaba_DrawStemExtended(EnDekubaba* this, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < stemSections; i++) {
        mtx.now_24 += 20.0f * Math_SinS(this->stemSectionAngle[i]) * this->size;
        horizontalStepSize = 20.0f * Math_CosS(this->stemSectionAngle[i]) * this->size;
        mtx.now_14 -= horizontalStepSize * Math_SinS(this->actor.shape.rot.y);
        mtx.now_34 -= horizontalStepSize * Math_CosS(this->actor.shape.rot.y);
        ...
        if (i == 0) {
            if (this->actionFunc != EnDekubaba_Sway) {
                this->actor.focus.pos.x = mtx.now_14;
                this->actor.focus.pos.y = mtx.now_24;
                this->actor.focus.pos.z = mtx.now_34;
            } else {
        ...
        if ((i < 2) && (this->actor.colorFilterTimer != 0)) {
            ...
            this->bodyPartsPos[i].x = mtx.now_14;
            this->bodyPartsPos[i].y = mtx.now_24 - spA4;
            this->bodyPartsPos[i].z = mtx.now_34;
        }
    }
