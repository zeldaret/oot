...
void EnGSwitch_DrawPot(Actor* thisx, PlayState* play) {
    ...
    if (!this->broken) {
        ...
        gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_g_switch.c", 925), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnGSwitch_DrawRupee(Actor* thisx, PlayState* play) {
    ...
    if (!this->broken) {
        ...
        gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_g_switch.c", 957), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EnGSwitch_DrawEffects(EnGSwitch* this, PlayState* play) {
    ...
    for (i = 0; i < this->numEffects; i++, effect++) {
        if (effect->flag) {
            ...
            gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_g_switch.c", 1088), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
