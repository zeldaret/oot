...
void ObjBean_SetupPathCount(ObjBean* this, PlayState* play) {
    this->pathCount = play->setupPathList[PARAMS_GET(this->dyna.actor.params, 8, 0x1F)].count - 1;
    this->currentPointIndex = 0;
    this->nextPointIndex = 1;
}

void ObjBean_SetupPath(ObjBean* this, PlayState* play) {
    Path* path = &play->setupPathList[PARAMS_GET(this->dyna.actor.params, 8, 0x1F)];
...
void ObjBean_FollowPath(ObjBean* this, PlayState* play) {
    ...
    path = &play->setupPathList[PARAMS_GET(this->dyna.actor.params, 8, 0x1F)];
...
void ObjBean_Init(Actor* thisx, PlayState* play) {
    ...
    if (LINK_AGE_IN_YEARS == YEARS_ADULT) {
        if (Flags_GetSwitch(play, PARAMS_GET(this->dyna.actor.params, 0, 0x3F)) || (mREG(1) == 1)) {
            path = PARAMS_GET(this->dyna.actor.params, 8, 0x1F);
            if (path == 0x1F) {
    ...
    } else if ((Flags_GetSwitch(play, PARAMS_GET(this->dyna.actor.params, 0, 0x3F)) != 0) || (mREG(1) == 1)) {
