...
s32 DoorShutter_SetupDoor(DoorShutter* this, PlayState* play) {
    TransitionActorEntry* transitionEntry = &play->transiActorCtx.list[ACTOR_TRANSITION_INDEX(&this->dyna.actor)];
...
void DoorShutter_Destroy(Actor* thisx, PlayState* play) {
    ...
    if (this->dyna.actor.room >= 0) {
        s32 transitionActorId = ACTOR_TRANSITION_INDEX(&this->dyna.actor);
...
void func_80997220(DoorShutter* this, PlayState* play) {
    ...
    if (this->dyna.actor.room >= 0) {
        ...
        this->dyna.actor.room =
            play->transiActorCtx.list[ACTOR_TRANSITION_INDEX(&this->dyna.actor)].sides[(vec.z < 0.0f) ? 0 : 1].room;
        ...
        if (this->unk_16C == 3) {
            ...
            if (sp70->b != NULL) {
                TransitionActorEntry* transitionEntry =
                    &play->transiActorCtx.list[ACTOR_TRANSITION_INDEX(&this->dyna.actor)];
