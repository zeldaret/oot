    ...
    if ((!(Flags_GetEventChkInf(5))) || LINK_IS_ADULT) {
        if (LINK_IS_CHILD) {
            if (Flags_GetEventChkInf(0xC)) {
                if (Actor_IsFacingPlayerAndNearby(&this->dyna.actor, 1658.0f, 0x7530)) {
            ...
            } else if (Actor_IsFacingPlayerAndNearby(&this->dyna.actor, 1658.0f, 0x4E20)) {
...
void func_808BC9EC(BgTreemouth* this, GlobalContext* globalCtx) {
    ...
    if (globalCtx->csCtx.state == CS_STATE_UNSKIPPABLE_INIT) {
        if (Actor_IsFacingPlayerAndNearby(&this->dyna.actor, 350.0f, 0x7530)) {
