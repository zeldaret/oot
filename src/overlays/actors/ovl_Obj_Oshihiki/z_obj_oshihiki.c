...
ObjOshihiki* ObjOshihiki_GetBlockUnder(ObjOshihiki* this, PlayState* play) {
    ...
    if ((this->floorBgIds[this->highestFloor] != BGCHECK_SCENE) &&
        (fabsf(this->bg.actor.floorHeight - this->bg.actor.world.pos.y) < 0.001f)) {
        bgActor = DynaPoly_GetActor(&play->colCtx, this->floorBgIds[this->highestFloor]);
        if ((bgActor != NULL) && (bgActor->actor.id == ACTOR_OBJ_OSHIHIKI)) {
            return (ObjOshihiki*)bgActor;
        }
    }
    return NULL;
}
...
void ObjOshihiki_UpdateInitPos(ObjOshihiki* this) {
    if (this->bg.actor.home.pos.x < this->bg.actor.world.pos.x) {
        while ((this->bg.actor.world.pos.x - this->bg.actor.home.pos.x) >= 20.0f) {
            this->bg.actor.home.pos.x += 20.0f;
        }
    } else {
        while ((this->bg.actor.home.pos.x - this->bg.actor.world.pos.x) >= 20.0f) {
            this->bg.actor.home.pos.x -= 20.0f;
        }
    }
    if (this->bg.actor.home.pos.z < this->bg.actor.world.pos.z) {
        while ((this->bg.actor.world.pos.z - this->bg.actor.home.pos.z) >= 20.0f) {
            this->bg.actor.home.pos.z += 20.0f;
        }
    } else {
        while ((this->bg.actor.home.pos.z - this->bg.actor.world.pos.z) >= 20.0f) {
            this->bg.actor.home.pos.z -= 20.0f;
        }
    }
}

s32 ObjOshihiki_NoSwitchPress(ObjOshihiki* this, BgActor* bgActor, PlayState* play) {
    s16 switchFlag;

    if (bgActor == NULL) {
        return 1;
    } else if (bgActor->actor.id == ACTOR_OBJ_SWITCH) {
        switchFlag = (bgActor->actor.params >> 8) & 0x3F;
        switch (bgActor->actor.params & 0x33) {
            case 0x20: // Normal blue switch
                if ((switchFlag == ((this->bg.actor.params >> 8) & 0x3F)) && Flags_GetSwitch(play, switchFlag)) {
                    return 0;
                }
                break;
...
void ObjOshihiki_SetScale(ObjOshihiki* this, PlayState* play) {
    Actor_SetScale(&this->bg.actor, sScales[this->bg.actor.params & 0xF]);
}
...
void ObjOshihiki_SetColor(ObjOshihiki* this, PlayState* play) {
    ...
    paramsColorIdx = (this->bg.actor.params >> 6) & 3;
...
void ObjOshihiki_Init(Actor* thisx, PlayState* play2) {
    ...
    if ((((this->bg.actor.params >> 8) & 0xFF) >= 0) && (((this->bg.actor.params >> 8) & 0xFF) <= 0x3F)) {
...
void ObjOshihiki_SetFloors(ObjOshihiki* this, PlayState* play) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        colCheckOffset.x = sColCheckPoints[i].x * (this->bg.actor.scale.x * 10.0f);
        colCheckOffset.y = sColCheckPoints[i].y * (this->bg.actor.scale.y * 10.0f);
        colCheckOffset.z = sColCheckPoints[i].z * (this->bg.actor.scale.z * 10.0f);
        ObjOshihiki_RotateXZ(&colCheckPoint, &colCheckOffset, this->yawSin, this->yawCos);
        colCheckPoint.x += this->bg.actor.world.pos.x;
        colCheckPoint.y += this->bg.actor.prevPos.y;
        colCheckPoint.z += this->bg.actor.world.pos.z;

        floorPoly = &this->floorPolys[i];
        floorBgId = &this->floorBgIds[i];
        this->floorHeights[i] =
            BgCheck_EntityRaycastDown6(&play->colCtx, floorPoly, floorBgId, &this->bg.actor, &colCheckPoint, 0.0f);
    }
}
...
void ObjOshihiki_SetGround(ObjOshihiki* this, PlayState* play) {
    ...
    this->bg.actor.floorHeight = this->floorHeights[this->highestFloor];
}
...
s32 ObjOshihiki_CheckGround(ObjOshihiki* this, PlayState* play) {
    if (this->bg.actor.world.pos.y <= BGCHECK_Y_MIN + 10.0f) {
...
s32 ObjOshihiki_CheckWall(PlayState* play, s16 angle, f32 direction, ObjOshihiki* this) {
    f32 maxDist = ((direction >= 0.0f) ? 1.0f : -1.0f) * (300.0f * this->bg.actor.scale.x + 20.0f - 0.5f);
    ...
    for (i = 0; i < 4; i++) {
        ...
        faceVtxOffset.x = (sFaceVtx[i].x * this->bg.actor.scale.x * 10.0f) + sFaceDirection[i].x;
        faceVtxOffset.y = (sFaceVtx[i].y * this->bg.actor.scale.y * 10.0f) + sFaceDirection[i].y;
        faceVtxOffset.z = 0.0f;
        ObjOshihiki_RotateXZ(&faceVtx, &faceVtxOffset, sn, cs);
        faceVtx.x += this->bg.actor.world.pos.x;
        faceVtx.y += this->bg.actor.world.pos.y;
        faceVtx.z += this->bg.actor.world.pos.z;
...
s32 ObjOshihiki_MoveWithBlockUnder(ObjOshihiki* this, PlayState* play) {
    ...
    if ((blockUnder != NULL) && (blockUnder->stateFlags & PUSHBLOCK_SETUP_PUSH) &&
        !ObjOshihiki_CheckWall(play, blockUnder->bg.unk_158, blockUnder->direction, this)) {
    ...
    if ((this->stateFlags & PUSHBLOCK_MOVE_UNDER) && (this->blockUnder != NULL)) {
        if (this->blockUnder->stateFlags & PUSHBLOCK_PUSH) {
            this->underDistX = this->blockUnder->bg.actor.world.pos.x - this->blockUnder->bg.actor.prevPos.x;
            this->underDistZ = this->blockUnder->bg.actor.world.pos.z - this->blockUnder->bg.actor.prevPos.z;
            this->bg.actor.world.pos.x += this->underDistX;
            this->bg.actor.world.pos.z += this->underDistZ;
...
void ObjOshihiki_SetupOnScene(ObjOshihiki* this, PlayState* play) {
    ...
    this->bg.actor.velocity.x = this->bg.actor.velocity.y = this->bg.actor.velocity.z = 0.0f;
}
...
void ObjOshihiki_OnScene(ObjOshihiki* this, PlayState* play) {
    ...
    if ((this->timer <= 0) && (fabsf(this->bg.unk_150) > 0.001f)) {
        if (ObjOshihiki_StrongEnough(this) && !ObjOshihiki_CheckWall(play, this->bg.unk_158, this->bg.unk_150, this)) {
            this->direction = this->bg.unk_150;
    ...
    if (ObjOshihiki_CheckFloor(this, play)) {
        ...
        if (bgId == BGCHECK_SCENE) {
            ...
            if (bgActor != NULL) {
                ...
                if ((this->timer <= 0) && (fabsf(this->bg.unk_150) > 0.001f)) {
                    if (ObjOshihiki_StrongEnough(this) && ObjOshihiki_NoSwitchPress(this, bgActor, play) &&
                        !ObjOshihiki_CheckWall(play, this->bg.unk_158, this->bg.unk_150, this)) {
        ...
        if (bgId == BGCHECK_SCENE) {
            ...
            if ((bgActor != NULL) && (bgActor->unk_15C & 1)) {
                BgActor_SetActorOnTop(bgActor);
                func_80043538(bgActor);
                this->bg.actor.world.pos.y = this->bg.actor.floorHeight;
            } else {
...
void ObjOshihiki_Push(ObjOshihiki* this, PlayState* play) {
    ...
    if (!ObjOshihiki_CheckFloor(this, play)) {
        ...
        if (ObjOshihiki_CheckWall(play, this->bg.unk_158, this->bg.unk_150, this)) {
...
void ObjOshihiki_Fall(ObjOshihiki* this, PlayState* play) {
    ...
    if (ObjOshihiki_CheckGround(this, play)) {
        ...
        Audio_PlayActorSfx2(&this->bg.actor, SurfaceType_GetSfxId(&play->colCtx, this->floorPolys[this->highestFloor],
...
void ObjOshihiki_Update(Actor* thisx, PlayState* play) {
    ...
    this->yawSin = Math_SinS(this->bg.actor.world.rot.y);
    this->yawCos = Math_CosS(this->bg.actor.world.rot.y);
