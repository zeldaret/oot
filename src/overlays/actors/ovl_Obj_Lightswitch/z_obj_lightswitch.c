...
void ObjLightswitch_SetSwitchFlag(ObjLightswitch* this, GlobalContext* globalCtx) {
    ...
    if (!Flags_GetSwitch(globalCtx, this->actor.params >> 8 & 0x3F)) {
        ...
        if (type == OBJLIGHTSWITCH_TYPE_1) {
            OnePointDemo_AttentionSetSfx(globalCtx, thisx, NA_SE_SY_TRE_BOX_APPEAR);
        } else if (type == OBJLIGHTSWITCH_TYPE_BURN) {
            OnePointDemo_AttentionSetSfx(globalCtx, thisx, NA_SE_SY_ERROR);
        } else {
            OnePointDemo_AttentionSetSfx(globalCtx, thisx, NA_SE_SY_CORRECT_CHIME);
        }
    }
}
...
void ObjLightswitch_ClearSwitchFlag(ObjLightswitch* this, GlobalContext* globalCtx) {
    if (Flags_GetSwitch(globalCtx, this->actor.params >> 8 & 0x3F)) {
        ...
        if ((this->actor.params >> 4 & 3) == OBJLIGHTSWITCH_TYPE_1) {
            OnePointDemo_AttentionSetSfx(globalCtx, &this->actor, NA_SE_SY_TRE_BOX_APPEAR);
        }
    }
}
