...
void Fishing_DrawFish(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->unk_150 == 0) {
        Matrix_RotateY(BINANG_TO_RAD_ALT(this->unk_16C) - (M_PI / 2), MTXMODE_APPLY);
        ...
        Matrix_RotateY(BINANG_TO_RAD_ALT(this->unk_16C), MTXMODE_APPLY);
        Matrix_Translate(0.0f, 0.0f, -3000.0f, MTXMODE_APPLY);
        Matrix_RotateY(-(M_PI / 2), MTXMODE_APPLY);
    ...
    for (i = 0; i < POND_PROP_COUNT; i++) {
        if (prop->type == FS_PROP_LILY_PAD) {
            ...
            if (prop->shouldDraw) {
                ...
                Matrix_RotateY(BINANG_TO_RAD_ALT(prop->lilyPadAngle), MTXMODE_APPLY);
