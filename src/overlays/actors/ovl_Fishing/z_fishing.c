...
//the score value for this minigame also stores many flags.
#define HS_FISHWEIGHT_CHILD 0x7f
#define HS_FISHWEIGHT_ADULT 0x7F000000
#define HS_PLAYED_CHILD 0x100
#define HS_PLAYED_ADULT 0x200
#define HS_PRIZE_CHILD 0x400
#define HS_PRIZE_ADULT 0x800
#define HS_STOLE_HAT 0x1000
#define HS_SINKINGLURECATCH_CHILD 0x80
#define HS_SINKINGLURECATCH_ADULT 0x80000000
...
typedef enum {
    FS_LURE_STOCK,
    FS_LURE_UNK, //hinted at with an "== 1"
    FS_LURE_SINKING,
} FishingLureTypes;
...
static f32 sFishingStormStep = 0.0f;

static u8 sFishingStormTimer = 0;
...
static f32 sFishCurrentCaughWeight = 0.0f;

static u8 D_80B7A674 = true;

static u16 sFishingCaughtWeight = 0;
...
static u8 sOwnerHatTheft = 0;
static u8 sOwnerHatHooked = 0;
...
static s16 sStickxCopy = 0;
static s16 sStickYCopy = 0;

static u8 D_80B7A6CC = 0;
static u8 D_80B7A6D0 = 0;
static u8 sSinkingLureFound = 0;
...
static f32 sFishingRecordWeight;
static u8 sFishingCaughtLoach;
...
void Fishing_Init(Actor* thisx, PlayState* play2) {
    ...
    if (thisx->params < 100) {
        ...
        if (sLinkAge != LINK_AGE_CHILD) {
            if (HIGH_SCORE(HS_FISHING) & HS_STOLE_HAT) {
        ...
        if (sLinkAge == LINK_AGE_CHILD) {
            if ((HIGH_SCORE(HS_FISHING) & HS_FISHWEIGHT_CHILD) != 0) {
                sFishingRecordWeight = HIGH_SCORE(HS_FISHING) & HS_FISHWEIGHT_CHILD;
            } else {
                sFishingRecordWeight = 40.0f; //6lbs
            }
        } else {
            if ((HIGH_SCORE(HS_FISHING) & HS_FISHWEIGHT_ADULT) != 0) {
                sFishingRecordWeight = (HIGH_SCORE(HS_FISHING) & HS_FISHWEIGHT_ADULT) >> 0x18;
            } else {
                sFishingRecordWeight = 45.0f; //7lbs
            }
        }
        ...
        if (thisx->params == 200) {
            ...
            this->preception = sFishInits[thisx->params - 100].unk_0C;
            this->fishWeight = sFishInits[thisx->params - 100].fishWeight;

            this->fishWeight += Rand_ZeroFloat(4.99999f);

            if ((this->fishWeight >= 65.0f) && (Rand_ZeroOne() < 0.05f)) {
                this->fishWeight += Rand_ZeroFloat(7.99999f);
            }

            if (KREG(6) != 0) {
                this->fishWeight = KREG(6) + 80.0f;
            }

            if (sLinkAge == LINK_AGE_CHILD) {
                this->fishWeight *= 0.73f;
            }
        }
    }
}
    ...
    for (i = 0; i < FISHING_EFFECT_COUNT; i++) {
        if (effect->type) {
            ...
            if (effect->type == FS_EFF_RIPPLE) {
                ...
                if (effect->unk_2C == 30) { //"Why did you do that? you owe me 50 rupees!"
...
void Fishing_UpdateLine(PlayState* play, Vec3f* basePos, Vec3f* pos, Vec3f* rot, Vec3f* unk) {
    ...
    if (D_80B7A6A4 != 0) {
        ...
        sRodLineReeledIn = 200.0f - (phi_f18 * 200.0f * 0.001f);
    }

    temp_s2 = sRodLineReeledIn;
    posSrc.z = 5.0f;
...
void Fishing_DrawLureHook(PlayState* play, Vec3f* pos, Vec3f* refPos, u8 hookIndex) {
    ...
    if ((D_80B7A694 == 3) && ((pos->y > WATER_SURFACE_Y(play)) || ((sOwnerHatTheft != 0) && hookIndex))) {
        offsetY = 0.0f;
...
void Fishing_DrawLureAndLine(PlayState* play, Vec3f* linePos, Vec3f* lineRot) {
    ...
    if ((D_80B7A694 == 4) || (D_80B7A694 == 5)) {
        ...
        if ((D_80B7A694 == 5) && (sLureEquipped == FS_LURE_SINKING)) {
            Matrix_RotateY(BINANG_TO_RAD(player->actor.shape.rot.y), MTXMODE_NEW);
            posSrc.x = 2.0f;
            posSrc.y = 0.0f;
            posSrc.z = 0.0f;
    ...
    if ((D_80B7A694 == 4) && ((D_80B7E124 != 0) || (sLureEquipped != FS_LURE_SINKING))) {
        f32 rx;
        f32 ry;
        f32 dist;
        f32 dx;
        ...
        for (i = spB4; i < LINE_SEG_COUNT - 1; i++) {
            if ((i == LINE_SEG_COUNT - 3) && (sLureEquipped == FS_LURE_STOCK) && (D_80B7A694 == 3)) {
                f32 rx;
                f32 ry;
                f32 dist;
                f32 dx;
...
void Fishing_DrawRod(PlayState* play) {
    ...
    if ((D_80B7A694 == 3) || (D_80B7A694 == 4)) {
        if ((input->rel.stick_x == 0) && (sStickxCopy != 0)) {
            D_80B7A6B0 = 0.0f;
        }
        if ((input->rel.stick_y == 0) && (sStickYCopy != 0)) {
            D_80B7A6B4 = 0.0f;
        }
...
void Fishing_UpdateLure(Fishing* this, PlayState* play) {
    ...
    if (D_80B7E0AC == 1) {
        ...
        if (((sLinkAge == LINK_AGE_CHILD) && (HIGH_SCORE(HS_FISHING) & HS_PRIZE_CHILD)) ||
            ((sLinkAge != LINK_AGE_CHILD) && (HIGH_SCORE(HS_FISHING) & HS_PRIZE_ADULT))) {
            sSinkingLureLocation = (u8)Rand_ZeroFloat(3.999f) + 1;
        }

        D_80B7E148 = 520.0f;
        sRodLineReeledIn = 195.0f;

        D_80B7A694 = sLureEquipped = D_80B7E0AE = D_80B7E0B0 = D_80B7E0B2 = D_80B7E0B4 = D_80B7E120 = D_80B7E114 =
            D_80B7E150 = 0;
        D_80B7E104 = sReelLinePosStep = D_80B7E108 = 0.0f;
    ...
    switch (D_80B7A694) {
        case 0:
            ...
            Math_ApproachF(&sRodLineReeledIn, 195.0f, 1.0f, 1.0f);
        ...
        case 1: //casting the line
            ...
            sLurePosDelta.x += D_80B7E0F8.x;
            sLurePosDelta.y += D_80B7E0F8.y;
            sLurePosDelta.z += D_80B7E0F8.z;

            if (CHECK_BTN_ALL(input->cur.button, BTN_A) || (sOwnerHatTheft != 0)) {
            ...
            sRodLineReeledIn = 200.0f - (phi_f16 * 200.0f * 0.001f);
        ...
        case 3:
            ...
            if ((sOwnerHatTheft != 0) && ((SQ(sLurePos.x) + SQ(sLurePos.z)) < SQ(500.0f))) {
                sOwnerHatHooked = 1;
            }
                ...
                if (sLurePos.y <= (spE4 + 4.0f)) {
                    ...
                    if (D_80B7E150 == 0) {
                        if (fabsf(input->rel.stick_x) > 30.0f) {
                            sp70 = fabsf((input->rel.stick_x - sStickxCopy) * (1.0f / 60.0f));
                        } else if (fabsf(input->rel.stick_y) > 30.0f) {
                            sp70 = fabsf((input->rel.stick_y - sStickYCopy) * (1.0f / 60.0f));
                        }
                    }
                    ...
                    if ((sp70 > 0.2f) && (D_80B7E138 < 4.0f)) {
                        ...
                        sLureRot.x = 0.0f;
                        D_80B7E10C = 0.5f;
                        sRodLineReeledIn += (fabsf(sp70) * (7.5f + (KREG(25) * 0.1f)));

                        func_800F436C(&sSoundPos, NA_SE_EV_LURE_MOVE_W, (sp70 * 1.999f * 0.25f) + 0.75f);
                    ...
                    if (sRodLineReeledIn > 150.0f) {
                        sLureRot.x = sReelLineRot[LINE_SEG_COUNT - 2].x + M_PI;
                        D_80B7E134 = sReelLineRot[LINE_SEG_COUNT - 2].y + M_PI;
                        sRodLineReeledIn += 2.0f;
                    }
                }
            } else {
                spE4 = ((sqrtf(spC8) - 920.0f) * 0.11f) + WATER_SURFACE_Y(play);
            ...
            sLurePosDelta.x = sLurePosDelta.y = sLurePosDelta.z = D_80B7E0F8.y = 0.0f;

            if (CHECK_BTN_ALL(input->cur.button, BTN_A)) {
                if (CHECK_BTN_ALL(input->cur.button, BTN_R)) {
                    sRodLineReeledIn += 1.5f;
                    func_80078884(NA_SE_IT_FISHING_REEL_HIGH - SFX_FLAG);
                    Math_ApproachF(&sReelLinePosStep, 1000.0f, 1.0f, 2.0f);
                } else {
                    sRodLineReeledIn += sRodReelingSpeed;
        ...
        case 4:
            ...
            if (CHECK_BTN_ALL(input->cur.button, BTN_A)) {
                if ((SQ(sLurePos.x) + SQ(sLurePos.z)) > SQ(920.0f)) {
                    sRodLineReeledIn += (1.0f + (KREG(65) * 0.1f));
                } else {
                    sRodLineReeledIn += sRodReelingSpeed;
                }
                func_80078884(NA_SE_IT_FISHING_REEL_SLOW - SFX_FLAG);
            ...
            if ((D_80B7E0AE & 0x1F) == 0) {
                if ((D_80B7E124 != 0) || (sLureEquipped != FS_LURE_SINKING)) {
                    D_80B7A6A4 = 5;
                }
            }
...
void func_80B70ED4(Fishing* this, Input* input) {
    ...
    lineLength = SQ(lineVec.x) + SQ(lineVec.y) + SQ(lineVec.z);

    if ((D_80B7A694 == 3) && (this->unk_1A2 == 0) && (sOwnerHatTheft == 0)) {
        Matrix_RotateY(BINANG_TO_RAD_ALT(-this->actor.shape.rot.y), MTXMODE_NEW);
        Matrix_MultVec3f(&lineVec, &sp28);

        if ((sp28.z > 0.0f) || (this->fishWeight < 40.0f)) {
            if ((this->unk_158 == 7) && (lineLength < SQ(200.0f))) {
                this->unk_158 = 4;
                this->fishTargetPos = sLurePos;
                this->unk_1B0 = 28672.0f;
                this->unk_188 = 5.0f;
            } else {
                if ((CHECK_BTN_ALL(input->cur.button, BTN_A) || (D_80B7E138 > 1.0f)) && (lineLength < SQ(120.0f))) {
                    this->unk_158 = 2;
                    this->unk_15E = 0;
                    this->unk_17A[0] = 0;
                    this->unk_17A[2] = (s16)Rand_ZeroFloat(100.0f) + 100;
                    this->preception = sFishInits[this->actor.params - 100].unk_0C;
                    this->unk_1B0 = 0.0f;
                }

                if ((this->unk_17A[1] == 0) && (lineLength < SQ(70.0f))) {
                    this->unk_158 = 2;
                    this->unk_15E = 0;
                    this->unk_17A[0] = 0;
                    this->unk_17A[2] = (s16)Rand_ZeroFloat(100.0f) + 100;
    ...
    } else if ((D_80B7A694 == 4) && (D_80B7E124 != 0) && (lineLength < SQ(100.0f)) && (this->unk_158 >= 10)) {
        this->unk_15A = 0;
        this->unk_158 = 1;
        this->unk_1A4 = 1000;
        this->unk_1A2 = 100;
    ...
    if ((sLureEquipped != FS_LURE_SINKING) && (D_80B7E114 != 0) && (this->fishWeight > 60.0f) && (lineLength < SQ(30.0f)) &&
        (this->unk_158 >= 10)) {
        this->unk_15A = 0;
        this->unk_158 = 1;
        this->unk_1A4 = 1000;
...
void Fishing_FishLeapSfx(Fishing* this, u8 outOfWater) {
    ...
    if (this->isLoach == 0) {
        mass = this->fishWeight;
    } else {
        mass = 2.0f * this->fishWeight;
    }
...
void Fishing_HandleAquariumDialog(Fishing* this, PlayState* play) {
    if (sLinkAge == LINK_AGE_CHILD) {
        if ((HIGH_SCORE(HS_FISHING) & HS_FISHWEIGHT_CHILD) != 0) {
            if (HIGH_SCORE(HS_FISHING) & HS_SINKINGLURECATCH_CHILD) {
        ...
        if ((HIGH_SCORE(HS_FISHING) & HS_FISHWEIGHT_ADULT) != 0) {
            if (HIGH_SCORE(HS_FISHING) & HS_SINKINGLURECATCH_ADULT) {
                this->actor.textId = 0x40B1;
            } else {
                this->actor.textId = 0x4089;
            }
    ...
    if (this->unk_1D3 == 0) {
        if (this->unk_1D4 == 0) {
            ...
            if (Actor_ProcessTalkRequest(&this->actor, play)) {
                sFishingCaughtWeight = sFishingRecordWeight;
                this->unk_1D3 = 1;
            } else {
...
void Fishing_UpdateFish(Actor* thisx, PlayState* play2) {
    ...
    if (this->isLoach == 0) {
        Actor_SetScale(&this->actor, this->fishWeight * 15.0f * 0.00001f);
        ...
        Actor_SetScale(&this->actor, this->fishWeight * 65.0f * 0.000001f);
    ...
    switch (this->unk_158) {
        case 100:
            ...
            this->fishWeight = sFishingRecordWeight;
        ...
        case 2:
            ...
            this->fishTargetPos.x = sLurePos.x + sp100.x;
            this->fishTargetPos.z = sLurePos.z + sp100.z;
            ...
            if (sp124 < ((this->fishWeight * 0.5f) + 20.0f)) {
                if (this->unk_15E == 0) {
                    this->unk_190 = 1.0f;
                    this->unk_194 = 500.0f;
            ...
            if (this->fishWeight >= 60.0f) {
                multiplier = 0.3f;
            } else if (this->fishWeight >= 45.0f) {
                multiplier = 0.6f;
            } else {
                multiplier = 1.0f;
            }
            ...
            if ((gSaveContext.dayTime >= CLOCK_TIME(17, 0)) && (gSaveContext.dayTime < CLOCK_TIME(19, 0))) {
                ...
                multiplier *= 1.5f;
            } else if ((u8)sFishingStormStep != 0) {
                multiplier *= 3.0f;
            }
        ...
        case 4:
            ...
            if ((D_80B7A694 != 3) || (this->unk_17A[0] == 0) || (sLurePos.y > (WATER_SURFACE_Y(play) + 5.0f)) ||
                ...
                if (sLureEquipped == FS_LURE_SINKING) {
                    if (this->fishWeight > 70.0f) {
                        phi_v0 = (s16)Rand_ZeroFloat(20.0f) + 10;
                    } else if (this->fishWeight > 60.0f) {
                        phi_v0 = (s16)Rand_ZeroFloat(30.0f) + 20;
                    } else if (this->fishWeight > 50.0f) {
        ...
        case 5:
            ...
            if ((D_80B7E122 != 0) && (D_80B7E124 == 0)) {
                if (((input->rel.stick_y < -50) && (sStickYCopy > -40)) || CHECK_BTN_ALL(input->press.button, BTN_A)) {
                    if (input->rel.stick_y < -50) {
                        temp_f0 = 40.0f - ((this->fishWeight - 30.0f) * 1.333333f);
                        if (temp_f0 > 0.0f) {
                    ...
                    if (this->isLoach == 1) {
                        rumbleStr = (this->fishWeight * 3.0f) + 120.0f;
                    } else {
                        rumbleStr = (2.0f * this->fishWeight) + 120.0f;
                    }
                    if (rumbleStr > 255.0f) {
                        rumbleStr = 255.0f;
                    }
                ...
                if (this->unk_17A[1] > 30) {
                    if (this->unk_17A[0] == 0) {
                        ...
                        for (spA2 = 0; spA2 < 100; spA2++) {
                            ...
                            this->fishTargetPos.x = this->actor.world.pos.x + sp100.x;
                            this->fishTargetPos.z = this->actor.world.pos.z + sp100.z;
                    ...
                    if (this->unk_17A[2] != 0) {
                        ...
                        if (this->isLoach == 0) {
                            sRodReelingSpeed = 1.0f - (this->fishWeight * 0.00899f);
                        } else {
                            sRodReelingSpeed = 1.0f - (this->fishWeight * 0.00899f * 1.4f);
                        }
                    }
                } else {
                    if (((this->unk_17A[1] & 0xF) == 0) && CHECK_BTN_ALL(input->cur.button, BTN_A) &&
                        (!(this->fishWeight >= 60.0f) || (D_80B7E080 >= 2000))) {
                    ...
                    if (this->isLoach == 0) {
                        sRodReelingSpeed = 1.3f - (this->fishWeight * 0.00899f);
                    } else {
                        sRodReelingSpeed = 1.3f - (this->fishWeight * 0.00899f * 1.4f);
                    }
            ...
            if (D_80B7E124 || (sLureEquipped != FS_LURE_SINKING)) {
            ...
            if ((D_80B7A694 < 3) || ((sReelLock != 0) && (D_80B7E080 > 50)) || (D_80B7E080 >= 6000) ||
        ...
        case 6:
            ...
            if (this->unk_17A[0] == 90) {
                ...
                if (this->isLoach == 0) {
                    sFishingCaughtWeight = this->fishWeight;

                    if (sFishingCaughtWeight >= 75) {
                        sFishingCaughtTextId = 0x409F;
                    } else if (sFishingCaughtWeight >= 50) {
                        sFishingCaughtTextId = 0x4091;
                    } else {
                        sFishingCaughtTextId = 0x4083;
                    }
                } else {
                    sFishingCaughtWeight = 2.0f * this->fishWeight;
                        ...
                        if ((Message_GetState(&play->msgCtx) == TEXT_STATE_CHOICE) ||
                            ...
                            if (Message_ShouldAdvance(play)) {
                                ...
                                if (play->msgCtx.choiceIndex == 0) {
                                    if (sFishCurrentCaughWeight == 0.0f) {
                                        sFishCurrentCaughWeight = this->fishWeight;
                                        sFishingCaughtLoach = this->isLoach;
                                        sLureCaughtWith = sLureEquipped;
                                        Actor_Kill(&this->actor);
                                    } else if ((this->isLoach == 0) && (sFishingCaughtLoach == 0) &&
                                               ((s16)this->fishWeight < (s16)sFishCurrentCaughWeight)) {
                                        this->unk_1D5 = 1;
                                        this->unk_17A[0] = 0x3C;
                                        //"Are you sure? it's smaller than your current fish."
                                        Message_StartTextbox(play, 0x4098, NULL);
                                    } else {
                                        f32 temp1 = sFishCurrentCaughWeight;
                                        s16 temp2 = sFishingCaughtLoach;
                                        sFishCurrentCaughWeight = this->fishWeight;
                                        sFishingCaughtLoach = this->isLoach;
                                        sLureCaughtWith = sLureEquipped;
                                        this->fishWeight = temp1;
                                        this->isLoach = temp2;
                                    }
                                }
                                if (this->unk_1D5 == 0) {
    ...
    if (this->unk_1D2 != 0) {
        s16 i;
        Vec3f pos;
        f32 range = (this->fishWeight * 0.075f) + 10.0f;
...
void Fishing_HandleOwnerDialog(Fishing* this, PlayState* play) {
    switch (this->unk_15C) {
        case 0:
            if (D_80B7E0AC == 0) {
                if (sLinkAge != LINK_AGE_CHILD) {
                    if ((HIGH_SCORE(HS_FISHING) & HS_PLAYED_CHILD) && !(HIGH_SCORE(HS_FISHING) & HS_PLAYED_ADULT)) {
                        this->actor.textId = 0x4093; //greeted as returning customer
                    } else {
                        this->actor.textId = 0x407B; //normal greeting
                    }
                } else {
                    this->actor.textId = 0x407B; //normal greeting
                }
            } else if (sOwnerHatTheft == 0) {
                this->actor.textId = 0x4084; //normal Owner Menu when fishing
            } else {
                this->actor.textId = 0x4097; //owner asks for hat back.
            }
            ...
            if (Actor_ProcessTalkRequest(&this->actor, play)) {
                if (D_80B7E0AC == 0) {
                    this->unk_15C = 1;
                    if (sLinkAge != LINK_AGE_CHILD) {
                        HIGH_SCORE(HS_FISHING) |= HS_PLAYED_ADULT;
                    } else {
                        HIGH_SCORE(HS_FISHING) |= HS_PLAYED_CHILD;
                    }
                } else {
                    this->unk_15C = 10;
                }
                ...
                switch (play->msgCtx.choiceIndex) {
                    case 0:
                        if (gSaveContext.rupees >= 20) {
                            ...
                            Message_ContinueTextbox(play, 0x407E); //you need more money.
                            this->unk_15C = 3;
                        }
                        break;
                    case 1:
        ...
        case 2:
            if ((Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(play)) {
                Message_CloseTextbox(play);
                Message_ContinueTextbox(play, 0x407F); //rules of fishing
                this->unk_15C = 4;
            }
            break;
        ...
        case 4:
            if ((Message_GetState(&play->msgCtx) == TEXT_STATE_CHOICE) && Message_ShouldAdvance(play)) {
                ...
                switch (play->msgCtx.choiceIndex) {
                    case 0:
                        sFishingCaughtWeight = sFishingRecordWeight;
                        Message_ContinueTextbox(play, 0x4080); //"you can only carry one fish at a time."
                        this->unk_15C = 5;
                        break;
                    case 1:
                        Message_ContinueTextbox(play, 0x407F); //rules of fishing
                    ...
                    switch (play->msgCtx.choiceIndex) {
                        case 0:
                            if (sFishCurrentCaughWeight == 0.0f) {
                                ...
                                this->unk_15C = 20;
                            } else if (sFishingCaughtLoach == 0) {
                                sFishingCaughtWeight = sFishCurrentCaughWeight;
                                if ((s16)sFishingRecordWeight < (s16)sFishCurrentCaughWeight) {
        ...
        case 11: //collect prize, update record.
            if (((Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) ||
                ...
                if (sFishingCaughtLoach == 0) {
                    sFishingRecordWeight = sFishCurrentCaughWeight;
                    sFishCurrentCaughWeight = 0.0f;

                    if (sLinkAge == LINK_AGE_CHILD) {
                        f32 temp;

                        HIGH_SCORE(HS_FISHING) &= 0xFFFFFF00;
                        HIGH_SCORE(HS_FISHING) |= (s32)sFishingRecordWeight & HS_FISHWEIGHT_CHILD;

                        temp = (HIGH_SCORE(HS_FISHING) & HS_FISHWEIGHT_ADULT) >> 0x18;
                        if (temp < sFishingRecordWeight) {
                            HIGH_SCORE(HS_FISHING) &= 0xFFFFFF;
                            HIGH_SCORE(HS_FISHING) |= ((s32)sFishingRecordWeight & HS_FISHWEIGHT_CHILD) << 0x18;

                            if (sLureCaughtWith == FS_LURE_SINKING) {
                                HIGH_SCORE(HS_FISHING) |= HS_SINKINGLURECATCH_ADULT;
                            }
                        }

                        if (sLureCaughtWith == FS_LURE_SINKING) {
                            HIGH_SCORE(HS_FISHING) |= HS_SINKINGLURECATCH_CHILD;
                            this->unk_15C = 0;
                            break;
                        }
                    } else {
                    ...
                    if (sFishingRecordWeight >= 60.0f) {
                        getItemId = GI_RUPEE_PURPLE;
                    } else if (sFishingRecordWeight >= 50.0f) {
                        getItemId = GI_RUPEE_RED;
                    } else if (sFishingRecordWeight >= 40.0f) {
                    ...
                    if (sLinkAge == LINK_AGE_CHILD) {
                        if ((sFishingRecordWeight >= 50.0f) && !(HIGH_SCORE(HS_FISHING) & HS_PRIZE_CHILD)) {
                            HIGH_SCORE(HS_FISHING) |= HS_PRIZE_CHILD;
                            getItemId = GI_HEART_PIECE;
                            sSinkingLureLocation = (u8)Rand_ZeroFloat(3.999f) + 1;
                        }
                    } else {
                        if ((sFishingRecordWeight >= 60.0f) && !(HIGH_SCORE(HS_FISHING) & HS_PRIZE_ADULT)) {
                            HIGH_SCORE(HS_FISHING) |= HS_PRIZE_ADULT;
...
void Fishing_UpdateOwner(Actor* thisx, PlayState* play2) {
    ...
    if ((sOwnerHatTheft == 0) && (sLureEquipped != FS_LURE_SINKING) && (D_80B7A694 > 0) && (sOwnerHair == FS_OWNER_CAPPED) && (D_80B7A684 == 0)) {
    ...
    if (sOwnerHair == FS_OWNER_BALD) {
        HIGH_SCORE(HS_FISHING) |= HS_STOLE_HAT;
    ...
    if (sREG(15) != 0) {
        if (sFishingStormTimer != (sREG(15) - 1)) {
            if (sFishingStormTimer == 0) {
    ...
    if ((sStormChanceTimer == 1) && (Message_GetState(&play->msgCtx) == TEXT_STATE_NONE) && ((D_80B7A680 & 0xFFF) == 0xFFF)) {
        sStormChanceTimer = 200;

        if (Rand_ZeroOne() < 0.5f) {
            sFishingStormTimer = (u8)Rand_ZeroFloat(10.0f) + 5;
            play->envCtx.stormRequest = STORM_REQUEST_START;
        } else {
            sFishingStormTimer = 0;
            play->envCtx.stormRequest = STORM_REQUEST_STOP;
        }
    }

    Math_ApproachF(&sFishingStormStep, sFishingStormTimer, 1.0f, 0.05f);

    if (sFishingStormStep > 0.0f) {
        target = (sFishingStormStep * 0.03f) + 0.8f;
        if (target > 1.2f) {
            target = 1.2f;
        }
    ...
    target = (10.0f - sFishingStormStep) * 150.1f;
    if (target < 0.0f) {
        target = 0.0f;
    }
    if (1) {}
    ...
    if (sFishingStormSfxPos.z < 1500.0f) {
        func_800F436C(&sFishingStormSfxPos, NA_SE_EV_RAIN - SFX_FLAG, D_80B7A668);
    }
    ...
    if ((u8)sFishingStormStep > 0) {
        ...
        for (i = 0; i < (u8)sFishingStormStep; i++) {
    ...
    gSaveContext.minigameScore = (SQ((f32)sFishingCaughtWeight) * 0.0036f) + 0.5f;
...
void Fishing_DrawOwner(Actor* thisx, PlayState* play) {
    ...
    if ((D_80B7E0AC != 0) && D_80B7A674) {
        ...
        sStickxCopy = input->rel.stick_x;
        sStickYCopy = input->rel.stick_y;
    }
    ...
    if ((D_80B7E0AC != 0) && (sLureEquipped == FS_LURE_SINKING)) {
        Fishing_DrawSinkingLure(play);
    }
