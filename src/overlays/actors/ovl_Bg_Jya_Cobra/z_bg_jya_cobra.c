...
void func_80895A70(BgJyaCobra* this) {
    ...
    switch (PARAMS_GET(this->dyna.actor.params, 0, 2)) {
        case 0:
    ...
    if (this->unk_18C <= 0.0f) {
        ...
        Matrix_RotateX(BINANG_TO_RAD(D_80897308[PARAMS_GET(this->dyna.actor.params, 0, 2)]), MTXMODE_APPLY);
        sp28.x = 0.0f;
        sp28.y = 0.0;
...
void func_80895C74(BgJyaCobra* this, PlayState* play) {
    ...
    if (PARAMS_GET(params, 0, 2) == 2 && mirror != NULL &&
        ...
        this->unk_18C = 1.0f;
        if (D_80897310[PARAMS_GET(params, 0, 2)]) {
            phi_v0 = this->dyna.actor.shape.rot.y - D_80897314[PARAMS_GET(params, 0, 2)];
            phi_v0 = ABS(phi_v0);
    ...
    if (PARAMS_GET(params, 0, 2) == 0) {
        this->unk_190 = 0.1f;
    } else if (PARAMS_GET(params, 0, 2) == 1) {
        phi_f0 = 0.1f;
    ...
    } else if (PARAMS_GET(params, 0, 2) == 2) {
        phi_f0 = 0.1f;
        phi_v0 = this->dyna.actor.shape.rot.y - 0x8000;
        if (phi_v0 < 0x500 && phi_v0 > -0x500) {
            phi_f0 = 0.34f;
...
void BgJyaCobra_Init(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET(this->dyna.actor.params, 0, 2) == 1 || PARAMS_GET(this->dyna.actor.params, 0, 2) == 2) {
        this->dyna.actor.room = -1;
    }

    if (PARAMS_GET(this->dyna.actor.params, 0, 2) == 1) {
...
void BgJyaCobra_Update(Actor* thisx, PlayState* play2) {
    ...
    if (PARAMS_GET(this->dyna.actor.params, 0, 2) == 0 || PARAMS_GET(this->dyna.actor.params, 0, 2) == 2) {
...
void func_80896D78(BgJyaCobra* this, PlayState* play) {
    ...
    sp44.x = D_80897308[PARAMS_GET(this->dyna.actor.params, 0, 2)] + this->dyna.actor.shape.rot.x;
...
void BgJyaCobra_DrawShadow(BgJyaCobra* this, PlayState* play) {
    s32 pad;
    s16 params = PARAMS_GET(this->dyna.actor.params, 0, 2);
    Vec3f sp64;
    Vec3s* phi_a3;
...
void BgJyaCobra_Draw(Actor* thisx, PlayState* play) {
    ...
    if (PARAMS_GET(this->dyna.actor.params, 0, 2) == 2) {
        BgJyaBigmirror* mirror = (BgJyaBigmirror*)this->dyna.actor.parent;
