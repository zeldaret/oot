...
void func_80895A70(BgJyaCobra* this) {
    ...
    if (this->unk_18C <= 0.0f) {
        ...
        Matrix_RotateY(BINANG_TO_RAD_ALT(this->dyna.actor.shape.rot.y), MTXMODE_NEW);
        Matrix_RotateX(BINANG_TO_RAD_ALT(D_80897308[this->dyna.actor.params & 3]), MTXMODE_APPLY);
        sp28.x = 0.0f;
        sp28.y = 0.0;
...
void BgJyaCobra_UpdateShadowFromSide(BgJyaCobra* this) {
    ...
    Matrix_RotateY(BINANG_TO_RAD_ALT(rotY), MTXMODE_APPLY);
