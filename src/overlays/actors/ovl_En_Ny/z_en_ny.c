...
void EnNy_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_en_ny.c", 845);
    ...
    if (this->unk_1E0 > 0.25f) {
        ...
        gSPMATRIX_SET_NEW(POLY_OPA_DISP++, play->state.gfxCtx, "../z_en_ny.c", 868);
...
void EnNy_DrawDeathEffect(Actor* thisx, PlayState* play) {
    ...
    for (i = 0; i < 8; i++) {
        if (this->timer < (i + 22)) {
            ...
            gSPMATRIX_SET_NEW(POLY_OPA_DISP++, play->state.gfxCtx, "../z_en_ny.c", 912);
