...
void EnTuboTrap_HandleImpact(EnTuboTrap* this, GlobalContext* globalCtx) {
    ...
    if ((this->actor.bgCheckFlags & 0x20) && (this->actor.yDistToWater > 15.0f)) {
        ...
        AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 40, NA_SE_EV_BOMB_DROP_WATER);
    ...
    if (this->collider.base.atFlags & AT_BOUNCED) {
        ...
        AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 40, NA_SE_IT_SHIELD_REFLECT_SW);
        AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 40, NA_SE_EV_POT_BROKEN);
        EnTuboTrap_DropCollectible(this, globalCtx);
        Actor_Kill(&this->actor);
        return;
    }
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        EnTuboTrap_SpawnEffectsOnLand(this, globalCtx);
        AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 40, NA_SE_EV_EXPLOSION);
        AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 40, NA_SE_EV_POT_BROKEN);
        EnTuboTrap_DropCollectible(this, globalCtx);
        Actor_Kill(&this->actor);
        return;
    ...
    if (this->collider.base.atFlags & AT_HIT) {
        ...
        if (this->collider.base.at == &player->actor) {
            EnTuboTrap_SpawnEffectsOnLand(this, globalCtx);
            AudioSource_CreateSoundSource(globalCtx, &this->actor.world.pos, 40, NA_SE_EV_POT_BROKEN);
            AudioSource_CreateSoundSource(globalCtx, &player2->actor.world.pos, 40, NA_SE_PL_BODY_HIT);
            EnTuboTrap_DropCollectible(this, globalCtx);
            Actor_Kill(&this->actor);
            return;
        }
