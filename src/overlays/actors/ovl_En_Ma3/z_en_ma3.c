...
u16 func_80AA2AA0(GlobalContext* globalCtx, Actor* thisx) {
    ...
    if (!(gSaveContext.save.info.infTable[11] & 0x100)) {
        return 0x2000;
    }
    ...
    if (gSaveContext.eventInf[0] & 0x400) {
        ...
        if ((gSaveContext.save.info.horseRaceRecord == 0) || (gSaveContext.save.info.horseRaceRecord >= 0xB4)) {
            gSaveContext.save.info.horseRaceRecord = 0xB4;
            gSaveContext.timer1Value = *timer1ValuePtr;
        }
        if (!(gSaveContext.save.info.eventChkInf[1] & 0x4000) && (gSaveContext.timer1Value < 0x32)) {
            return 0x208F;
        } else if (gSaveContext.timer1Value < gSaveContext.save.info.horseRaceRecord) {
            return 0x2012;
        } else {
            return 0x2004;
        }
    ...
    if (!(gSaveContext.save.info.infTable[11] & 0x200)) {
        return 0x2002;
    } else {
        return 0x2003;
    }
...
s16 func_80AA2BD4(GlobalContext* globalCtx, Actor* thisx) {
    ...
    switch (func_8010BDBC(&globalCtx->msgCtx)) {
        ...
        case 4:
            if (func_80106BC8(globalCtx) != 0) {
                gSaveContext.save.info.infTable[11] |= 0x200;
                if (globalCtx->msgCtx.choiceIndex == 0) {
                    if (gSaveContext.save.info.eventChkInf[1] & 0x4000) {
                        func_8010B720(globalCtx, 0x2091);
                    } else if (gSaveContext.save.info.horseRaceRecord == 0) {
        ...
        case 2:
            switch (thisx->textId) {
                case 0x2000:
                    gSaveContext.save.info.infTable[11] |= 0x100;
                    ret = 0;
                    break;
                case 0x208F:
                    gSaveContext.save.info.eventChkInf[1] |= 0x4000;
                case 0x2004:
                case 0x2012:
                    if (gSaveContext.save.info.horseRaceRecord > gSaveContext.timer1Value) {
                        gSaveContext.save.info.horseRaceRecord = gSaveContext.timer1Value;
                    }
                case 0x208E:
                    gSaveContext.eventInf[0] &= ~0x400;
                    thisx->flags &= ~0x10000;
...
s32 func_80AA2EC8(EnMa3* this, GlobalContext* globalCtx) {
    if (gSaveContext.save.linkAge == 1) {
        return 2;
    }
    if (!(gSaveContext.save.info.eventChkInf[1] & 0x100)) {
        return 2;
    }
    if (gSaveContext.eventInf[0] & 0x400) {
        return 1;
