...
u16 func_80AA2AA0(GlobalContext* globalCtx, Actor* thisx) {
    ...
    if (gSaveContext.eventInf[0] & 0x400) {
        ...
        if ((HIGHSCORE(HIGHSCORE_HORSE_RACE) == 0) || (HIGHSCORE(HIGHSCORE_HORSE_RACE) >= 0xB4)) {
            HIGHSCORE(HIGHSCORE_HORSE_RACE) = 0xB4;
            gSaveContext.timer1Value = *timer1ValuePtr;
        }
        if (!(gSaveContext.eventChkInf[1] & 0x4000) && (gSaveContext.timer1Value < 0x32)) {
            return 0x208F;
        } else if (gSaveContext.timer1Value < HIGHSCORE(HIGHSCORE_HORSE_RACE)) {
            return 0x2012;
        } else {
            return 0x2004;
        }
    ...
    switch (func_8010BDBC(&globalCtx->msgCtx)) {
        ...
        case 4:
            if (func_80106BC8(globalCtx) != 0) {
                ...
                if (globalCtx->msgCtx.choiceIndex == 0) {
                    ...
                    } else if (HIGHSCORE(HIGHSCORE_HORSE_RACE) == 0) {
        ...
        case 2:
            switch (thisx->textId) {
                ...
                case 0x2004:
                case 0x2012:
                    if (HIGHSCORE(HIGHSCORE_HORSE_RACE) > gSaveContext.timer1Value) {
                        HIGHSCORE(HIGHSCORE_HORSE_RACE) = gSaveContext.timer1Value;
                    }
                case 0x208E:
                    gSaveContext.eventInf[0] &= ~0x400;
                    thisx->flags &= ~0x10000;
