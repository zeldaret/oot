...
void BgGndDarkmeiro_Init(Actor* thisx, PlayState* play2) {
    ...
    switch (PARAMS_GET(this->dyna.actor.params, 0, 0xFF)) {
        ...
        case DARKMEIRO_CLEAR_BLOCK:
            ...
            if (PARAMS_GET(this->dyna.actor.params, 8, 0x3F) == 0x3F) {
        ...
        case DARKMEIRO_BLOCK_TIMER:
            ...
            thisx->draw = NULL;
            if (Flags_GetSwitch(play, PARAMS_GET(this->dyna.actor.params, 8, 0x3F) + 1)) {
                this->timer1 = 64;
                this->actionFlags |= 4;
            }
            if (Flags_GetSwitch(play, PARAMS_GET(this->dyna.actor.params, 8, 0x3F) + 2)) {
                this->timer2 = 64;
                this->actionFlags |= 8;
            }
            if ((this->timer1 != 0) || (this->timer2 != 0)) {
                ...
                Flags_UnsetSwitch(play, PARAMS_GET(this->dyna.actor.params, 8, 0x3F));
            }
            break;
    }
}
...
void BgGndDarkmeiro_Destroy(Actor* thisx, PlayState* play2) {
    ...
    if (PARAMS_GET(this->dyna.actor.params, 0, 0xFF) == 1) {
        if (1) {}
    ...
    if (Flags_GetSwitch(play, PARAMS_GET(this->dyna.actor.params, 8, 0x3F) + 1)) {
        if (this->actionFlags & 4) {
            if (this->timer1 > 0) {
                ...
                Flags_UnsetSwitch(play, PARAMS_GET(this->dyna.actor.params, 8, 0x3F) + 1);
                this->actionFlags &= ~4;
            }
        } else {
            this->actionFlags |= 4;
    ...
    if (Flags_GetSwitch(play, PARAMS_GET(this->dyna.actor.params, 8, 0x3F) + 2)) {
        if (this->actionFlags & 8) {
            if (this->timer2 > 0) {
                ...
                Flags_UnsetSwitch(play, PARAMS_GET(this->dyna.actor.params, 8, 0x3F) + 2);
                this->actionFlags &= ~8;
            }
        } else {
            this->actionFlags |= 8;
