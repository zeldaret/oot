...
void BgHidanCurtain_WaitForSwitchOn(BgHidanCurtain* this, GlobalContext* globalCtx) {
    if (Flags_GetSwitch(globalCtx, this->actor.params)) {
        if (this->type == 1) {
            ...
            OnePointDemo_Init(globalCtx, 3350, -99, &this->actor, 0);
            this->timer = 50;
        } else if (this->type == 3) {
            this->actionFunc = BgHidanCurtain_WaitForCutscene;
            OnePointDemo_Init(globalCtx, 3360, 60, &this->actor, 0);
            this->timer = 30;
        } else {
...
void BgHidanCurtain_Update(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if ((globalCtx->cameraPtrs[0]->setting == CAM_SET_ITEM0) || (globalCtx->cameraPtrs[0]->setting == CAM_SET_ITEM2)) {
