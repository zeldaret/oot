...
void BgHidanHamstep_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->dyna.actor.params & 0xFF) == 0) {
        CollisionHeader_GetVirtual(&gHidanObjectsCol_00DE44, &colHeader);
    } else {
        CollisionHeader_GetVirtual(&gHidanObjectsCol_00DD1C, &colHeader);
    }
...
void BgHidanHamstep_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((thisx->params & 0xFF) == 0) {
        gSPDisplayList(POLY_OPA_DISP++, gHidanObjectsDL_00A668);
    } else {
        gSPDisplayList(POLY_OPA_DISP++, gHidanObjectsDL_00A548);
    }
