...
void EnSb_SetupWaitClosed(EnSb* this) {
    SkelAnime_ChangeAnim(&this->skelAnime, &D_0600004C, 1.0f, 0, SkelAnime_GetLastFrame(&D_0600004C), 2, 0.0f);
    this->behavior = SHELLBLADE_WAIT_CLOSED;
    this->actionFunc = EnSb_WaitClosed;
}

void EnSb_SetupOpen(EnSb* this) {
    SkelAnime_ChangeAnim(&this->skelAnime, &D_06000194, 1.0f, 0, SkelAnime_GetLastFrame(&D_06000194), 2, 0.0f);
...
void EnSb_SetupWaitOpen(EnSb* this) {
    SkelAnime_ChangeAnim(&this->skelAnime, &D_06002C8C, 1.0f, 0, SkelAnime_GetLastFrame(&D_06002C8C), 0, 0.0f);
    this->behavior = SHELLBLADE_WAIT_OPEN;
    this->actionFunc = EnSb_WaitOpen;
}

void EnSb_SetupLunge(EnSb* this) {
    f32 frameCount = SkelAnime_GetLastFrame(&D_06000124);
...
void EnSb_SetupBounce(EnSb* this) {
    SkelAnime_ChangeAnim(&this->skelAnime, &D_060000B4, 1.0f, 0, SkelAnime_GetLastFrame(&D_060000B4), 2, 0.0f);
    this->behavior = SHELLBLADE_BOUNCE;
    this->actionFunc = EnSb_Bounce;
}

void EnSb_SetupCooldown(EnSb* this, s32 changeSpeed) {
    f32 frameCount = SkelAnime_GetLastFrame(&D_0600004C);
...
void EnSb_Open(EnSb* this, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_GetLastFrame(&D_06000194) <= currentFrame) {
        this->timer = 15;
...
void EnSb_Bounce(EnSb* this, GlobalContext* globalCtx) {
    ...
    frameCount = SkelAnime_GetLastFrame(&D_060000B4);
    Math_ApproxF(&this->actor.speedXZ, 0.0f, 0.2f);
