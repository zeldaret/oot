...
void EnFu_WaitChild(EnFu* this, PlayState* play) {
    ...
    if (textId == 0) {
        textId = GET_EVENTCHKINF(EVENTCHKINF_DRAINED_WELL_IN_KAKARIKO) ? 0x5033 : 0x5032;
    }
...
void func_80A1DB60(EnFu* this, PlayState* play) {
    if (play->csCtx.state == CS_STATE_IDLE) {
        this->actionFunc = EnFu_WaitAdult;
        SET_EVENTCHKINF(EVENTCHKINF_LEARNED_SONG_OF_STORMS);
...
void EnFu_WaitAdult(EnFu* this, PlayState* play) {
    ...
    yawDiff = this->actor.yawTowardsPlayer - this->actor.shape.rot.y;
    if (GET_EVENTCHKINF(EVENTCHKINF_LEARNED_SONG_OF_STORMS)) {
