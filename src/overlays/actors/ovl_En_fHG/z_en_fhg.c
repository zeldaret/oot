...
void EnfHG_Intro(EnfHG* this, PlayState* play) {
    ...
    switch (this->cutsceneState) {
        ...
        case INTRO_LAUGH:
            ...
            if (this->timers[0] == 35) {
                Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&audioVec, NA_SE_EN_FANTOM_EYE);
            }
            if (this->timers[0] == 130) {
                bossGnd->work[GND_EYE_STATE] = GND_EYESTATE_FADE;
                Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&audioVec, NA_SE_EN_FANTOM_ST_LAUGH);
            }
            if (this->timers[0] == 20) {
                SEQCMD_PLAY_SEQUENCE(SEQ_PLAYER_BGM_MAIN, 0, 0, NA_BGM_BOSS);
            }
...
void EnfHG_Approach(EnfHG* this, PlayState* play) {
    ...
    if (this->actor.params == GND_REAL_BOSS) {
        ...
        if ((this->gallopTimer % 8) == 0) {
            Audio_PlaySfxGeneralWithDefaultSettingsAtPosition(&this->hoofSfxPos, NA_SE_EV_HORSE_RUN);
        }
    }
    SkelAnime_Update(&this->skin.skelAnime);
