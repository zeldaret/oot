...
void func_808C0C50(BgZg* this) {
    Audio_PlaySfxGeneral(NA_SE_EV_METALDOOR_OPEN, &this->bg.actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
}

s32 func_808C0C98(BgZg* this, PlayState* play) {
    s32 flag = (this->bg.actor.params >> 8) & 0xFF;

    return Flags_GetSwitch(play, flag);
}

s32 func_808C0CC8(BgZg* this) {
    s32 flag = this->bg.actor.params & 0xFF;
...
void func_808C0D08(BgZg* this, PlayState* play) {
    this->bg.actor.world.pos.y += (kREG(16) + 20.0f) * 1.2f;
    if ((((kREG(17) + 200.0f) * 1.2f) + this->bg.actor.home.pos.y) <= this->bg.actor.world.pos.y) {
...
void BgZg_Init(Actor* thisx, PlayState* play) {
    ...
    if ((func_808C0CC8(this) == 8) || (func_808C0CC8(this) == 9)) {
        this->bg.actor.scale.x = this->bg.actor.scale.x * 1.3f;
        this->bg.actor.scale.z = this->bg.actor.scale.z * 1.3f;
        this->bg.actor.scale.y = this->bg.actor.scale.y * 1.2f;
    }
