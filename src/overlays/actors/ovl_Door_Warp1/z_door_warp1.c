...
void DoorWarp1_ChildWarpIdle(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    if (DoorWarp1_PlayerInRange(this, globalCtx)) {
        ...
        Audio_PlaySfxGeneral(NA_SE_EV_LINK_WARP, &player->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
...
void func_80999FE4(DoorWarp1* this, GlobalContext* globalCtx) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_NONE) {
        Audio_PlaySfxGeneral(NA_SE_EV_LINK_WARP, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
