...
void EnVm_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_Init(globalCtx, &this->skelAnime, &gVmSkel, &gVmAnim, this->jointTable, this->morphTable, 11);
...
void EnVm_SetupWait(EnVm* this) {
    f32 frameCount = Animation_GetLastFrame(&gVmAnim);

    Animation_Change(&this->skelAnime, &gVmAnim, 1.0f, frameCount, frameCount, ANIMMODE_ONCE, 0.0f);
...
void EnVm_SetupAttack(EnVm* this) {
    Animation_Change(&this->skelAnime, &gVmAnim, 3.0f, 3.0f, 7.0f, ANIMMODE_ONCE, 0.0f);
    this->timer = 305;
...
void EnVm_SetupStun(EnVm* this) {
    Animation_Change(&this->skelAnime, &gVmAnim, -1.0f, Animation_GetLastFrame(&gVmAnim), 0.0f, ANIMMODE_ONCE,
                     0.0f);
    this->unk_260 = 0;
    this->timer = 180;
    ...
    if (this->timer == 0) {
        if (SkelAnime_Update(&this->skelAnime)) {
            ...
            if (this->unk_25E == 3) {
                ...
                Animation_Change(&this->skelAnime, &gVmAnim, 1.0f, 0.0f, Animation_GetLastFrame(&gVmAnim),
                                 ANIMMODE_ONCE, 0.0f);
            } else {
                this->timer = 10;
                this->skelAnime.curFrame = 0.0f;
...
void EnVm_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_vm.c", 1063),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_OPA_DISP++, gVmDL);
