...
void EnDoor_Destroy(Actor* thisx, PlayState* play) {
    ...
    transitionEntry = &play->transiActorCtx.list[PARAMS_GET_NOMASK((u16)this->actor.params, 10)];
...
void EnDoor_SetupType(EnDoor* this, PlayState* play) {
    ...
    if (Object_IsLoaded(&play->objectCtx, this->requiredObjBankIndex)) {
        doorType = PARAMS_GET(this->actor.params, 7, 3);
        ...
        if (doorType == DOOR_LOCKED) {
            ...
            this->actor.textId = PARAMS_GET(this->actor.params, 0, 6) + 0x0200;
