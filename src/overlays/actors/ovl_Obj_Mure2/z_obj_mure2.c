...
void ObjMure2_SetPosShrubCircle(Vec3f* vec, ObjMure2* this) {
    ...
    for (i = 1; i < D_80B9A818[PARAMS_GET(this->actor.params, 0, 2)]; i++) {
...
void ObjMure2_SetPosShrubScattered(Vec3f* vec, ObjMure2* this) {
    ...
    for (i = 0; i < D_80B9A818[PARAMS_GET(this->actor.params, 0, 2)]; i++) {
        Math_Vec3f_Copy(vec + i, &this->actor.world.pos);
...
void ObjMure2_SetActorSpawnParams(s16* params, ObjMure2* this) {
    ...
    s16 dropTable = PARAMS_GET(this->actor.params, 8, 4);

    if (dropTable >= 13) {
        dropTable = 0;
    }
    *params = actorSpawnParams[PARAMS_GET(this->actor.params, 0, 2)] & 0xF0FF;
    *params |= (dropTable << 8);
}
...
void ObjMure2_SpawnActors(ObjMure2* this, PlayState* play) {
    ...
    s32 actorNum = PARAMS_GET(this->actor.params, 0, 2);
    s32 i;
    Vec3f spawnPos[12];
    s16 params;
...
void func_80B9A668(ObjMure2* this, PlayState* play) {
    if (Math3D_Dist1DSq(this->actor.projectedPos.x, this->actor.projectedPos.z) <
        (sDistSquared1[PARAMS_GET(this->actor.params, 0, 2)] * this->unk_184)) {
...
void func_80B9A6F8(ObjMure2* this, PlayState* play) {
    func_80B9A534(this);
    if ((sDistSquared2[PARAMS_GET(this->actor.params, 0, 2)] * this->unk_184) <=
