...
void EnStream_SuckPlayer(EnStream* this, GlobalContext* globalCtx) {
    ...
    if (func_80B0B81C(&this->actor.world.pos, &player->actor.world.pos, &posDifference, this->actor.scale.y) != 0) {
        ...
        player->pushYaw = RAD_TO_BINANG(Math_FAtan2F(-posDifference.x, -posDifference.z));
        if (xzDist > 3.0f) {
            Math_SmoothStepToF(&player->pushSpeed, 3.0f, 0.5f, xzDist, 0.0f);
        } else {
            player->pushSpeed = 0.0f;
