    ...
    while (actor != NULL) {
        if (actor->id == ACTOR_BG_GANON_OTYUKA) {
            ...
            if (platform->dyna.actor.projectedPos.z > spBC) {
                ...
                gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_bg_ganon_otyuka.c", 766), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
                ...
                for (i = 0; i < ARRAY_COUNT(sSides); i++) {
                    if (platform->visibleSides & sSides[i]) {
                        ...
                        gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_bg_ganon_otyuka.c", 785), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            ...
            if ((platform->dyna.actor.projectedPos.z > -30.0f) && (platform->flashState != FLASH_NONE)) {
                ...
                for (i = 0; i < ARRAY_COUNT(sSides); i++) {
                    if (platform->unwalledSides & sSides[i]) {
                        ...
                        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_bg_ganon_otyuka.c", 847), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
