...
void EnTa_Init(Actor* thisx, PlayState* play2) {
    ...
    switch (this->actor.params) {
        case 1:
            osSyncPrintf(TE_FGCOL(CYAN) " 追放タロン \n" TE_RST);
            ...
            break;
        case 2:
            osSyncPrintf(TE_FGCOL(CYAN) " 出戻りタロン \n" TE_RST);
            if (!GET_EVENTCHKINF(EVENTCHKINF_6B)) {
                Actor_Kill(&this->actor);
            } else if (!LINK_IS_ADULT) {
                Actor_Kill(&this->actor);
                ...
                Actor_Kill(&this->actor);
                osSyncPrintf(TE_FGCOL(CYAN) " 夜はいない \n" TE_RST);
            } else {
            ...
            break;
        default:
            osSyncPrintf(TE_FGCOL(CYAN) " その他のタロン \n" TE_RST);
            if (play->sceneId == SCENE_SPOT15) {
                ...
                osSyncPrintf(TE_FGCOL(CYAN) " ロンロン牧場の倉庫 の タロン\n" TE_RST);
                if (!GET_EVENTCHKINF(EVENTCHKINF_14)) {
                    Actor_Kill(&this->actor);
                } else if (LINK_IS_ADULT) {
                    Actor_Kill(&this->actor);
