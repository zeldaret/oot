...
void EnViewer_InitImpl(EnViewer* this, PlayState* play) {
    ...
    s32 skelObjectLoadEntryIndex = Object_GetLoadEntryIndex(&play->objectCtx, initData->skeletonObject);

    ASSERT(skelObjectLoadEntryIndex >= 0, "bank_ID >= 0", "../z_en_viewer.c", 576);

    this->animObjectLoadEntryIndex = Object_GetLoadEntryIndex(&play->objectCtx, initData->animObject);
    ASSERT(this->animObjectLoadEntryIndex >= 0, "this->anime_bank_ID >= 0", "../z_en_viewer.c", 579);

    if (!Object_IsLoadEntryLoaded(&play->objectCtx, skelObjectLoadEntryIndex) ||
        !Object_IsLoadEntryLoaded(&play->objectCtx, this->animObjectLoadEntryIndex)) {
