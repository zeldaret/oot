    ...
    if (type == ENVIEWER_TYPE_2_ZELDA) {
        ...
        if (gSaveContext.sceneSetupIndex == 4) {
            switch (globalCtx->csCtx.frames) {
                ...
                case 409:
                case 438:
                    Audio_PlaySfxGeneral(NA_SE_SY_DEMO_CUT, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
        ...
        if (globalCtx->csCtx.frames == 960) {
            Audio_PlaySfxGeneral(NA_SE_EV_HORSE_GROAN, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
        }
        ...
        switch (this->state) {
            case 0:
                if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL &&
                    ...
                    Audio_PlaySfxGeneral(NA_SE_EN_GANON_LAUGH, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
                                         &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
...
void EnViewer_UpdatePosition(EnViewer* this, GlobalContext* globalCtx) {
    ...
    if (type <= ENVIEWER_TYPE_2_ZELDA) { // zelda's horse, impa and zelda
        if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[0] != NULL &&
            ...
            if (type == ENVIEWER_TYPE_0_HORSE_ZELDA) {
                if (!sHorseSfxPlayed) {
                    ...
                    Audio_PlaySfxGeneral(NA_SE_EV_HORSE_NEIGH, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                         &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
                }
                Audio_PlayActorSound2(&this->actor, NA_SE_EV_HORSE_RUN_LEVEL - SFX_FLAG);
            }
        ...
        if (type == ENVIEWER_TYPE_5_GANONDORF) {
            Audio_PlaySfxGeneral(NA_SE_EV_BURNING - SFX_FLAG, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
