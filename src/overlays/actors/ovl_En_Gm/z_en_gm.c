...
void EnGm_Init(Actor* thisx, PlayState* play) {
    ...
    this->gmObjectEntry = Object_GetEntry(&play->objectCtx, OBJECT_GM);

    if (this->gmObjectEntry < 0) {
...
void func_80A3D838(EnGm* this, PlayState* play) {
    if (Object_IsEntryLoaded(&play->objectCtx, this->gmObjectEntry)) {
        this->actor.flags &= ~ACTOR_FLAG_4;
        SkelAnime_InitFlex(play, &this->skelAnime, &gGoronSkel, NULL, this->jointTable, this->morphTable, 18);
        gSegments[6] = VIRTUAL_TO_PHYSICAL(play->objectCtx.entries[this->gmObjectEntry].segment);
