...
void EnGm_Init(Actor* thisx, PlayState* play) {
    ...
    this->gmObjectLoadEntryIndex = Object_GetLoadEntryIndex(&play->objectCtx, OBJECT_GM);
...
void func_80A3D838(EnGm* this, PlayState* play) {
    if (Object_IsLoadEntryLoaded(&play->objectCtx, this->gmObjectLoadEntryIndex)) {
        this->actor.flags &= ~ACTOR_FLAG_4;
        SkelAnime_InitFlex(play, &this->skelAnime, &gGoronSkel, NULL, this->jointTable, this->morphTable, 18);
        gSegments[6] = VIRTUAL_TO_PHYSICAL(play->objectCtx.loadEntries[this->gmObjectLoadEntryIndex].segment);
