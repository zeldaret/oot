...
void EnDntDemo_Judge(EnDntDemo* this, GlobalContext* globalCtx) {
    ...
    if (this->leaderSignal != DNT_SIGNAL_NONE) {
        ...
        if (this->subCamera != CAM_ID_MAIN) {
            this->subCamera = CAM_ID_MAIN;
        }
        ...
        if ((Player_GetMask(globalCtx) != 0) && (this->subCamera == CAM_ID_MAIN)) {
            this->subCamera = OnePointCutscene_Init(globalCtx, 2220, -99, &this->scrubs[3]->actor, CAM_ID_MAIN);
        }
