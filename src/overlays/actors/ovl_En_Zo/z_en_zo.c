...
void EnZo_SpawnBubble(EnZo* this, Vec3f* pos) {
    ...
    for (i = 0; i < EN_ZO_EFFECT_COUNT; i++, effect++) {
        ...
        waterSurface = this->actor.world.pos.y + this->actor.yDistUnderWater;
...
void EnZo_TreadWaterRipples(EnZo* this, f32 scale, f32 targetScale, u8 alpha) {
    ...
    pos.y = this->actor.world.pos.y + this->actor.yDistUnderWater;
...
s32 EnZo_PlayerInProximity(EnZo* this, PlayState* play) {
    ...
    surfacePos.y = this->actor.world.pos.y + this->actor.yDistUnderWater;
...
void EnZo_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.yDistUnderWater < 54.0f || (this->actor.params & 0x3F) == 8) {
...
void EnZo_Surface(EnZo* this, PlayState* play) {
    if (this->actor.yDistUnderWater < 54.0f) {
        ...
        this->alpha = 255.0f;
    } else if (this->actor.yDistUnderWater < 80.0f) {
...
void EnZo_TreadWater(EnZo* this, PlayState* play) {
    ...
    Math_ApproachF(&this->actor.velocity.y, this->actor.yDistUnderWater < 54.0f ? -0.6f : 0.6f, 0.3f, 0.2f);
...
void EnZo_Dive(EnZo* this, PlayState* play) {
    ...
    if (this->actor.yDistUnderWater > 80.0f || this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
