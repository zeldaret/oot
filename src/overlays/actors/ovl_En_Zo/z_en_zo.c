...
u16 func_80B61024(PlayState* play, Actor* thisx) {
    ...
    switch (PARAMS_GET(thisx->params, 0, 0x3F)) {
        case 8:
...
void EnZo_Init(Actor* thisx, PlayState* play) {
    ...
    if (LINK_IS_ADULT && (PARAMS_GET(this->actor.params, 0, 0x3F) == 8)) {
    ...
    if (this->actor.yDistToWater < 54.0f || PARAMS_GET(this->actor.params, 0, 0x3F) == 8) {
