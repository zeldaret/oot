...
void EnBa_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params < EN_BA_DEAD_BLOB) {
        ...
        Matrix_Pop();
        gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_ba.c", 973), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        ...
        gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_en_ba.c", 991), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
