...
void EnKusa_SpawnFragments(EnKusa* this, GlobalContext* globalCtx) {
    ...
    u32 i;
    s32 index;
    Vec3f* effectMultiplyer;
    s32 pad;

    for (i = 0; i < ARRAY_COUNT(D_80A9C23C); i++) {
        effectMultiplyer = &D_80A9C23C[i];
        effectPosition.x = this->actor.posRot.pos.x + (effectMultiplyer->x * this->actor.scale.x * 20.0f);
        effectPosition.y = this->actor.posRot.pos.y + (effectMultiplyer->y * this->actor.scale.y * 20.0f) + 10.0f;
        effectPosition.z = this->actor.posRot.pos.z + (effectMultiplyer->z * this->actor.scale.z * 20.0f);
        ...
        effectPosition.x = this->actor.posRot.pos.x + (effectMultiplyer->x * this->actor.scale.x * 40.0f);
        effectPosition.y = this->actor.posRot.pos.y + (effectMultiplyer->y * this->actor.scale.y * 40.0f) + 10.0f;
        effectPosition.z = this->actor.posRot.pos.z + (effectMultiplyer->z * this->actor.scale.z * 40.0f);
