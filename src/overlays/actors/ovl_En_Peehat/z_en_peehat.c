...
void EnPeehat_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_Init(globalCtx, &this->skelAnime, &gPeehatSkel, &gPeehatAnim3, this->jointTable, this->morphTable, 24);
...
void EnPeehat_Ground_SetStateGround(EnPeehat* this) {
    Animation_Change(&this->skelAnime, &gPeehatAnim3, 0.0f, 3.0f, Animation_GetLastFrame(&gPeehatAnim3), 2, 0.0f);
...
void EnPeehat_Flying_SetStateFly(EnPeehat* this) {
    Animation_PlayLoop(&this->skelAnime, &gPeehatAnim1);
...
void EnPeehat_Ground_SetStateRise(EnPeehat* this) {
    ...
    lastFrame = Animation_GetLastFrame(&gPeehatAnim3);
    if (this->state != PEAHAT_STATE_STUNNED) {
        Animation_Change(&this->skelAnime, &gPeehatAnim3, 0.0f, 3.0f, lastFrame, 2, 0.0f);
    }
...
void EnPeehat_Ground_SetStateLanding(EnPeehat* this) {
    ...
    Animation_PlayOnce(&this->skelAnime, &gPeehatAnim0);
...
void EnPeehat_SetStateAttackRecoil(EnPeehat* this) {
    Animation_MorphToPlayOnce(&this->skelAnime, &gPeehatAnim2, -4.0f);
