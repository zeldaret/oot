...
void ObjectKankyo_DrawFairies(Actor* thisx, PlayState* play2) {
    ...
    if (!(play->cameraPtrs[CAM_ID_MAIN]->stateFlags & CAM_STATE_CAMERA_IN_WATER)) {
        ...
        for (i = 0; i < play->envCtx.precipitation[PRECIP_SNOW_MAX]; i++) {
            ...
            gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_object_kankyo.c", 913), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void ObjectKankyo_DrawSnow(Actor* thisx, PlayState* play2) {
    ...
    if (!(play->cameraPtrs[CAM_ID_MAIN]->stateFlags & CAM_STATE_CAMERA_IN_WATER)) {
        ...
        for (i = 0; i < play->envCtx.precipitation[PRECIP_SNOW_CUR]; i++) {
            ...
            gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_object_kankyo.c", 1107), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void ObjectKankyo_DrawLightning(Actor* thisx, PlayState* play) {
    ...
    if (this->effects[0].state == 1) {
        ...
        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_object_kankyo.c", 1213), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void ObjectKankyo_DrawSunGraveSpark(Actor* thisx, PlayState* play2) {
    ...
    if (play->csCtx.state != CS_STATE_IDLE) {
        if (play->csCtx.actorCues[1] != NULL && play->csCtx.actorCues[1]->id == 2 && this->requiredObjectLoaded) {
            ...
            Matrix_Mult(&play->billboardMtxF, MTXMODE_APPLY);
            gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_object_kankyo.c", 1416), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    ...
    if (this->requiredObjectLoaded) {
        for (i = 0; i < 6; i++) {
            if (this->effects[i].size > 0.001f) {
                ...
                gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_object_kankyo.c", 1586), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
