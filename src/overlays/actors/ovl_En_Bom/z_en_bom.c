...
void EnBom_Move(EnBom* this, GlobalContext* globalCtx) {
    ...
    if ((this->actor.velocity.y > 0.0f) && (this->actor.bgCheckFlags & BGCHECKFLAG_4)) {
        this->actor.velocity.y = -this->actor.velocity.y;
    }

    // rebound bomb off the wall it hits
    if ((this->actor.speedXZ != 0.0f) && (this->actor.bgCheckFlags & BGCHECKFLAG_3)) {
    ...
    if (!(this->actor.bgCheckFlags & BGCHECKFLAG_0)) {
        ...
        if ((this->actor.bgCheckFlags & BGCHECKFLAG_1) && (this->actor.velocity.y < -3.0f)) {
