...
void BgSpot06Objects_GateSpawnBubbles(BgSpot06Objects* this, PlayState* play) {
    ...
    if ((play->gameplayFrames % 3) == 0) {
        ...
        sp34.x = (Math_SinS(this->bg.actor.shape.rot.y + 0x4000) * tmp) + this->bg.actor.world.pos.x;
        sp34.y = this->bg.actor.world.pos.y;
        sp34.z = (Math_CosS(this->bg.actor.shape.rot.y + 0x4000) * tmp) + this->bg.actor.world.pos.z;
...
void BgSpot06Objects_GateWaitForSwitch(BgSpot06Objects* this, PlayState* play) {
    ...
    if (Flags_GetSwitch(play, this->switchFlag)) {
        this->timer = 100;
        this->bg.actor.world.pos.y += 3.0f;
...
void BgSpot06Objects_GateOpen(BgSpot06Objects* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->bg.actor.world.pos.y, this->bg.actor.home.pos.y + 120.0f, 0.6f)) {
        this->actionFunc = BgSpot06Objects_DoNothing;
        this->timer = 0;
...
void BgSpot06Objects_LockSpawnWaterRipples(BgSpot06Objects* this, PlayState* play, s32 flag) {
    if (flag || !(play->gameplayFrames % 7)) {
        EffectSsGRipple_Spawn(play, &this->bg.actor.home.pos, 300, 700, 0);
    }
}

void BgSpot06Objects_LockSpawnBubbles(BgSpot06Objects* this, PlayState* play, s32 flag) {
    if (!(play->gameplayFrames % 7) || flag) {
        EffectSsBubble_Spawn(play, &this->bg.actor.world.pos, 0.0f, 40.0f, 30.0f, (Rand_ZeroOne() * 0.05f) + 0.175f);
    }
}
...
void BgSpot06Objects_LockWait(BgSpot06Objects* this, PlayState* play) {
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        sin = Math_SinS(this->bg.actor.world.rot.y);
        cos = Math_CosS(this->bg.actor.world.rot.y);
        this->bg.actor.world.pos.x += (3.0f * sin);
        this->bg.actor.world.pos.z += (3.0f * cos);

        for (i = 0; i < 20; i++) {
            BgSpot06Objects_LockSpawnBubbles(this, play, 1);
        }

        effectPos.x = this->bg.actor.world.pos.x + (5.0f * sin);
        effectPos.y = this->bg.actor.world.pos.y;
        effectPos.z = this->bg.actor.world.pos.z + (5.0f * cos);

        for (i = 0; i < 3; i++) {
            EffectSsBubble_Spawn(play, &effectPos, 0.0f, 20.0f, 20.0f, (Rand_ZeroOne() * 0.1f) + 0.7f);
        }

        EffectSsGSplash_Spawn(play, &this->bg.actor.world.pos, NULL, NULL, 1, 700);
        ...
        OnePointCutscene_Init(play, 4120, 170, &this->bg.actor, CAM_ID_MAIN);
    } else {
...
void BgSpot06Objects_LockPullOutward(BgSpot06Objects* this, PlayState* play) {
    ...
    this->bg.actor.world.pos.x += (0.3f * Math_SinS(this->bg.actor.world.rot.y));
    this->bg.actor.world.pos.z += (0.3f * Math_CosS(this->bg.actor.world.rot.y));
    BgSpot06Objects_LockSpawnBubbles(this, play, 0);

    if (this->timer == 0) {
        this->bg.actor.velocity.y = 0.5f;
        this->bg.actor.flags &= ~ACTOR_FLAG_13;
...
void BgSpot06Objects_LockSwimToSurface(BgSpot06Objects* this, PlayState* play) {
    ...
    if (this->bg.actor.velocity.y <= 0.0f) {
        cos = Math_CosS(this->bg.actor.shape.rot.x) * 4.3f;
        this->bg.actor.world.pos.x += (cos * Math_SinS(this->bg.actor.shape.rot.y));
        this->bg.actor.world.pos.z += (cos * Math_CosS(this->bg.actor.shape.rot.y));
        this->bg.actor.world.pos.y = this->bg.actor.world.pos.y - 1.3f;
        BgSpot06Objects_LockSpawnWaterRipples(this, play, 0);

        if (Math_ScaledStepToS(&this->bg.actor.shape.rot.x, 0, 0x260) != 0) {
            this->bg.actor.home.pos.x = this->bg.actor.world.pos.x - (Math_SinS(this->bg.actor.shape.rot.y) * 16.0f);
            this->bg.actor.home.pos.z = this->bg.actor.world.pos.z - (Math_CosS(this->bg.actor.shape.rot.y) * 16.0f);
            this->bg.actor.world.pos.y = -1993.0f;
            this->timer = 32;
            this->bg.actor.flags &= ~ACTOR_FLAG_4;
        ...
        if (this->bg.actor.world.pos.y >= -1973.0f) {
            this->bg.actor.velocity.y = 0.0f;
            BgSpot06Objects_LockSpawnWaterRipples(this, play, 1);
            EffectSsGSplash_Spawn(play, &this->bg.actor.home.pos, NULL, NULL, 1, 700);
        } else if (this->bg.actor.shape.rot.x == -0x4000) {
            this->bg.actor.velocity.y += 0.02f;
            this->bg.actor.world.pos.x = Rand_CenteredFloat(1.0f) + this->bg.actor.home.pos.x;
            this->bg.actor.world.pos.z = Rand_CenteredFloat(1.0f) + this->bg.actor.home.pos.z;
            this->bg.actor.velocity.y = (this->bg.actor.velocity.y > 10.0f) ? (10.0f) : (this->bg.actor.velocity.y);
            BgSpot06Objects_LockSpawnBubbles(this, play, 0);
        } else {
            BgSpot06Objects_LockSpawnBubbles(this, play, 0);

            if (Math_ScaledStepToS(&this->bg.actor.shape.rot.x, -0x4000, 0x30)) {
                this->bg.actor.home.pos.x = this->bg.actor.world.pos.x;
                this->bg.actor.home.pos.y = -1993.0f;
                this->bg.actor.home.pos.z = this->bg.actor.world.pos.z;
            }
        }
    }
}
...
void BgSpot06Objects_LockFloat(BgSpot06Objects* this, PlayState* play) {
    ...
    this->bg.actor.world.pos.y = (2.0f * sinf(this->timer * (M_PI / 16.0f))) + this->bg.actor.home.pos.y;
...
void BgSpot06Objects_WaterPlaneCutsceneRise(BgSpot06Objects* this, PlayState* play) {
    ...
    this->bg.actor.world.pos.y = this->lakeHyliaWaterLevel + WATER_LEVEL_RAISED;

    if (this->lakeHyliaWaterLevel >= 0.0001f) {
        this->bg.actor.world.pos.y = WATER_LEVEL_RAISED;
        this->actionFunc = BgSpot06Objects_DoNothing;
    } else {
        ...
        play->colCtx.colHeader->waterBoxes[LHWB_MAIN_1].ySurface = this->bg.actor.world.pos.y;
        play->colCtx.colHeader->waterBoxes[LHWB_MAIN_2].ySurface = this->bg.actor.world.pos.y;
    }

    func_8002F948(&this->bg.actor, NA_SE_EV_WATER_LEVEL_DOWN - SFX_FLAG);
}
