...
void EnEiyer_SetupAppearFromGround(EnEiyer* this) {
    this->collider.elem.acDmgInfo.dmgFlags_ColliderElementDamageInfoAC = DMG_BOOMERANG | DMG_EXPLOSIVE | DMG_DEKU_NUT;
...
void EnEiyer_SetupAmbush(EnEiyer* this, PlayState* play) {
    ...
    this->collider.elem.acDmgInfo.dmgFlags_ColliderElementDamageInfoAC = DMG_DEFAULT;
...
void EnEiyer_SetupDie(EnEiyer* this) {
    ...
    if (this->collider.elem.acDmgInfo.dmgFlags_ColliderElementDamageInfoAC != (DMG_BOOMERANG | DMG_EXPLOSIVE | DMG_DEKU_NUT)) {
...
void EnEiyer_UpdateDamage(EnEiyer* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if (this->actor.colChkInfo.damageEffect_CollisionCheckInfo != 0 || this->actor.colChkInfo.damage_CollisionCheckInfo != 0) {
            ...
            } else if (this->actor.colChkInfo.damageEffect_CollisionCheckInfo == 1) {
...
s32 EnEiyer_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx,
    ...
    if (this->collider.elem.acDmgInfo.dmgFlags_ColliderElementDamageInfoAC == (DMG_BOOMERANG | DMG_EXPLOSIVE | DMG_DEKU_NUT) && limbIndex != 9 &&
        limbIndex != 10) {
        *dList = NULL;
    }
    return 0;
