...
void func_80967FFC(Actor* thisx, PlayState* play) {
...

    {
        ...
        for (i = 0; i < 6; i++) {
            ...
            gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1115);
...
void func_80968298(Actor* thisx, PlayState* play) {
    ...
    Matrix_RotateX(-M_PI / 2, MTXMODE_APPLY);
    gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1170);
    ...
    gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1189);
    Gfx_SetupDL_25Xlu(play->state.gfxCtx);
...
void func_8096865C(Actor* thisx, PlayState* play) {
    ...
    if (!(this->flags & 1)) {
        ...
        gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1230);
        gSPDisplayList(POLY_XLU_DISP++, displayList);
        Matrix_Pop();
        Matrix_RotateZ(DEG_TO_RAD(-(f32)(this->timer2 * 6)), MTXMODE_APPLY);
        gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1236);
        gSPDisplayList(POLY_XLU_DISP++, displayList);
    }
...
void func_809688C4(Actor* thisx, PlayState* play2) {
    ...
    if ((i = (play->csCtx.state != CS_STATE_IDLE) && (play->csCtx.actorCues[1] != NULL)) &&
        ...
        for (i = 0; i < 16; i++) {
            ...
            gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1297);
...
void func_80968B70(Actor* thisx, PlayState* play) {
    ...
    Matrix_Mult(&play->billboardMtxF, MTXMODE_APPLY);
    Matrix_RotateX(M_PI / 2, MTXMODE_APPLY);
    gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1322);
    gSPSegment(POLY_XLU_DISP++, 0x08,
...
void func_80968FB0(Actor* thisx, PlayState* play) {
    ...
    gSPMATRIX_SET_NEW(POLY_XLU_DISP++, play->state.gfxCtx, "../z_demo_6k.c", 1394);
