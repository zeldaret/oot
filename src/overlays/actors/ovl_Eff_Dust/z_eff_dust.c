...
void EffDust_DrawFunc_8099E4F4(Actor* thisx, PlayState* play2) {
    ...
    for (i = 0; i < 64; i++) {
        if (*distanceTraveled < 1.0f) {
            ...
            gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(gfxCtx, "../z_eff_dust.c", 449), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void EffDust_DrawFunc_8099E784(Actor* thisx, PlayState* play2) {
    ...
    for (i = 0; i < 64; i++) {
        if (*distanceTraveled < 1.0f) {
            ...
            gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(gfxCtx, "../z_eff_dust.c", 506), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_XLU_DISP++, SEGMENTED_TO_VIRTUAL(gEffSparklesDL));
        }
