...
s32 func_80A58838(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList,
             Vec3f* pos, Vec3s* rot, EnHintnuts* enHintnuts);
...
void EnHintnuts_Init(EnHintnuts* this, GlobalContext* globalCtx)
{
    ...
    Actor_ProcessInitChain(&this->actor, &D_80A58A38);
    ...
    else
    {
        ...
        ActorCollider_InitCylinder(globalCtx, &this->collider, &this->actor, &D_80A58A00);
        func_80061ED4(&this->actor.sub_98, NULL, &D_80A58A2C);
        ...
        func_80A57454(this);
        Actor_SpawnAttached(&globalCtx->actorCtx, &this->actor, globalCtx, ACTOR_EN_HINTNUTS, this->actor.posRot.pos.x, this->actor.posRot.pos.y, this->actor.posRot.pos.z, 0, this->actor.posRot.rot.y, 0, 0xA);
    }
}
...
{
    if (this->actor.textId != 0 && this->actor.type == ACTORTYPE_ENEMY && ((this->actor.params == 0) || (D_80A58A34 == 2)))
    {
...
void func_80A57454(EnHintnuts* this)
{
    ...
    this->collider.base.collideFlags &= ~1;
    this->actionFunc = (ActorFunc)func_80A578FC;
}
...
void func_80A57558(EnHintnuts* this)
{
    ...
    this->actionFunc = (ActorFunc)func_80A57BE4;
}
...
void func_80A575C0(EnHintnuts* this)
{
    ...
    this->actionFunc = (ActorFunc)func_80A57DE4;
}
...
//             D_80A58A34 = 0;
//         }
//         temp_a0 = D_80A58A34 + 1;
//         if (temp_a0 == this->actor.params)
//         {
//             D_80A58A34 = temp_a0;
//         }
//         else
...
//             D_80A58A34 = phi_v0 - 1;   
//         }
//         this->actionFunc = (ActorFunc)func_80A57F5C;
//         this->actor.flags |= 0x10;
//     }
//     else {
//         this->actionFunc = (ActorFunc)func_80A57EF8;
//     }
// } //pretty close
...
//     Audio_PlayActorSound2(&this->actor, NA_SE_EN_NUTS_DAMAGE);
//     Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_ITEM00, this->actor.posRot.pos.x, this->actor.posRot.pos.y, this->actor.posRot.pos.z, 0x0, 0x0, 0x0, 0x3); //recovery heart
//     this->actionFunc = (ActorFunc)func_80A58320;
// } //close, regalloc
...
void func_80A57850(EnHintnuts* this)
{
    ...
    this->actionFunc = (ActorFunc)func_80A584DC;
}
...
void func_80A578FC(EnHintnuts* this, GlobalContext* globalCtx)
{
    ...
    if (sp2C != 0 && 160.0f < this->actor.xzDistanceFromLink && fabsf(this->actor.yDistanceFromLink) < 120.0f && ((this->unk_194 == 0) || (this->actor.xzDistanceFromLink < 480.0f)))
    {
...
void func_80A57CA8(EnHintnuts* this, GlobalContext* globalCtx)
{
    ...
    else if (func_800A56C8(&this->skelAnime, 6.0f) != 0)
    {
        ...
        if (Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_NUTSBALL, nutPos.x, nutPos.y, nutPos.z, this->actor.shape.rot.x, this->actor.shape.rot.y, this->actor.shape.rot.z, 1) != 0)
        {
...
//         temp = this->skelAnime.animCurrentFrame;
...
//             else
//             {
//                 // phi_f0 = this->skelAnime.animCurrentFrame;
//                 phi_f0 = temp;

//             }
//         }
//         this->collider.dim.yShift = (((3.0f - phi_f0) * 12.0f) + 5.0f);
//     }
...
void func_80A58028(EnHintnuts* this, GlobalContext* globalCtx)
{
    ...
    else if (this->unk_194 == 0 && func_8002DBB0(&this->actor, &this->actor.initPosRot.pos) < 20.0f && fabsf(this->actor.posRot.pos.y - this->actor.initPosRot.pos.y) < 2.0f)
    {
        this->actor.speedXZ = 0.0f;
...
//     {
//         phi_a1 = this->actor.unk_7E;
//     }
//     else
//     {
//         temp_a1 = (this->actor.rotTowardsLinkY - func_8005A9F4(globalCtx->cameraCtx.activeCameraPtrs[globalCtx->cameraCtx.unk_5C0])) - 0x8000;
//         if (ABS(temp_a1) >= 0x4001)
//         {
//             phi_a1 = func_8005A9F4(globalCtx->cameraCtx.activeCameraPtrs[globalCtx->cameraCtx.unk_5C0]) + 0x8000;
//         }
//         else
//         {
//             phi_a1 = (func_8005A9F4(globalCtx->cameraCtx.activeCameraPtrs[globalCtx->cameraCtx.unk_5C0]) - (temp_a1 >> 1)) + 0x8000;
//         }
//     }
//     Math_ApproxUpdateScaledS(&this->actor.shape.rot.y, phi_a1, 0x800);
//     this->actor.posRot.rot.y = this->actor.shape.rot.y;
...
void func_80A584DC(EnHintnuts* this, GlobalContext* globalCtx)
{
    ...
    else if (Math_ApproxF(&this->actor.posRot.pos.y, this->actor.initPosRot.pos.y - 35.0f, 7.0f) != 0)
    {
        ...
        else 
        {
            ...
            this->actor.sub_98.health = D_80A58A2C.health;
...
void EnHintnuts_Update(EnHintnuts* this, GlobalContext* globalCtx)
{
    ...
    if (this->actor.params != 0xa)
    {
        ...
        if (this->actionFunc != (ActorFunc)func_80A584DC && this->actionFunc != (ActorFunc)func_80A57F5C)
        {
        ...
        if (this->collider.base.collideFlags & 1)
        {
            Actor_CollisionCheck_SetAC(globalCtx, &globalCtx->sub_11E60, &this->collider);
        }
        Actor_CollisionCheck_SetOT(globalCtx, &globalCtx->sub_11E60, &this->collider);
        if (this->actionFunc == (ActorFunc)func_80A578FC)
        {
            Actor_SetHeight(&this->actor, this->skelAnime.animCurrentFrame);
        }
        else if (this->actionFunc == (ActorFunc)func_80A57DE4)
        {
            Actor_SetHeight(&this->actor, 20.0f - ((this->skelAnime.animCurrentFrame * 20.0f) / (f32) SkelAnime_GetFrameCount(&D_060024CC)));
        }
        else 
        {
...
//     if (limbIndex == 5 && enHintnuts->actionFunc == (ActorFunc)func_80A57CA8)
//     {
//         if (enHintnuts->skelAnime.animCurrentFrame <= 6.0f)
//         {
//             temp_f12 = (enHintnuts->skelAnime.animCurrentFrame * D_80A58A54) + 1.0f;
//             phi_f12.x = temp_f12;
//             phi_f12.y = 1.0f - (enHintnuts->skelAnime.animCurrentFrame * D_80A58A50);
//             phi_f12.z = temp_f12;
//         }
//         else if (enHintnuts->skelAnime.animCurrentFrame <= 7.0f)
//         {
//             temp_f0 = enHintnuts->skelAnime.animCurrentFrame - 6.0f;
//             temp_f12_2 = D_80A58A58 - (temp_f0 * D_80A58A5C);
//             phi_f12.x = temp_f12_2;
//             phi_f12.y = 0.5f + temp_f0;
//             phi_f12.z = temp_f12_2;
//         }
//         else if (enHintnuts->skelAnime.animCurrentFrame <= 10.0f)
//         {
//             phi_f12.x = 1.0f;
//             phi_f12.y = 1.5f - ((enHintnuts->skelAnime.animCurrentFrame - 7.0f) * D_80A58A60);
//             phi_f12.z = 1.0f;
//         }
//         else 
//         {
//             Matrix_Scale(phi_f12.x, phi_f12.y, phi_f12.z, 1);
//         }
//     }
//     return 0;
// } //disastor
...
void EnHintnuts_Draw(EnHintnuts* this, GlobalContext* globalCtx)
{
    if (this->actor.params == 0xA)
    {
        ...
        SkelAnime_Draw(globalCtx, this->skelAnime.skeleton, this->skelAnime.actorDrawTbl, func_80A58838, NULL, &this->actor);
    }
}
