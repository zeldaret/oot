...
void BgSpot00Hanebasi_DrawTorches(Actor* thisx, PlayState* play2) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_bg_spot00_hanebasi.c", 674), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void BgSpot00Hanebasi_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPMatrix(POLY_OPA_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_bg_spot00_hanebasi.c", 702), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
