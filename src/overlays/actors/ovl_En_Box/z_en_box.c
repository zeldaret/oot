...
void EnBox_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    dynaUnk = 0;
    lastFrame = SkelAnime_GetLastFrame(anim);
    ...
    SkelAnime_ChangeAnim(&this->skelanime, anim, 1.5f, animFrameStart, lastFrame, 2, 0.0f);
...
void EnBox_WaitOpen(EnBox* this, GlobalContext* globalCtx) {
    ...
    if (this->unk_1F4 != 0) { // unk_1F4 is modified by player code
        ...
        frameCount = SkelAnime_GetLastFrame(anim);
...
void EnBox_Open(EnBox* this, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_Update(&this->skelanime)) {
        ...
        if (this->skelanime.jointTbl[3].z > 0) {
            this->unk_1B0 = (0x7D00 - this->skelanime.jointTbl[3].z) * 0.00006f;
