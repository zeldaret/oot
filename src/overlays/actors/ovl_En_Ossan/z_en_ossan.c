...
void func_80AC2DD8(EnOssan* this, GlobalContext* globalCtx, OssanStruct1* arg2) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        temp_v0 = D_80AC8C9C[arg2[i].funcIdx](arg2[i].funcIdx);
        tana = &this->tana;
        if (temp_v0 < 0) {
            this->unk_200[i] = NULL;
            continue;
        ...
        this->unk_200[i] = (EnGirlA*)Actor_Spawn(
            &globalCtx->actorCtx, globalCtx, ACTOR_EN_GIRLA, arg2[i].pos.x + (*tana)->actor.posRot.pos.x,
            arg2[i].pos.y + (*tana)->actor.posRot.pos.y, arg2[i].pos.z + (*tana)->actor.posRot.pos.z,
...
u16 func_80AC6F2C(EnOssan* this) {
    this->unk_1EB = 8;
    if (this->actor.params == 10) {
        if (INV_CONTENT(ITEM_WEIRD_EGG) == 0x2C) {
            ...
            if ((gSaveContext.itemGetInf[3] & 0x800) != 0) {
                return 0x70AC;
            } else if (((gSaveContext.itemGetInf[3] & 0x400) == 0) && ((gSaveContext.itemGetInf[2] & 0x10) == 0) &&
                       ((gSaveContext.itemGetInf[3] & 0x100) == 0)) {
