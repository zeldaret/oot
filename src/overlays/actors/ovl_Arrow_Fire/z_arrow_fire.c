...
void ArrowFire_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if ((arrow != NULL) && (arrow->actor.update != NULL) && (this->timer < 255)) {
        ...
        Matrix_RotateY(BINANG_TO_RAD_ALT(tranform->shape.rot.y), MTXMODE_APPLY);
        Matrix_RotateX(BINANG_TO_RAD_ALT(tranform->shape.rot.x), MTXMODE_APPLY);
        Matrix_RotateZ(BINANG_TO_RAD_ALT(tranform->shape.rot.z), MTXMODE_APPLY);
