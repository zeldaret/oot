...
void func_80B395F0(EnWonderTalk* this, GlobalContext* globalCtx) {
    if (this->unk_156 == func_8010BDBC(&globalCtx->msgCtx) && Message_ShouldAdvance(globalCtx)) {
        if (this->switchFlag >= 0) {
            this->actor.flags &= -2;
            Flags_SetSwitch(globalCtx, this->switchFlag);
        }
