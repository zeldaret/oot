...
void MirRay_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((D_80B8E670 == 0) && !this->unLit && Player_HasMirrorShieldSetToDraw(play)) {
        ...
        if (!(this->reflectIntensity <= 0.0f)) {
            ...
            gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_mir_ray.c", 972), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            ...
            for (i = 0; i < 6; i++) {
                if (reflection[i].reflectionPoly != NULL) {
                    ...
                    gSPMatrix(POLY_XLU_DISP++, MATRIX_NEW(play->state.gfxCtx, "../z_mir_ray.c", 1006), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
