...
void BgGndFiremeiro_Shake(BgGndFiremeiro* this, PlayState* play) {
    ...
    if (DynaPoly_HasPlayerOnTop(&this->dyna)) { // Player standing on it
        if (this->timer > 0) {
            this->timer--;
...
void BgGndFiremeiro_Rise(BgGndFiremeiro* this, PlayState* play) {
    ...
    if ((player->currentBoots != PLAYER_BOOTS_HOVER) && DynaPoly_HasPlayerOnTop(&this->dyna)) { // Player standing on it
