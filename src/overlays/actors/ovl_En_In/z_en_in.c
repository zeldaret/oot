...
void EnIn_Init(Actor* thisx, PlayState* play) {
    ...
    this->waitObjectEntry = Object_GetEntry(&play->objectCtx, OBJECT_IN);
    if (this->waitObjectEntry < 0 && this->actor.params > 0) {
...
void EnIn_WaitForObject(EnIn* this, PlayState* play) {
    ...
    if (Object_IsEntryLoaded(&play->objectCtx, this->waitObjectEntry) || this->actor.params <= 0) {
