...
void func_80A7A304(EnIn* this, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_Update(&this->skelAnime) != 0) {
        ...
        if (this->animationIdx == 3 || this->animationIdx == 4) {
            Audio_PlaySfxGeneral(NA_SE_IT_LASH, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
            if (Rand_ZeroOne() < 0.3f) {
                Audio_PlaySfxGeneral(NA_SE_IT_INGO_HORSE_NEIGH, &this->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                     &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
            }
        }
        Animation_Change(&this->skelAnime, D_80A7B918[this->animationIdx], 1.0f, 0.0f,
...
void func_80A7A568(EnIn* this, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.timer1State == 10) {
        Audio_PlaySfxGeneral(NA_SE_SY_FOUND, &gSfxPosScreenCenter, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
