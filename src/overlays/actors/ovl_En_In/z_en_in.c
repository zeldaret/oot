...
s32 func_80A79830(EnIn* this, GlobalContext* globalCtx) {
    if (globalCtx->sceneNum == SCENE_SPOT20 && LINK_IS_NOT_ADULT && IS_DAY && this->actor.shape.rot.z == 1 &&
        !(gSaveContext.eventChkInf[1] & 0x10)) {
        return 1;
    }
    if (globalCtx->sceneNum == SCENE_MALON_STABLE && LINK_IS_NOT_ADULT && IS_DAY && this->actor.shape.rot.z == 3 &&
        (gSaveContext.eventChkInf[1] & 0x10)) {
        return 1;
    }
    if (globalCtx->sceneNum == SCENE_MALON_STABLE && LINK_IS_NOT_ADULT && IS_NOT_DAY) {
    ...
    if (globalCtx->sceneNum == SCENE_SOUKO && LINK_IS_ADULT && IS_NOT_DAY) {
