...
void EnMThunder_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (player->stateFlags2 & PLAYER_STATE2_17) {
        if (!gSaveContext.magicAcquired || gSaveContext.unk_13F0 ||
            ...
            Audio_PlaySfxGeneral(NA_SE_IT_ROLLING_CUT, &player->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
            Audio_PlaySfxGeneral(NA_SE_IT_SWORD_SWING_HARD, &player->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
        ...
        this->unk_1C4 = 8;
        Audio_PlaySfxGeneral(NA_SE_IT_ROLLING_CUT_LV1, &player->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                             &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
        this->unk_1AC = 1.0f;
    } else {
...
void func_80A9F408(EnMThunder* this, GlobalContext* globalCtx) {
    ...
    if (player->stateFlags2 & PLAYER_STATE2_17) {
        ...
        if (player->unk_858 <= 0.15f) {
            ...
            func_80A9EFE0(this, func_80A9F9B4);
            this->unk_1C4 = 8;
            Audio_PlaySfxGeneral(sSfxIds[this->unk_1C6], &player->actor.projectedPos, 4, &gSfxFreqOrVolDefaultVal,
                                 &gSfxFreqOrVolDefaultVal, &gSfxReverbAddNone);
            this->unk_1AC = 1.0f;
            return;
        }
