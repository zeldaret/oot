...
void EnMThunder_Init(Actor* thisx, PlayState* play2) {
    ...
    this->unk_1C7 = PARAMS_GET2(this->actor.params, 0, 8) - 1;
...
void func_80A9F408(EnMThunder* this, PlayState* play) {
    ...
    if (this->unk_1CA == 0) {
        if (player->unk_858 >= 0.1f) {
            if ((gSaveContext.magicState != MAGIC_STATE_IDLE) ||
                (PARAMS_GET2(this->actor.params, 8, 8) &&
                 !(Magic_RequestChange(play, PARAMS_GET2(this->actor.params, 8, 8), MAGIC_CONSUME_WAIT_PREVIEW)))) {
    ...
    if (player->stateFlags2 & PLAYER_STATE2_17) {
        ...
        if (player->unk_858 <= 0.15f) {
            ...
            player->stateFlags2 &= ~PLAYER_STATE2_17;
            if (PARAMS_GET2(this->actor.params, 8, 8)) {
