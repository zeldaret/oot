...
void BgSpot16Doughnut_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    } else if (globalCtx->csCtx.state != 0 && globalCtx->csCtx.npcActions[2] != NULL && (u16)globalCtx->csCtx.npcActions[2]->action == 2) {
...
void BgSpot16Doughnut_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (1) {}
    gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot16_doughnut.c", 213), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    if (this->fireFlag & 1) {
        gSPSegment(gfxCtx->polyXlu.p++, 0x08, Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, scroll*(-1), 0, 16, 32, 1, scroll, scroll*(-2), 16, 32));
...
void BgSpot16Doughnut_Draw2(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_80093D84(globalCtx->state.gfxCtx);
    gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot16_doughnut.c", 248), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gDPSetEnvColor(gfxCtx->polyXlu.p++, 0xFF, 0xFF, 0xFF, this->envColorAlpha);
    gDPSetPrimColor(gfxCtx->polyXlu.p++, 0, 0, 0xFF, 0xFF, 0xFF, 0xFF);
    gSPDisplayList(gfxCtx->polyXlu.p++, D_06000FC0);
