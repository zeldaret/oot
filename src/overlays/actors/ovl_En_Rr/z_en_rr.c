...
void EnRr_SetupReleasePlayer(EnRr* this, PlayState* play) {
    ...
    osSyncPrintf(TE_FGCOL(YELLOW) "%s[%d] : Rr_Catch_Cancel" TE_RST "\n", "../z_en_rr.c", 650);
...
void EnRr_CollisionCheck(EnRr* this, PlayState* play) {
    ...
    if (this->collider2.base.acFlags & AC_HIT) {
        ...
        osSyncPrintf(TE_FGCOL(GREEN) "カキン(%d)！！" TE_RST "\n", this->frameCount);
        ...
        if (this->collider1.base.acFlags & AC_HIT) {
            ...
            switch (this->actor.colChkInfo.damageEffect) {
                ...
                case RR_DMG_NORMAL:
                    // "ouch"
                    osSyncPrintf(TE_FGCOL(RED) "いてっ( %d : LIFE %d : DAMAGE %d : %x )！！" TE_RST "\n",
        ...
        if ((this->ocTimer == 0) && (this->actor.colorFilterTimer == 0) && (player->invincibilityTimer == 0) &&
            ...
            // "catch"
            osSyncPrintf(TE_FGCOL(GREEN) "キャッチ(%d)！！" TE_RST "\n", this->frameCount);
...
void EnRr_Death(EnRr* this, PlayState* play) {
    ...
    if (this->frameCount < 40) {
        ...
        // "dropped"
        osSyncPrintf(TE_FGCOL(GREEN) "「%s」が出た！！" TE_RST "\n", sDropNames[this->dropType]);
