...
void KaleidoScope_DrawDebugEditor(GlobalContext* globalCtx) {
    ...
    KaleidoScope_DrawDigit(
        globalCtx, ((gSaveContext.inventory.questItems & 0xF0000000) & 0xF0000000) >> QUEST_HEART_PIECE, 210, 185);
    ...
    switch (curSection) {
        ...
        case 0x5C:
            if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
                if ((((gSaveContext.inventory.questItems & 0xF0000000) & 0xF0000000) >> QUEST_HEART_PIECE) != 0) {
                    gSaveContext.inventory.questItems -= (1 << QUEST_HEART_PIECE);
                }
            } else if (CHECK_BTN_ALL(input->press.button, BTN_CDOWN) ||
                       CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                if ((gSaveContext.inventory.questItems & 0xF0000000) <= (1 << QUEST_HEART_CONTAINER)) {
                    gSaveContext.inventory.questItems += (1 << QUEST_HEART_PIECE);
                }
            }
            break;
