...
void FileChoose_SelectCopySource(GameState* thisx) {
    ...
    if (((this->buttonIndex == FS_BTN_COPY_QUIT) && CHECK_BTN_ANY(input->press.button, BTN_A | BTN_START)) ||
        ...
        Audio_PlaySfxGeneral(NA_SE_SY_FSEL_CLOSE, &gSfxPosScreenCenter, 4, &gSfxDefaultFreqAndVolScale,
        ...
        if (SLOT_OCCUPIED(sramCtx, this->buttonIndex)) {
            ...
            Audio_PlaySfxGeneral(NA_SE_SY_FSEL_DECIDE_L, &gSfxPosScreenCenter, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        } else {
            Audio_PlaySfxGeneral(NA_SE_SY_FSEL_ERROR, &gSfxPosScreenCenter, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        }
    } else {
        if (ABS(this->stickRelY) >= 30) {
            Audio_PlaySfxGeneral(NA_SE_SY_FSEL_CURSOR, &gSfxPosScreenCenter, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
...
void FileChoose_CopyAnim3(GameState* thisx) {
    ...
    if (this->actionTimer == 75) {
        ...
        Audio_PlaySfxGeneral(NA_SE_EV_DIAMOND_SWITCH, &gSfxPosScreenCenter, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
    }
...
void FileChoose_EraseAnim1(GameState* thisx) {
    ...
    if (sEraseDelayTimer == 0) {
        ...
        if (sEraseDelayTimer == 0) {
            Audio_PlaySfxGeneral(NA_SE_OC_ABYSS, &gSfxPosScreenCenter, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        }
    }
}
