...
void EffectSsEnFire_Draw(GlobalContext* globalCtx, u32 index, EffectSs* this) {
    ...
    Matrix_RotateY(BINANG_TO_RAD_ALT(camYaw), MTXMODE_APPLY);
        ...
        if (this->actor->update != NULL) {
            ...
            if (this->rBodyPart < 0) {
                Matrix_Translate(this->actor->world.pos.x, this->actor->world.pos.y, this->actor->world.pos.z,
                                 MTXMODE_NEW);
                Matrix_RotateY(BINANG_TO_RAD_ALT(this->rYaw + this->actor->shape.rot.y), MTXMODE_APPLY);
                Matrix_RotateX(BINANG_TO_RAD_ALT(this->rPitch + this->actor->shape.rot.x), MTXMODE_APPLY);
