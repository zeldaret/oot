...
u32 EffectSsSibuki_Init(GlobalContext* globalCtx, u32 index, EffectSs* this, void* initParamsx) {
    ...
    if (KREG(2) != 0) {
        this->gfx = SEGMENTED_TO_VIRTUAL(&gEffectSsBubble2Tex);
    } else {
        this->gfx = SEGMENTED_TO_VIRTUAL(&gEffectSsBubble1Tex);
    }
...
void EffectSsSibuki_Draw(GlobalContext* globalCtx, u32 index, EffectSs* this) {
    ...
    gSPDisplayList(POLY_OPA_DISP++, SEGMENTED_TO_VIRTUAL(gEffectSsBubbleDL));
