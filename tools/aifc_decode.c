...
#define UNUSED __attribute__((unused))
	...
	#define LOG_1(msg, a) printf(msg, a)
	#define LOG_2(msg, a, b) printf(msg, a, b)
	#define LOG_3(msg, a, b, c) printf(msg, a, b, c)
	#define LOG_4(msg, a, b, c, d) printf(msg, a, b, c, d)
	#define LOG_7(msg, a, b, c, d, e, f, g) printf(msg, a, b, c, d, e, f, g)
#else
	#define LOG(msg) 1==1
	#define LOG_1(msg, a) 1==1
	#define LOG_2(msg, a, b) 1==1
	#define LOG_3(msg, a, b, c) 1==1
	#define LOG_4(msg, a, b, c, d) 1==1
	#define LOG_7(msg, a, b, c, d, e, f, g) 1==1
#endif
...
s32 readaifccodebook(FILE *fhandle, s32 ****table, s16 *order, s16 *npredictors)
{
    ...
    BSWAP16(*npredictors);
	LOG_2("Decoding codebook: order = %d, tablecnt = %d\n", *order, *npredictors);
...
ALADPCMloop *readlooppoints(FILE *ifile, s16 *nloops)
{
    ...
    for (s32 i = 0; i < *nloops; i++) {
        ...
        BSWAP32(al[i].end);
        BSWAP32(al[i].count);
		LOG_4("looppoint[%d]=(start=%d,end=%d,rpt=%d)\n", i, al[i].start, al[i].end, al[i].count);
...
void my_decodeframe(u8 *frame, s32 *state, s32 order, s32 ***coefTable)
{
	...
	LOG_3("frame=%x scale=%x pred=%x\n", header, scale, optimalp);
    ...
    for (s32 j = 0; j < 2; j++) {
        ...
        if (j == 0) {
            for (s32 i = 0; i < order; i++) {
				LOG_2("i=%d j=0 state[%d]\n", i, 16 - order + i);
                in_vec[i] = state[16 - order + i];
            }
        } else {
            for (s32 i = 0; i < order; i++) {
				LOG_3("i=%d j=%d state[%d]\n", i, j, 16 - order + i);
        ...
        for (s32 i = 0; i < 8; i++) {
            s32 ind = j * 8 + i;
			LOG_7("i=%d j=%d product[%d] order=%d ix[]=%d, optimalp=%d coef[][]=%d\n", i, j, j * 8 + 1, order, ix[ind], optimalp, coefTable[optimalp][i] == NULL ? -8008 : *coefTable[optimalp][i]);
...
int main(int argc, char **argv)
{
	...
	LOG("writing compression header\n");
