...
def read_file(filename):
    ...
    pattern = re.compile("gRegsContext->data\[((0[xX])?[0-9a-fA-F]+)\]")
...
def main():
    parser = argparse.ArgumentParser(description="Converts a gRegsContext->data index to a REG macro.")
    ...
    index_group.add_argument("index", help="index of gRegsContext->data in decimal (or hexadecimal if starts with 0x)")
    index_group.add_argument("--offset", help="treat index argument as an offset to gRegsContext instead", action="store_true")
    ...
    file_group.add_argument("--file", help="use the parameter as a filepath, then replace every occurrence of `gRegsContext->data[number]` in that file a REG macro", action="store_true")
